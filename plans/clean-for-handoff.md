...
> **State / forward plan** (`docs/agents/BOZ.md §4`). **Status: DRAFT (2026-06-30, Boz + Jacob).** Purpose: get the codebase to a state where **human developers can be handed the package and it reads clean** — no corpse-lies, no dead paths mounted as if live, one render pipeline instead of forks, tabled work quarantined honestly.
>
...
> ▶ Design for the finish: **`_handoffs/HANDOFF-mobile-profile.md`** — ⭐ **now TRACKED (2026-09-02)**, along with `HANDOFF-preview-measurement.md`; the rest of `_handoffs/` stays local. They were ignored, so every pointer to them died at the clone boundary and this arc read as abandoned. Board rows: **`ROADMAP.md H1` · Publish-confidence / mobile-measurement** (names `mobile-profile B/C/E` + `render-conformance Ph4–7`; the hard gate is real per-tier device numbers — today `phone-hi==phone-lo`, both guessed) and `_archive/AUDIT-MATRIX-DIARY-2026-09-13.md` "mobile profile unauthored".
