...
## The runtime swap (depends on D1) — impostor geometry is VIEW-dependent, not scene-wholesale
**HiPointe gets BOTH street/hero AND overhead shots** (confirmed Jacob 2026-07-05). So an impostor-role tree is NOT one fixed geometry — it renders **the hero X in a street/hero shot** and **the overhead hula in the top-down browse shot** (the two impostors "differ by viewing hemisphere" — BATON). The X (vertical cross-planes, built P1) holds silhouette across azimuths for a horizontal look; the hula (horizontal ruched disc-layers, D1) holds vertical parallax + rim motion for a top-down look. Same tree, two constructions, selected by which way the camera looks.
...
- **Select the impostor geometry by the active VIEW context**, not the scene: hero/street camera → the hero `buildImpostorGeometry` cross (built); overhead/browse camera → `buildOverheadHulaGeometry` (D1). A top-down cross reads as an ugly line/X and a hero-viewed hula reads as flat stacked discs — each construction is for its own hemisphere. **How the runtime knows the active view context is the key D2 design question** — likely the same browse-vs-hero signal `presetFraming`/the camera mode already carries; confirm the seam before building, don't invent a parallel one.
- ⚠️ **Cull is also view-dependent** (reinforces the trap above): the hero-pan cull is right *for hero shots* but must not drop trees the *overhead* needs. Blunt-first: don't cull for the overhead; a proper per-view visibility (the deferred Browse oracle) is the real fix.
- If D1 hasn't landed when you validate **perf**, the hero cross is acceptable **for the GPU/frame-ms measurement only, with the QC tint** — never as the ship look.
