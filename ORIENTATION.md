...
- **⛔⛔ NO FALLBACKS — a fallback turns a failure into a plausible-looking success, which is the worst outcome a kit can have.** If town #2 fails to load and the operator is shown Lafayette Square, they do not see a bug; they see a map, and they trust it. Jacob, 2026-07-31: *"There should be no fallback; it would help exactly 0 to have a failed neighborhood loading to fall back to Lafayette Square."* **Measured, live, unfixed (`ROADMAP` A07):** the docs say *"the curb is a concentric offset"* — but `tileGround.js` has **two** curb producers and picks between them silently, sending **at least 30 of LS's 101 blocks** to the older boolean carve instead. Its own comment says *"Falling back to legacy is never a regression."* The carve may look fine; that is what makes it dangerous — on town #2 the operator reads the invariant, sees a plausible curb, and cannot tell which method built it. *(Previously this slot cited `measureModel.js` bleeding LS street widths into every town by street name — **fixed 2026-07-31**; when you close a fallback, re-arm this slot with a live one rather than leaving an expired receipt.)* Silent substitution is the defect — the same shape as a stale artifact that still renders, a scene that quietly isn't the one you built, and a skip list that makes unhandled cases look handled.
