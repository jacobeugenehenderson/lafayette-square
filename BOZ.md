...
| **skeleton** | **`skeleton.json`** | **`SKELETON.md`** (deep) · `PIPELINE §skeleton/P1` · `FEATURES` (the pitch) · `/ORIENTATION.md` (the doctrine) · `OPERATIONS` (knobs) | BACKLOG NOW | NOTES · `OSM-FORENSICS.md` |
...
| **Dead-ends** (cul-de-sac · stub · spike) | **`HANDOFF-dead-end-typology.md`** (3 cap types) · `OSM-FORENSICS §1` (node typology) · `SECTION.md §6`/D6a + G8 (cap *fill*) · the prune-attempt narrative → `NOTES.md` 2026-06-11 | render **clean WOVEN** with their authored cap (round bulb+wrap / flat abut). The pendant-prune was reverted (`dd4ddb6`, 2026-06-11; brief retired to git): asphalt is tile-sourced so pruning deletes the footprint, and the slit it fixed mostly isn't real (bollard's forensic). The cap is a free per-dead-end choice → no prune discriminator |
| **Divided roads / carriageways / medians** | **`RIBBONS.md §3.1`** (the live model — inner-edge anchor **LOCKED**) · forensics (archived): `cartograph/_archive/DIVIDED-CORRIDOR-PLAN.md` · `cartograph/_archive/TRUMAN-FORENSICS.md` (the parkway knot) | median emergent / E2-constructed; two-carriageway model locked |
| **Divided↔undivided transition** (false corner · the "d" bulge) | **`PREBAKE-POLYGONIZATION-PLAN.md`** · `SKELETON.md §5b/5d/5e/5f` · `HANDOFF-freeze-the-curb-in-the-first-bake.md` | false corner resolved (`9c275ce`); the "d" bulge = the unfrozen curb (the superseded `divided-transition-block-tongue` brief retired to git → freeze-the-curb) |
| **Junction / corner construction** (width-step family · aprons · de-taper) | **`SKELETON.md §5e`** (the live cure) · `OSM2STREETS-GROUNDING.md` (the standard) · forensic (archived): `cartograph/_archive/JUNCTION-CURE-PLAN.md` (E3) | E3.2/.3 landed; the corner **FILL** → `SECTION.md §6` |
...
| **Correctness suite** (the anti-hand-fix detector — one RED-until-true invariant per bug-class) | **`POLYGON-FIRST.md §5`** (the system + scope) · findings: `SIEVE-DETECTOR-FINDINGS.md` (width-step flagship) · `LOOM-TOPO-FINDINGS.md` (loop-closure + cap-tangent) · `THROAT-JUNCTION-FINDINGS.md` (junction-band) · harness `scratch/correctness-detector.mjs` | the detector is the deliverable, not the 6 fixes; validated against the 35 curated; the 35 lean topological → graph checks |
