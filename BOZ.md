...
- **⭐ Use the CSS doc — the suite has a design system, so build from it, never beside it.** `src/tokens/design.css` is the SSOT (`:root`-scoped: `--surface*`, `--on-surface*`, `--outline*`, the `--success/info/warning` status accents, `--type-*`, `--radius-*`); `src/index.css` imports it; `src/cartograph/cartograph.css` layers the `--carto-*` set on top, **scoped to `.cartograph`**. Two failure modes, both hit on 2026-07-20 building the Asset Library: (1) **colour literals inline in JSX** — `style={{ color: '#4a8' }}` can't be themed, can't be overridden, and drifts from the tokens everything else uses; put it in a class and let the stylesheet own the look. (2) **`--carto-*` in anything that portals to `<body>`** — those tokens die outside `.cartograph`, so every `var()` collapses and the panel renders with no background and unreadable text. Anything escaping the app subtree must build on the **project** tokens, which are `:root`-scoped and therefore portal-safe. ▶ **Two standing passes on this surface, neither yet run — pair them, they touch the same files.** (1) **"Extracting CSS from JS and HTML into a true product dashboard"** (Jacob, 2026-07-20) — the suite is carrying a lot of inline `style={{}}`, and consolidating it onto the token system is what makes the tools read as one product rather than a pile of operator panels. (2) **A real accessibility pass** (Jacob, 2026-07-21) — the operator tools have never had one: focus order and visible focus rings, keyboard operation of every control (the Materials source buttons and the Extent pen especially), contrast against the token palette in both standard and `.accessible` mode, `aria` roles/labels on the dialogs and expandable rows, and `prefers-reduced-motion`. The app side has an `.accessible` high-contrast mode already (`src/tokens/design.css`); the helpers do not honour it. Do this WITH the CSS pass — an a11y fix written against inline styles has to be redone once the styles move.
