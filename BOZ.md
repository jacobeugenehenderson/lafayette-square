...
**Handoff baton (the relay note).** A `BATON-*.md` is a SHORT, paste-able launch note that hands a FRESH agent into an existing brief — the relay hand-off, not a second brief. Jacob copies it into a new window to spawn the agent; it carries the decisive `Agent: FRESH/WARM` call + the routed reading path + the ONE cut to do, and points INTO the deep `HANDOFF-*.md` (or the canon §) for depth — **never a fork of it** (§3, one home). It lives and dies with that brief: retire it when the arc lands. Reach for it when a HANDOFF is too long to paste as the launch prompt, or to slice **one cut** off a multi-cut arc for an agent without re-stating the whole thing.
