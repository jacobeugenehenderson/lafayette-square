...
Today the live contract text is LS/Missouri values in **LS's own copy module**, `src/instances/copy/lafayette-square.jsx` (`COURIER_SECTIONS`, `RESTAURANT_SECTIONS`, the privacy body); an installation with no entry in `src/instances/copy/index.jsx` renders "not declared", never LS's terms (moved 2026-09-24). That is still two potential homes for the same fact. **The target state:** the JSX *derives* its sections from a parameterized legal module that reads rider values from `INSTANCE`, exactly as the runtime derives geography from `INSTANCE`. Then there is genuinely one home, and a new neighborhood's contracts regenerate from its rider. Until that refactor lands, this canon is authoritative and the JSX is propagated *from* it by hand — keep them in lockstep.
