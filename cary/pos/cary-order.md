...
1. **`ref` stability across menu edits — ✅ RULED 2026-09-10: stable ids, at the schema step.** `"sectionIdx-itemIdx"` is positional, so a Guardian reordering the menu shifts every ref behind it. The old note called this low urgency because "orders are short-lived" — **that was true only while nothing persisted.** Once orders are rows that get reconciled against a POS tender record and refunded weeks later, a positional ref names the wrong item; and a menu edited mid-cart re-points a live cart. Stable per-item ids land in Track B **before** persistence, not after. ⭐ They are also what makes menu **re-pour diffable** (a new PDF must match an existing item to leave it confirmed), so the id scheme is designed for re-pour, not just for orders.
