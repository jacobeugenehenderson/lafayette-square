...
- ⬜ Wire the requester **order submit** — the *order-capture* surface is **already built client-side** (`ls/CARY.md §6` — `MenuTab`: cart + full priced order + kitchen note, legal-canon-accurate, instance-parameterized tax). What's missing is everything past "Place order": **persist → Stripe food-PaymentIntent → `requests`/`sessions` row → dispatch → inject.** The canonical `CaryOrder` object is ~defined by that UI already; formalize it (add structured **modifiers**, a **Cary order ID**, the **food PaymentIntent id**) as the injection contract.
