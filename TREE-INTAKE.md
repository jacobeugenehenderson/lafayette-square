...
**The canonical doc for the neighborhood tree pipeline: from raw data → a baked, rendered canopy.** Generalizable per-town (Tier ①); Hi-Pointe/DeMun (`hipointe-demun`) is the reference build. Written 2026-07-05; revised 2026-07-16 (frozen-curb `makeZoneTester` mask + real-first doctrine; **Lafayette Square landed through this same pipeline** — normalized off its `DEFAULT_MAP` special-case, no longer the park-only exception).
