...
- **⭐ Perf / WEIGHT (the live frontier):** LS 5,768 + HPDM 10,352 instanced trees at real density. The cold-load audit (`?loadAudit`, `src/lib/loadAudit.js`) measured LS at **~73 MB of trees** hero-critical (`lod1` GLBs 39 MB + atlas 28 MB). The answer is NOT thinning trees (density is real) — it's the impostor foundation + **KTX2 the atlas** (28→~5 MB). ⚠️ **Built 2026-07-22, and it does NOT yet pay for itself:** the hero variety pool is ~70 MB of PNG against the ~39 MB of `lod1` it relieves, so compression moved from adjacent-nicety to prerequisite. Render state + open weight work: `arborist/ARCHITECTURE.md §"Tree-render reality at LS"`.
