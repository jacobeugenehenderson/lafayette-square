...
export default function Panel() {
  ...
  return (
    <div className="carto-panel">
      ...
      {/* The layer-visibility + Look sections live under the DESIGN tool (no
          tool active) — Survey/Measure show only their own authoring controls.
          Corner radius moved to Survey (its CornersSubsection); Blocks › Shape
          keeps only curb width here. */}
      {!tool && sections.map(([name, defs]) => (
        <Section key={name} name={name}
          ...
          {/* Block-shape controls live above the visibility list — curb width
              shapes every block's silhouette, conceptually upstream of "what do
              I render". */}
          {name === 'Blocks' && (
