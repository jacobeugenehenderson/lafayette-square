...
/**
 * NeonBands v2 — wall-mounted glass-tube signage along the rooftop
 * perimeter of every open place. One merged mesh per scene, one draw,
 * one ShaderMaterial. Fresh rewrite to settle the long iteration on
 * v1 — see git log for the trail.
 *
 ...
 *   4. Index winding emits outward-facing triangles → FrontSide renders
 *      the visible (outside) half of the tube; the inside half is
 *      backface-culled. No DoubleSide → no symmetric brightness wash
 */
...
export default function NeonBandsV2({ places, forceOn = true, lookId }) {
  ...
  const geometry = useMemo(() => {
    ...
    for (const p of places) {
      if (!p.neon?.category) { _tubeSkipNoCat++; continue }
      const tube = buildTube(p)
      if (!tube) { _tubeSkipBuild++; continue }
      _tubeOk++
  ...
  if (!materialRef.current) {
    materialRef.current = new THREE.ShaderMaterial({
      ...
      depthWrite: false,
      toneMapped: false,
      // DoubleSide so every camera angle sees SOMETHING of the tube
      // (FrontSide-only culls the half facing away → tube only visible
      // from one side at a time, sub-pixel at hero distance).
      // The fragment shader does NOT flip the normal on back faces —
      // back faces compute dot(N, V) < 0 which clamps to 0, producing
      // a dim bleed-only halo on the camera-far side. That's the
      // physically correct look for an omnidirectional glass emitter:
      // bright on the near side, soft glow on the far side. Earlier
      // attempts that did flip the normal made both sides equally
      // bright and washed the curve into a flat ribbon.
