...
function getResultStyle(result) {
  ...
  if (type === 'building') return { hex: '#7A8B6F', icon: CATEGORY_ICONS.building }
...
// ── Shared search hook — used by EventTicker pulldown drawer ──────────
export function useGlassSearch() {
  ...
  const results = useMemo(() => {
    ...
    const bldgs = matches.filter(m => m.type === 'building')
    ...
    return [...places.slice(0, 4), ...bldgs.slice(0, 4), ...uniqueMenuTypes.slice(0, 4), ...Object.values(itemsByPlace).flat().slice(0, 6)].slice(0, 10)
  }, [query, searchIndex])
  ...
  return (
    ...
    >
      {logo ? (
        ...
        <svg className="w-4 h-4" style={{ color: hex }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          {icon}
        </svg>
      )}
    </div>
...
export function SearchDropdown({ results, selectPlace }) {
  return (
    <div className="glass-dropdown mt-1.5 rounded-xl">
      {results.map((result, i) => {
        ...
        if (result.type === 'building') {
          ...
          return (
            <button key={`bldg-${b.id}`} onClick={() => selectPlace(null, b, 'building')} className="w-full px-4 py-2.5 flex items-center gap-3 hover:bg-white/5 transition-colors text-left">
        ...
        if (result.type === 'menu-type') {
          return (
            <button key={`mtype-${listing.id}-${result.menuType}-${i}`} onClick={() => selectPlace(listing, null, 'menu-type')} className="w-full px-4 py-2 flex items-center gap-3 hover:bg-white/5 transition-colors text-left">
        ...
        if (result.type === 'menu-item') {
          const { item } = result
          return (
            <button key={`menu-${listing.id}-${item.name}-${i}`} onClick={() => selectPlace(listing, null, 'menu-item')} className="w-full px-4 py-2 flex items-center gap-3 hover:bg-white/5 transition-colors text-left">
              <CategoryIcon hex={style.hex} icon={style.icon} logo={logo} />
              <div className="min-w-0 flex-1">
        ...
        return (
          <button key={`place-${listing.id}`} onClick={() => selectPlace(listing, null, 'place')} className="w-full px-4 py-2.5 flex items-center gap-3 hover:bg-white/5 transition-colors text-left">
            <CategoryIcon hex={style.hex} icon={style.icon} logo={logo} />
            <div className="min-w-0 flex-1">
