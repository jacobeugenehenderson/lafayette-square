...
function BusinessCard({ landmark, building, onClose }) {
  ...
  const rating = building?.rating || (landmark ? 3.5 + (name.length % 15) / 10 : null)
  const reviewCount = building?.review_count || null
  const photos = building?.photos || landmark?.photos || null
  ...
  const amenities = building?.amenities || null
  const history = building?.history || null
