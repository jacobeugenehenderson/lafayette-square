...
/* ⛔ `heroAimTarget` LIVED HERE FOR ONE AFTERNOON and is gone. It held the
   subject at an authored MARK in the frame — [sx, sy] in NDC — so that pitch
   could be chosen while the Hero Lock still guaranteed the subject stayed in
   shot. It worked, and it was the wrong shape: one mark for a whole move, and a
   second mechanism to maintain beside the keyframes. Jacob: "we can get rid of
   the lock because it's not as much to manage as I feared."
   ⭐ Keyframes carry `target` now, interpolated on the same parameter as
   `position` (heroAnim.js). Pitch is not a feature — it is where you pointed
   the camera. Archived: git show 5e8f9eca:src/lib/heroSubject.js */
