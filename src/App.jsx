 ...
 *
 * ⭐ Exists because the inbox had no indicator of any kind: messages arrived,
 * sat there, and nothing anywhere said so — the operator did not know the
 * feature existed (2026-08-24). An alert SMS is easy to miss or bury; this is
 * the fallback that does not depend on remembering.
 */
function SmsUnreadBadge() {
  const n = useSmsInbox((s) => s.unreadCount)
  if (!n) return null
  return (
    <span className="ml-auto min-w-[1.25rem] px-1.5 py-0.5 rounded-full bg-red-500 text-white text-caption font-medium text-center leading-none">
      {n > 99 ? '99+' : n}
    </span>
  )
}
