...
**⭐ Depth regime is the marquee global-quality knob (forensic verdict 2026-05-26, [[project_production_linear_depth_gap]]).** Production is LINEAR; authoring surfaces are LOG. **Desktop → LOG** (validated, far-field gain, N8AO/post-fx fine under it, NeonBands auto-adapts). **Mobile → stays LINEAR pending a virtual-phone measurement** — a global LOG flip writes `gl_FragDepth` on mobile WebGL2 → kills early-Z → taxes the canopy fill-rate budget the tree arc fights. **MEASURE-FIRST discipline:** the mobile early-Z penalty's magnitude is *unprovable from the repo* — decide mobile's depth (and validate the other mobile-quality values) via the Phase-4 phone meter, don't assume. This is the cleanest proof that the depth regime belongs in `INSTANCE.mobileQuality`: it's a per-device Canvas-construction knob, exactly this home.
