...
- **Leg ribbons and corner pads are two passes that must share edges.** The band-stripe pass (`StreetRibbons.jsx` / `ribbonsGeometry.js`) renders each chain from start to end at perp = `pavementHW`, `pavementHW + curbW`, and `pavementHW + curbW + ped` per side. The corner-pad pass (the IP rule, see `cartograph/BACKLOG.md §"Pad geometry from NACTO/AASHTO/ADA convention"`) builds a separate polygon at each intersection vertex V. **The pad polygon's outer edges along each leg must coincide with the band-stripe pass's perp positions on that leg** — they share an outline. The pad is not a free-floating shape that "looks like a corner"; it closes the open ends of the leg ribbons at V. When debugging visual artifacts, verify this continuity contract first (see BACKLOG for the per-leg perp checks). Most "the corner looks weird" bugs are coord-space mismatches between the two passes.
