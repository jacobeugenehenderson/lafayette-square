...
| # | Divergence | Type | The truth | Locus | Status |
|---|---|---|---|---|---|
| B7 | bake-route skips `pipeline` with the message *"scene-specific pipeline not yet implemented"* | ⛔ **REGRESSION-shaped live defect — reclassified 2026-09-23 (Boz). The message is TRUE; the GATE is the defect.** | The Bake handler runs `pipeline` + `promote-ribbons` only when `bakeScene === DEFAULT_MAP` (LS); every other town skips them, so a skeleton change never reaches `ribbons.json` / Survey except through the Extent **Pour**. Measured on huron 2026-09-23: skeleton 10:35 carried A19's 5 chains, `ribbons.json` was Sep 20, the Bake's `shape.json` carried 0. ⚠️ The dirty-check output is LS's `src/data/ribbons.json`. *(Was filed as a corpse-lie comment — "update the message" — which would have hidden it.)* | `cartograph/serve.js`, the bake handler's `isDefaultMap` gate | ▶ fix inside `ROADMAP A19` |
