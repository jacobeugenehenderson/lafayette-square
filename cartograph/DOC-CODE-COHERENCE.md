...
| D9 | **Divided-carriageway CHAIN POSITION contradiction** — `FEATURES §371` says the chain stays at the carriageway **center** (OSM way center); `PREBAKE-POLYGONIZATION-PLAN §2` + D1 assumed it sits at the **inner edge** (gap≈median). The emit's correctness (symmetric vs inboard-zeroed/offset) depends on which is true — likely behind "asphalt emits from the left of the lane, not center." | `FEATURES §371` vs `PREBAKE-POLYGONIZATION-PLAN §2` / D1 | 🔎 **resolve against the aerial** (operator's eye) before any divided-road build; may mean D1 offset the asphalt on center-chains |
