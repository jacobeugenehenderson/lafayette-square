...
3. **Delete** the raw-OSM face path (`vehicularStreets`→`densify`→`nodeEdges`→`polygonize`→`classify` as a *faces* source) + `ribbons.faces`/`intersections` from the artifact. `medians[]` has **zero `src/` consumers** (the vestigial decoy, per the Truman finding) — delete in the same pass.
...
| **D2** | **Prebake face freeze (topology).** Move `extractFaces` to prebake over skeleton chains; freeze `tiles[]` (ring + per-edge `(skelId, side)`) into the artifact; `tileGround` consumes frozen tiles. Decide the smoothing wrinkle (§1). **No corner change.** | — | M | **Byte-identical render A/B** (frozen-vs-derived assert harness + screenshot diff). The freeze must be invisible. |
...
| **D4** | **LU at prebake + retire raw-OSM faces (C5).** §3's three steps; delete `medians[]`. | D2 | L | LU-color A/B byte-identical; IX markers/corner handles unchanged. |
