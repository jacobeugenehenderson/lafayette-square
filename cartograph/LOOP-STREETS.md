...
- **L.5** 🔧 LS: Benton + Waverly both **render at sane widths now** (the bad-data guard, Stadia `8cdc0d4`) — collapse fixed, Waverly pills gone. **✅ The enclosed FACE now forms (2026-06-11, `e8cc310`, §4):** the endpoint-weld closes Benton's 3.2 cm / Saint Vincent's 2.2 cm gap so all three teardrop/bulb loops emit a proper loop road + grass interior (Park Place was already right). **REMAINING — the role cross-sections (§2) are still partly unmodeled:** `chain.loop{loopId,role}` doesn't exist, so `isMedianTile` ped-zeros the *center* (the median ring) but the loop body's interior face still carries a full ped band → **Benton renders an inner sidewalk ring** where §2 says the inner (median-facing) side is curb+treelawn only (treelawn flows into the median). **Waverly's cut-thru renders full ROW** (§2 says curb+asphalt only). → the **L.3 role-model** (the inner-side ped-zero for the loop body) is the current follow-up. **Also: Benton's stem-joint shows the perpendicular-join protrusion** (Jacob 2026-06-05) — a junction-construction artifact (NOT a loop/width issue), the same class as the T-base bulges → **E3** (BACKLOG). Benton's full bless waits on the role model + E3.
