...
> ⚠️ **RESOLVED by the grounding (2026-06-06, `OSM2STREETS-GROUNDING.md`).** **"Loop street" is NOT a standard concept.** In the reference model Benton is just `Roads` + a `LandUseBlock`; the roles compile down to asymmetric per-side lane lists. **Keep "loop" as an authoring-card NAME only**, not a geometry concept — the topology/median is standard road-casing + the LandUseBlock. And **18th's "loop" was never a loop**: it was a divided MIS-detection (a `motorway_link` ramp + a service drive mis-paired by our *geometric* detector) — fixed **data-first** (`870a1fd`, `skeleton.js` `carriagewayGates`: class gate + ramp/service exclusion + split/rejoin trace; brief never filed — see `OSM2STREETS-GROUNDING §4.2`). Read the rest of this doc as the authoring-card reference, not a model.
...
`overlay.loops[]` (per-scene, canonical) — ✅ **the override IS wired**: `derive.js` reads `overlay.loops` and resolves `overlayLoops[loopId]?.interior || 'median'`, keyed by body `skelId`. ⛔ **`chain.loop = {loopId, role}` does not exist** in `skeleton.js`, `derive.js` or `tileGround.js`, and LS's `overlay.json` has no `loops` key at all — this is the L.3 design, not the as-built shape (§6 concedes L.3 isn't built, but this section reads as current). Auto-detect runs in the pipeline; operator override via Survey UI (the loop card). *(The detect + override UI is L.3, not yet built — see §6 status.)*
...
- ✅ **LIVE — tile handling.** `tileGround.js`: median-tile detection — ⛔ **`isMedianTile = isDividedMedian || (isLoopInterior && medArea > 0.5)`**, where `isLoopInterior = runs.length === 1`. **There is NO boundary-fraction test.** `medArea > 0.5` is an absolute m² floor on the constructed median ring's overlap. *(This bullet said ">40% median-facing boundary" and §6 L.5 said "the >50% area ratio"; the doc carried two thresholds and the code has neither. Corrected 2026-08-04 — anyone tuning why a loop interior did or didn't zero its ped band was hunting a threshold that does not exist.)* the **thin-tile capacity guard** (`thinTile = 2·area/perimeter < cw+tl+sw` → `bandJoin='round'`); single-run-loop rounding. These keep a thin loop interior from thorning.
- ✅ **LIVE — the endpoint-weld closes the enclosed face (2026-06-11, `e8cc310`).** ⭐ **The fix that finally made the emergent-median model work.** `tileGround.extractFaces` welds near-coincident chain **endpoints** (within `ENDPOINT_SNAP = 0.15 m`) to a shared node *before* the face walk. A body that closes **above** the 0.1 mm node quantization — measured 2026-06-11 as Benton **3.2 cm**, Saint Vincent **2.2 cm**; ⚠️ **re-measured 2026-07-31 both close EXACTLY (gap 0.000), as does Park Place** (`scratch/` inline census), so the weld currently fires on NOTHING in LS — the frame moved under it. Keep the weld (it is general, and a re-pour can reintroduce a near-miss), but do not cite those two numbers as live — otherwise reads as an **open chain whose two endpoints are distinct nodes**, so its ring never closes and its interior face never forms. The weld gives the near-closed loops the footing **Park Place (gap 0.000) always had** — Park Place closed on its own and rendered right the whole time; that was the tell. With the face closed: the loop **road** emerges from the interior+exterior tile pavement strips, and the interior ped-zeros to **grass**. ENDPOINTS only (mid-chain geometry untouched), tolerance well below junction separation, so **general** — any near-miss endpoint slit closes, not just the named loops. **Audited clean** (`scratch/weld-audit.mjs`): only Benton + Saint Vincent welded, max 3.2 cm, **no spurious junction merges**; faces 101→103 (+2 = the two interiors), verts 795→881 (peeling still removes genuine dead-end spurs).
- ✅ **LIVE — loop interior emitted as `kind:'median'` (2026-06-11, `e8cc310`).** `derive.js` emits each snap-closed loop interior as a constructed `kind:'median'` ring (inset **`hw + curb`** — pavement half-width **plus the curb only, ≈ hw + 0.15 m**, deliberately *not* past a sidewalk, so the grass ring covers the interior face; guarded by a 20 m² min so a tiny turning circle that collapses stays paved). *(This read "pavement+curb+sidewalk ≈ 5.6 m" — §6 L.5 of this same doc described it correctly, and the code comment cites §2 as the reason NOT to inset past a sidewalk. The difference is the whole §2 body cross-section: hw+curb covers the interior face so `isMedianTile` fires; 5.6 m would not.)*. This is the **loop's `isMedianTile` trigger** — the LOOP-body median keys on its `ribbons.medians` ring overlap (the frozen `med`). ⚠️ **DIVIDED medians no longer share this path** (2026-06-16): a divided median is a derived walked face, face-read (a tile bounded by both carriageways of a pair), **no ring** — `RIBBONS §3.5`. So the two median paths now diverge: **loop = ring-clip; divided = `luRemainder` flood.** Unifying the loop interior onto the divided walked-face path is an open follow-on. Per-loop override `overlay.loops[loopId].interior` (`'median'` default | `'block'` = the 18th-St subtype, emit nothing). *(The pragmatic reconciliation of L.0's "median is emergent, never a separately-authored polygon": the **FACE** is emergent from the weld; the median **RING** is just the loop's detection trigger painted onto it — geometry still comes from the face, not the ring.)*
- ⚠️ **LIVE, AND WRONG — `derive.js LOOP_STREET_NAMES` (`:1416`)** = `['Benton Place','Mackay Place']`. ⛔ **DO NOT DELETE IT.** *(This line read "⛔ DEAD … **Delete**" until 2026-08-02; verified false — it is dereferenced **8×** at `derive.js:1426/1611/1716/1740/1942/1997/2118`, all inside `deriveLayers` (`:1008`), which `pipeline.js:130` runs on **every pour**. Following the old instruction breaks the pour.)*
  **The real defect is that it is a hard-coded LS name list at all** — Layer 0: it is an instance patch in the SHAPE producer. On town #2 it matches nothing and every loop-cut path silently no-ops. The content is also wrong *here* (Mackay isn't a loop; Waverly is missing), which is the tell — a name list nobody can validate. **The fix is to derive loop membership from the frame** (`isClosedLoop` already exists in `skeleton.js`, §4 above) and delete the list *as the last step of that migration*, never before.
...
- **L.5** 🔧 LS: Benton + Waverly both **render at sane widths now** (the bad-data guard, Stadia `8cdc0d4`) — collapse fixed, Waverly pills gone. **✅ The enclosed FACE now forms (2026-06-11, `e8cc310`, §4):** the endpoint-weld closes Benton's 3.2 cm / Saint Vincent's 2.2 cm gap so all three teardrop/bulb loops emit a proper loop road + grass interior (Park Place was already right). **✅ The §2 BODY cross-section now renders (2026-06-11, `ed250b3`):** the loop median ring insets to the CURB's inner edge (`hw + curb`, not past a sidewalk) so the grass fills the interior face → `isMedianTile` fires (now also via `isLoopInterior = runs.length === 1`, a single-street enclosed face, independent of any area *ratio* — the test is the absolute `medArea > 0.5` m² floor) → the inner ped band zeros. **Benton's inner sidewalk ring is gone** — outer side = full ROW (treelawn + sidewalk + curb), inner side = curb + grass, treelawn flowing into the median, exactly §2. **REMAINING:** **Waverly's cut-thru** (type B) is a different topology (not a single closed chain) and still renders full ROW (§2 says curb+asphalt only) — the couplet role is the open piece. The small turning-circle bulbs (Park Place, Saint Vincent) keep a thin walk ring around their island, which reads fine for a roundabout. A full `chain.loop{loopId,role}` tag would generalize this (Waverly, cut-thrus); the targeted `isLoopInterior` detection covers the teardrop/bulb body without it. **⛔ Benton's stem-joint — UNPARKED 2026-09-05.** It sat here as *"a junction-construction artifact, the same class as the T-base bulges → E3; Benton's full bless waits on E3"* — **for three months, behind a campaign that has now been closed without being built** (Jacob: *"E3 is superseded"*). ⭐ **MEASURED, and it is not a junction construction:** at the loop joint `(58.69, −233.97)` — where the closed one-way body `benton-place-1` meets the two-way stem `benton-place-0`, three arms at one node — the frozen curb carries two `filletRing` arcs 3.3 m out, **θ 29.9° / 51.4°, insets 15.6 m / 9.4 m**, achieved `r` 4.167 against an authored 4.5 (the 45%-of-gap clamp firing silently). Those are the curls on screen. ⛔ **Cause of the acute wedge NOT established** — the arms at the node are 133.4°/142.7°, where the averaged-normal branch is only ~8% short of the true miter, so the branch is present but not shown to be the producer. ⭐ The class ports and names no street: **44/513 LS · 174/898 HPDM · 336/6736 Altadena** fillets under 60°, worst insets 44–91 m. ▶ `node scratch/benton-grout-joint.mjs`. **Benton's bless now waits on the grout** (`RIBBONS §1`), where `filletRing` retires.
- **L.6** ⏭ **RESCOPED 2026-08-02** — *not* a cleanup/delete. `LOOP_STREET_NAMES` is **live on every pour** (§4); the work is to **derive** loop membership from the frame so the hard-coded LS name list can be retired behind it. Eye-gated (it moves loop-cut + median-creation paths), **not** a delete.
