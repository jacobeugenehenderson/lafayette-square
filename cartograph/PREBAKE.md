...
- **Building MEMBERSHIP — inside-circle − exclusion loops + overrides** (the excluder model, 2026-07-14, supersedes the boundary-street polygon): every building inside the circle is IN by default; the operator's flattened **exclusion loops** (`nb.exclusions`) carve strays OUT via `pointInPolygon` (`pipeline.js:242-254`), *plus* per-building `activate`/`hide` overrides (`NEIGHBORHOOD-INPUTS §5.2`). The **circle** is the boundary + slab disc/fade. Applied in `pipeline.js` so `map.json` is the single filtered source (2D Designer + bake); `bake-buildings.js:603-613` re-applies the same belt-and-suspenders. A legacy hood's inclusion `nb.polygon` is dropped on excluder re-bake (`24323ab2`).
