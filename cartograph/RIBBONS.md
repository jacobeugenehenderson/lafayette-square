...
> - **The campaign (two halves of one move):** **(1) intersection-everywhere** — construct the intersection polygon at EVERY node (trim-back + corners-by-clockwise-adjacency), promoting E3 from its ~86 censused nodes (the `OSM2STREETS-GROUNDING §4` recommendation #2); **(2) constructed generic median** — a positive symmetric strip from the carriageway gap, inboard `pavementHW` clamped to the median edge, painted positively (not the subtractive `luRemainder`). **Brief: `HANDOFF-construct-hard-polygons.md`.**
...
- **Divided carriageways stay two centerlines; the median is an emergent geometric face.** `anchor:'inner-edge'` zeroes the inboard ped zone; the chain stays at carriageway center. The median is whatever face `extractFaces` walks between the two carriageways — **not** a chain-identity consequence, **not** an authored object. The two-carriageway model is **LOCKED** (no pair synthesis, no collapse to a single spine). Frame topology (longitudinal weld, station-overlap pairing, `phase.spineAt*` the frozen transition link) lives in `SKELETON.md §2/§3` + `_archive/TRUMAN-FORENSICS.md`.
...
- **Median:** `medianClipFor` / `isMedianTile` route a loop/divided interior to grass (ped-zero), inset to the curb (`LOOP-STREETS.md`).
