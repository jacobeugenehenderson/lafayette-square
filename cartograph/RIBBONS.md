...
> The tile model replaced the **figure-ground** regime (blocks-as-positive, streets-as-subtracted-void) in the ~2026-06-01 re-pour. Figure-ground (`buildBlockGeometryV2.js`) is **dead in the live render + bake** — archived at [`_archive/RIBBONS-figureground-emitter-2026-06-15.md`](_archive/RIBBONS-figureground-emitter-2026-06-15.md). If you find yourself reading about `blockSharp` / `asphaltRounded` / `cornersAtIx` / `silhouetteStraightEmitter`, you are in the dead path.
...
> ⚠️ **The Designer authoring overlays still mount the DEAD figure-ground compute** (`BlockGeometryV2Debug.jsx` mounts `buildBlockGeometryV2`; only its meshes early-return). This is a real perf/reliability drag and the unifying cause of the authoring-on-the-dying-path cluster. **T3 migrates the authoring channel onto tiles; T4 deletes figure-ground.** Until then: do NOT fix authoring in-place on the dying path, and do NOT delete it (it returns in T3, migrated). (`HANDOFF-tile-feature-ledger.md` A-note.)
...
### 6.4 Dead-end mouth-collapse — OPEN (the polygon-path fix is iA-gated; 2026-06-22)
Where a side street **dead-ends/T's into a through street**, `extractFaces` walks it as a **zero-width out-and-back spur** — the face's mouth vertex revisits its own coordinate (tile[53] Albion: `ring[1]==ring[3]`, 0.0 m). The FILL keys corners **by vertex** (`cornerT`), so the **two mouth corners collapse onto one key** → only one fillet wraps, the other **butt-caps** (Section-only; the curb is smooth in Survey, and `iA` already carries *both* mouth fillets). The doctrine fix is to **build the face correctly at the source** — open the spur to a width-wide notch so the FILL sees two corners (a `RIBBONS §1` "construct the hard polygon" move, NOT a FILL patch). ⛔ **BLOCKING CONSTRAINT (decisive, `DEAD-END-MOUTH-FORENSIC.md`):** any ring-reshape **moves `iA`** — the mouth corner is a *depth-dependent offset-intersection* (frontage hw ∩ the depth-bearing dead-end wall), and `offsetRingVariable`/`filletRing` re-round on *any* perturbation (false at 2 mm; **catastrophic 98 m on multi-spur tiles** like tile[11]=6/tile[43]=7). The visual fix works (mouth wraps, proxy-verified) but **iA byte-identity is unachievable** — the gate must move to the eye + a bounded spur-tile delta + multi-spur robustness clamps (open decision). Scope ~50 spurs / ~21 faces; preserve `kennett-place`/`park-avenue-1` customs. **Companion:** dead-ends must be **strip-swappable** (the cap ped-wrap forces SW today — the FILL side, depends on the notch landing). Full forensic + the wrong turns: `DEAD-END-MOUTH-FORENSIC.md`.
