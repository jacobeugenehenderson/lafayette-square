...
- [ ] **🚨 De-Parking session** (DECIDED 2026-05-03). Eliminate the park frame; absorb park into one neighborhood frame. Migrate park-local data (trees, water, paths) to neighborhood frame at rest, delete all PARK_GRID_ROTATION callsites, extract `terrainCommon.js` for V_EXAG / getElevation / bounds. Closes "floating buildings", "Stage vs Preview drift", "rotation in 5 files", "park-as-entity?" — these are five disguises of one decision. Real refactor session, not patchwork. See `project_de_parking_decision.md` for the strategic memo + step-by-step. Supersedes the rotation-formalize and park-as-entity backlog items.
