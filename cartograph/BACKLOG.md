...
- **SC.2 — Post-FX**. Bake: `scene.postFx`. Consume: production `PostProcessing` reads it; Preview already reads same source. **Parity audit:** Stage's post-FX chain in `StageApp.jsx` may share `PostProcessing` with production (check); `PreviewPostFx.jsx` is a separate consumer that also touches the store (1 hit). Audit before touching.
- **SC.3 — Exposure / tonemapping**. Bake: `scene.canvas` (or fold into `scene.postFx`). Consume: Canvas-level `gl` prop reads the value. **Parity audit:** Canvas mount is per-host (Scene.jsx, CartographApp.jsx, PreviewApp.jsx) — three separate Canvas-prop-passing sites that need to be aligned, not a shared consumer fork.
- **SC.4 — Time defaults & overrides**. Bake: `scene.time`. Consume: `useTimeOfDay` initializer accepts authored hour / sun curve. SC.4 was effectively empty as of c333e50 — DawnTimeline doesn't persist anything; revisit when it grows a "save default" surface.
- **SC.5 — Per-shot camera**. Bake: `scene.shots` (Browse/Hero/Street). Consume: `Scene.jsx`'s shot table reads authored positions where present, defaults otherwise. **Parity audit:** Stage's CameraRig (in `CartographApp.jsx`) and production's CameraRig (in `Scene.jsx`) are separate camera-driver implementations — likely a real fork. Consolidation candidate.
...
- **SC.7 — Arch tuning**. Bake: `scene.arch`. Consume: production `GatewayArch` reads it. **Parity audit:** `src/stage/StageArch.jsx` exists alongside `src/components/GatewayArch.jsx` — known fork. SC.7 includes the consolidation (same shape as SC.1's StageSky retirement).
