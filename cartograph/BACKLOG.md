...
- **`43ca3fe` — Add `tubeRadius` as a TOD-animatable field in the Neon channel.** Slider in Cartograph Sky & Light Neon panel (range 0.1–3.0 m, default 1.0 m); writes per-Look value into `scene.json.neon.values.tubeRadius`; consumed by `NeonBands.buildTube` and the geometry `useMemo` deps. Forward-compat: Looks without the field default to 1.0 m unchanged. See `feedback_geometry_affecting_channels_need_debounce` for the rebuild-cost doctrine.
- **`9805305` — Fix `tubeRadius` slider debounce never elapsing under steady fps.** The original 120 ms debounce kept resetting itself when the per-frame `useFrame` poll fired faster than the timeout interval; pause-then-commit semantics broken. Fix: only schedule a new debounce when the quantized value actually changes from the last quantized commit.
- **`e2b9095` — Decouple neon from Society Pages tags + drop `tubeRadius` debounce.** Society Pages `activeTags` no longer force `info.forceOn = true` on `neonLookup` entries (per `feedback_panel_is_source_of_truth_for_authored_channels`); panel toggle becomes the master gate. Once the additive-OR was removed, the underlying rebuild churn that motivated the debounce went away, so the debounce machinery was retired in favor of plain step-quantization.
- **`6fcbd2d` — Stage / production gate split via prop presence.** `NeonBands` consumes its gate via `forceNeonOn` prop presence: passed = Stage path (panel is sole master); omitted = production path (hours is sole arbiter). Eliminates the prior additive-OR confusion. **Also incidentally landed** the long-pending `frustumCulled={false}` on `Foundations` + `Building` meshes — third 2026-05-14-claimed fix discovered missing this session (alongside the anchor rule and the `patchTerrainAtCentroidRaw` helper orphan, both landed in `3e51641`). See `feedback_verify_doctrine_against_code` memory.
- **`766bfcc` — Extend neon eligibility to every building.** `openPlaces` now iterates all 1082 `_allBuildings`, not just listings-authored ones. Default color per non-listing building derives from the St. Louis zoning code (1036 of 1082 records populated): `A B C D E` → residential (Sage), `F G H I` → services (Prussian Blue), etc. — see `LafayetteScene.jsx`'s default-neon-color helper for the canonical table. Stage Force On = whole-city neon; production behavior unchanged. Bake-side eligibility is **out of scope here** — queued as the next future arc.

**Deferred items** (queued for future arcs):
- **Bake-side eligibility.** Today's extension is runtime-only; tubes are runtime-built per-building. The eventual baked-asset architecture (per-building baked tubes shipped in `scene.json` / a sibling `.bin`, lit subset chosen at runtime) is the natural next step. Tier 1 = businesses with hours (automatic eligibility); Tier 2 = other buildings (opt-in via PlaceCard Events tab — UI is also future work).
- **`Building`'s dead `neonInfo` block cleanup.** `categoryHex` / `listingHours` / `neonForceOn` / `showNeon` / `effectiveHex` derived locals (`LafayetteScene.jsx:629–633`) are unreferenced downstream. The frustum-cull `useFrame` at `LafayetteScene.jsx:~1313` only feeds these dead vars. Small cleanup commit.
- **Real frustum / LOD cull at 1082-tube scale.** Geometry rebuild on slider movement is heavier than the prior 30–83-tube scale; still acceptable but worth profiling.
- **Hospitality category gap in `CATEGORY_HEX`.** Surfaced during the eligibility extension; some buildings classify as a category not yet present in the hex table.
- **PlaceCard Events tab for per-building neon opt-in / hours authoring.** Adjacent feature, separate UX arc.

Canonical close-out narrative: `scratch/handoff-2026-05-18-neon-arc-closeout.md`.
...
- **Foundation/wall anchor rule corrected.**
  > **2026-05-18 addendum (fix actually landed).** Investigation surfaced that the 2026-05-14 fix described below never actually landed in LS-live code — Foundations were still stamping single-point centroid into `aCentroidY`, walls were calling `patchTerrain(mat)` (GPU mesh-origin sample) rather than the documented `patchTerrainAtCentroidRaw`, and the helper was orphaned across `src/`. Preview (baked) was correct because `cartograph/bake-buildings.js:571–575` always computed canonical mean-corner. Fixed in this commit: Foundations stamp from mean-of-footprint-corners; walls use `patchTerrainAtCentroidRaw(mat, meanCornerRaw)`; helper now has live call sites. Live + baked + doctrine in agreement.

  `Foundations` + `Building` both now lift by `mean(getElevationRaw(footprint[i]))`, matching `cartograph/bake-buildings.js`'s canonical `centroidY` computation. Previously LafayetteScene Foundations used `getElevationRaw(building.position)` (single-point centroid sample) → diverged from the bake by the heightfield convexity across the footprint, baked extra exposure into every building. Helper `patchTerrainAtCentroidRaw(mat, centroidRaw)` added in `terrainShader.js` for materials whose anchor isn't the mesh origin.
...
- **Foundation + Building frustum culling disabled.** **2026-05-18 addendum (fix actually landed).** Investigation surfaced that this 2026-05-14 fix described below never actually shipped to LS-live code — both `Foundations` and `Building` meshes still defaulted to `frustumCulled={true}`. Symptom persisted as "buildings pop out as the camera moves past them" and was misdiagnosed during the 2026-05-18 session (coordinator searched for opacity/transparency/raycast fade logic before realizing the 2026-05-14 entry described the right fix but the code was missing it). Landed in commit `6fcbd2d` (swept into the Stage/production gate split commit alongside the Force Neon On semantics fix): `frustumCulled={false}` on the `Foundations` merged mesh (`LafayetteScene.jsx:488`) and the per-building wall mesh (`LafayetteScene.jsx:959`). The third 2026-05-14-claimed fix discovered missing this session, alongside the mean-of-corners anchor rule and the `patchTerrainAtCentroidRaw` helper orphan (both landed 2026-05-18 in commit `3e51641`). Pattern documented in `feedback_verify_doctrine_against_code` memory. — Original 2026-05-14 description below: With GPU vertex displacement up to ~50 m, the geometry's cached `boundingSphere` is at the un-displaced Y; three's frustum culler popped buildings out as the camera moved past where the stale sphere sat. `frustumCulled={false}` on both meshes.
...
### Trees — Procedural v1.5 + Procedural fallback v1 + SpeedTree

Migrated 2026-05-18 to [`arborist/BACKLOG.md`](../arborist/BACKLOG.md). Phase tracking for the v1.5 arc (A / D / B-core / B.1.a / C / C.1 / C.1b shipped; E / F / G.1–G.5 / B.1.b / B.1.c / B.2 in flight or deferred), perf phases W + H, roster-recalibration + v1.6 deferred items, the 2026-05-14 procedural-fallback v1 ship notes, and the SpeedTree tier all live there. The per-shot tree-scale slider (Stage-side, camera-connected) moved with them.
...
- [ ] **Author an answer for the big park intersections.** Multi-way
  ...
  - [~] **Phase 2.2 — Curb stroke smoothing.** REVERTED 2026-05-16
    (`c360fc2`, reverted in following commit). Morphological-closing
    approach (dilate-then-erode-by-ε on curb stroke) structurally
    cascades through Clipper precision into adjacent block geometry —
    operator screencap at LS showed black voids in block interiors +
    curb stroke STILL missing on long curves. Not tunable; the
    approach itself reaches past the stroke. Path (b) polyline-offset
