...
## 2026-05-13 — Terrain follow-ups (from bake-terrain landing)

Filed after the terrain clip+binarize commit (`cartograph/bake-terrain.js`, 5 m sampling, .bin pattern matching ground.bin/buildings.bin).

- **Per-instance terrain config**. `M_PER_SAMPLE`, `STENCIL_BUFFER_M`, and the EPQS endpoint are hardcoded constants in `bake-terrain.js`. Extract to `cartograph/data/<scene>/terrain-config.json` so a hillier instance can dial finer sampling without touching kit code. ~30 LOC, isolated. Same pattern as `BBOX` belonging next to `neighborhood_boundary.json`.
- **Elevation exaggeration as an authored channel** (slab-completeness). `V_EXAG = 5` in `src/lib/terrainCommon.js` is a hardwire — per `[Hardwires come out when channels install]` it should become an authored channel. Stage slider → `design.elevationExag` → bake into `scene.elevationExag` → BakedGround / Terrain / building displacement consume from scene.json with paletteOverride-shape carve-out for Stage live-wire. A flatter neighborhood wants more drama; a hillier one wants less. Adds one slot to the SC matrix.
- **Build-break + runtime TDZ fix landed alongside Couplers §6** (`vite.config.js`: `build.target: 'es2022'` for top-level await + `assetsInclude: ['**/*.bin']` restored after SC.7 revert; `src/utils/terrainShader.js`: V_EXAG re-export rerouted from `./elevation` to `../lib/terrainCommon.js` to break the terrainShader↔elevation cycle that produced TDZ at elevation.js:11 under TLA). **Visible terrain output now needs tuning:** with the runtime unblocked, b24fce5's clipped-GeoTIFF pipeline renders for the first time and the displacement is way over the top — terrain "insanely" hilly, lamps lift ~50 ft, ground swallows buildings. Two likely causes (terrain operator's call): (a) V_EXAG=5 too aggressive against the new pipeline's `local-min = 0` normalization (the old EPQS bake's offset square clipped most of LS's real relief; new clip-to-stencil captures the actual gradient); (b) building meshes may not get `patchTerrain*` coverage parity with lamps (`StreetLights.jsx` uses `patchTerrainInstanced`; audit which consumers actually patch). The "elevation exaggeration as an authored channel" item above is the structural fix; a magnitude tweak in `terrainCommon.js:9` is the cheap interim. Comment in `terrainShader.js:9` says "V_EXAG (10)" which conflicts with the actual value `5` — resolve the historical confusion when you're in there.
...
- **SC.6 — Meteorologist clouds**. ⏳ pending Jacob's strip-vs-wire decision. Re-evaluate per principle: if clouds are part of the authored product (Sky & Light's clouds panel writes presets), they belong in the slab — wire `<Atmosphere />` per `meteorologist/README.md`'s direction. Otherwise drop the panel entirely; don't ship authored-but-unconsumed UI. **Parity audit:** `CloudDome.jsx` is shared between Stage and production today (good — no fork).
- **SC.7 — Arch + horizon tuning** ⏳ in flight. Promote `archState` (the last module-scope authoring bridge in the codebase, 19 fields covering arch placement/transform/foot-fade/uplights L+R + horizon disc) to baked cartograph-store channels. **Parity audit (known fork):** `src/stage/StageArch.jsx` (574 LOC, archState-driven) vs `src/components/GatewayArch.jsx` (318 LOC, no archState reach) — two completely independent renderers of the same landmark. SC.7 consolidates per `project_stage_consumer_parity` (same shape as SC.1's StageSky retirement, larger). Brief drafted in current session.
