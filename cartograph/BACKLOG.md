...
| **intersection-everywhere + trim** ⭐ = "the initial thing" (Jacob) | the E3 GENERALIZATION the standard way: intersection at EVERY node + trim-from-edge-collision (generalizes to the plain Ts → **fixes the marks / perpendicular-join artifacts**) + `innerSign`=face-adjacency. Extends the E3.1/.2/.3 machinery; consume-by-identity at prebake. `HANDOFF-intersection-everywhere.md`. | NOW — **dispatch-ready** (FRESH) |
