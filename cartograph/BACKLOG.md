...
| **divided-detection-data-first** ⭐ (the 18th complex) | replace geometric divided-detection with data-first (class gate · `*_link` exclusion · split/rejoin connectivity · drop 60m · geometry→confirmation) + the `makeStreet:1452` class-flattening fix + `innerSign`=face-adjacency. Un-fabricates 18th. `HANDOFF-divided-detection-data-first.md`. | NOW — **dispatch-ready** (FRESH) |
| **intersection-everywhere + trim** = "the initial thing" (Jacob) | the E3 GENERALIZATION the standard way: construct an intersection polygon at EVERY node, trim roads to edge-collision (generalizes to the plain Ts → fixes the marks / the perpendicular-join artifacts). Rides the consume-by-identity pattern, lands in the PREBAKE polygon-ization. | NEXT — Boz drafts after 18th |
