...
| **e3.4-datum-repair** | **FRAME-SIDE** (Jacob's call, E1 round 3): innerSign vote-flip systemic fix + scripted trace-repair of §5 rows + marked curbs (#4/#6/#5/#1/#2) + G9 perim datum; terminus/deg-6 corners auto-complete. **Accept the long tail** (`[[feedback_accept_the_long_tail]]`). | NOW — **dispatch-ready** (FRESH) |
| post-E3.2 marks (Jacob 2026-06-06, mutable file — captured) | **→ E3.4 datums:** #4/#6 Lafayette, #5 S-18th, #1 Missouri, #2 Hickory (curb smooth, wrong width). **→ L.3 loop roles:** #3/#7/#8 Benton, #0 Waverly (inner sidewalk still shows). E3.4/L.3 validate against these; flag any that don't fit those two classes. | mapped (no new plan) |
