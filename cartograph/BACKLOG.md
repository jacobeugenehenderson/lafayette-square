...
| **e3.3-fillet-identities** | `filletRing` reads the junctionMap corner identities → rounds outer-edge legs only at constructed nodes (the §5e "wrong legs" fix, now trivial). Park corners ≤ datum; no regress at 84 cross-nodes. | NOW — **dispatch-ready** (FRESH) |
