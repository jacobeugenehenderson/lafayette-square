# BOZ — the full charter as it stood 2026-09-13, before the re-founding

> **Diary. Retired for CURRENCY, not for TRUTH.** This is `docs/agents/BOZ.md` exactly as it stood at 216 lines on
> 2026-09-13, the day the seat was re-founded. The live charter is `docs/agents/BOZ.md`; it keeps the same section
> numbers (`§0`–`§5`) and the same subjects, so every `BOZ §N` pointer in the corpus still resolves.
>
> **Why the cut.** The seat's *definition* had merged with the seat's *incident record*. Measured that
> day: the `§5` read-in prescribed ~385 KB / ~96k tokens before the first standup (`ROADMAP.md` alone is
> 187 KB), while `§5` itself conceded the canon "does not fit"; `MEMORY.md` carried 182 emphasis markers
> across 73 lines and sat at 100.0% of its budget with 4 bytes free. Every lesson had been written into
> the load path and none had ever left it, so the load path outgrew the context it loads into — which is
> the mechanism by which the seat became, in `§5`'s own words, "a fresh agent with a long context."
>
> **What this file is FOR.** The dated receipts below are the evidence for the live charter's rules. When
> a rule in `docs/agents/BOZ.md` is questioned — *why does Jacob dispatch? why can't Boz write a probe?* — the instance
> that produced it is here. Consult it on that question. Do not read it at start of day.
>
> ⚠️ One rule below is known ROT and was NOT carried forward: `§3`'s *"⛔ No standalone `BRIEF-*.md`"*.
> Contradicted by practice (21 tracked briefs at root on the day of the cut, two written that day) and by
> two other canon docs — `cartograph/BACKLOG.md` ("Root `BRIEF-*.md` are git-tracked campaign briefs")
> and `ROADMAP.md` ("Active dispatch briefs stay tracked at root"). The live `§3` follows those three.
