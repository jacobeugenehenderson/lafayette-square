> ⛔ **SUPERSEDED 2026-09-06 — this is PIPELINE.md v0.2, "the address map".** Replaced by the plain-language progression at `cartograph/PIPELINE.md`, which absorbed `WALL.md` and the six stage-doc openings. Kept for the record. ⚠️ Its code citations were measured 51-of-99 stale, and its repeated citation of `WALL.md §31` points at a section that never existed.
...
**Where the wall is today — CONSUMER side DONE, PRODUCER side open.** *(Corrected 2026-07-31. This paragraph previously read "the curb… is re-stroked live from chains **every frame**." That was **false**, and because this is the spine doc the routing gate points at, it taught readers that a working wall did not exist. `WALL.md §31` carried the accurate version the whole time. ⚠️ `ARCHITECTURE.md §2.1` did **not** — it *opened* with the same false sentence and only corrected it after an em-dash, so it was miscredited here; both are fixed 2026-08-04.)*

- ✅ **The CONSUMER boundary is real, wired, and defended.** **Every non-Survey view — Section/Measure *and* the neutral Design view — renders from the frozen `shape.json`**, not a live build (`BlockGeometryV2Debug.jsx:562`; `sectionFrozen = !surveyActive && !!frozenShape`). `shape.json` carries frozen `iA` on **93 of 101** LS tiles plus per-run curb polylines with their measures. `sectionOpen` has **no chain in lexical scope** — it physically cannot re-derive (`tileGround.js:1812`). The path is race-guarded twice (`72bbc989`, `59e5f109`) under a named rule: *"the frozen artifact OWNS the render… 'Not ready yet' is NOT an invitation for `buildTileGround`."*
- 🔴 **Only SURVEY live-strokes — **not a WALL violation** — Survey is the tool that *edits* the SHAPE, so re-stroking the edited element is the requirement. ⚠️ **But that is not a blessing of the current implementation:** today every edit re-strokes the **WHOLE MAP**, and the standing requirement is fluid asymmetric editing of a **single polygon** — nothing that retraces the whole map at 60fps works (`ORIENTATION` §the-chain, the condensation principle). The whole-map scope is a known perf defect (D6c / the block-local loop), just not a wall one.
- 🔴 **What is genuinely open is the PRODUCER** (`WALL.md §31`, the accurate SSOT): `shape.json` is *minted* by `buildTileGround(liveRibbons, …)` and then snapshotted, so the frozen artifact is **a photograph of a live chain-stroke, not a pure function of the frozen frame**. That is **Check C** (`POLYGON-FIRST §2`), and it is **RED**. The remaining program is D6b/c of `HANDOFF-freeze-the-curb-in-the-first-bake.md`: build the curb **once in prebake from the frozen frame** so the artifact is *derived*, not *traced*.
- **The wall degrades LOUDLY, by design.** If the `shape.json` fetch fails — or the scene has no freeze yet — the consumer still *draws* from a live build, but it sets `shapeFreezeMissing` on the store as a **reason string** (`useCartographStore.js`), at **both** doors in `BlockGeometryV2Debug.jsx` (absent/empty freeze, and fetch/parse failure), cleared on entering Survey; `StatusBar.jsx` renders it as a non-dismissable alarm banner. Drawing-anyway is deliberate — the operator must not be given a blank screen — and the **silence** was the defect. ⏳ The one genuine residual: the banner is **not eye-verified**, because it cannot fire on any scene that has a freeze.

The fix direction for a **producer** defect is always *finish the First Bake (freeze the curb)* — never *patch the live construction deeper* (`PREBAKE.md §4.1/§5`). ⛔ But do **not** cite this as evidence that downstream consumers re-derive: they do not.
...
Builds the planar graph from shared vertices of `streets[].points` (excludes `gradeSeparated`), welds near-coincident endpoints (`ENDPOINT_SNAP`), walks the enclosed faces → tiles. Loop interiors emerge as median faces. ⛔ **CORRECTED 2026-08-12 — "the perimeter face is included so exterior streets get asphalt (G9)" is FALSE** (the same sentence stood in `RIBBONS §3.2`; both fixed). `tileGround.js:984` drops **every** non-positive-area face, the outer face included — its only filter is `signedArea > 1e-3`. **What actually happens:** `derive.js:4632-4648` clips the face-streets to the boundary and **injects the boundary ring as closing edges** (`skelId: '__boundary__'`), so the perimeter faces close into real bounded tiles — **31 of 101 LS tiles carry a `__boundary__` edge, 290 edges in all, every one `side:'right'` as filler, and `__boundary__` is absent from `ribbons.streets`.** → `RIBBONS §1` (the rim is a second unowned class; the 2026-08-12 ruling makes the boundary a closed chain).
