...
- ⛔⛔ **NEW 2026-07-31 — THE WALL CONTAINS A SILENT FALLBACK, and that is a Layer-0 violation in the one place that is supposed to be the guarantee.** The consumer path degrades to a live build **without failing**, by two doors: a failed `shape.json` fetch (`BlockGeometryV2Debug.jsx:589` — `'no frozen shape artifact (Section falls back to live build)'`, a `console.warn`) and a scene with no freeze yet (`:595`). The live build is correctly gated against *racing* an in-flight freeze (`frozenPending`, `72bbc989`/`59e5f109`) — the hole is not the race, it is the **absent** case. On a town where the freeze failed or never ran, the operator is shown a plausible map and **never learns the wall did not hold**; on LS the freeze always exists, so the fallback is invisible in exactly the scene you would use to prove the wall works. Same silent-substitution family as `BRIEF-ls-bleed-excision` (`CLAUDE.md` Layer 0: *a fallback converts a failure into a plausible-looking success*). ✅ **FIXED 2026-07-31 (`ROADMAP A02`) — ⚠️ eye-gate owed.** The store carries **`shapeFreezeMissing`** — a *reason string*, not a boolean, so "this scene was never frozen" and "the freeze FAILED to load" read differently to the operator; set at both doors, cleared on entering Survey (where live-stroking IS the point, so the banner must not cry wolf). `StatusBar` renders it as the same non-dismissable red banner `overlaySaveBlocked` already used; both now share `.carto-status--alarm` instead of inline colour literals (`src/tokens/design.css` is the SSOT; `ROADMAP` C10). ⭐ **Deliberate call: the live build still DRAWS.** A fresh pour with no freeze is a legitimate state and refusing to render would make onboarding town #2 impossible — **the defect was the silence, not the draw.** The view is now *labelled* a live re-derivation rather than masquerading as the frozen shape, which is what Layer 0 actually demands (no *plausible-looking success*). ⚠️ **Not eye-verified:** the banner cannot fire on any of the 7 scenes that have a freeze; only `default` lacks one. Confirm by pointing the tool at an unfrozen scene.
...
- ⭐ **AUTHORING IS NOT A CHAIN — the wall kills chains, it does not kill the operator's overrides** *(ruled 2026-08-04, Jacob)*. The rule above is about **where geometry comes from**, not about which identifiers appear downstream. `sectionOpen` reading `blockCustoms` is **legitimate**: its one use is a dictionary lookup (`blockCustoms?.[run.skelId]?.[run.side]?.[run.segOrd]`, `tileGround.js:1107`) returning **scalars** — treelawn/sidewalk widths, `capFlip` — while every polygon still comes from the frozen artifact. **A wall that blinded the open side to authoring would make every post-wall view render the un-authored to-code default** — the wall erasing the product, which is `CLAUDE.md` Layer 0 q3 exactly. *(`scratch/hadrian-wall-open-proof.mjs` had `blockCustoms` on its FORBIDDEN list and went RED when that read landed; the entry was removed — the list keeps only the chain **graph**: `ribbons`/`streets`/`extractFaces`/`centerline`/`skelId`/`buildTileGround`. The list had already granted this exact exception to `skelId` itself, then banned the table keyed by it.)*
  > ⚠️ **The real hazard the scan can't see:** `blockCustoms` is **keyed by `skelId`**, so the authoring table's key space is chain-shaped. Not a wall violation — but **if chains renumber, authored customs orphan silently.** That is the **T3** gate, not this proof: *"T3 must prove key parity first or authored customs silently orphan"* (`README` §construction model; `ROADMAP` C4).
