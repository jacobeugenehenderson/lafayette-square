> ⛔ **RETIRED 2026-09-06 — folded whole into `cartograph/PIPELINE.md`.** Its doctrine (the two halves of the promise; *freezing wrong data is worse than not freezing*; chains die here) is LIVE and lives in `PIPELINE.md §The Wall`. Kept here for the record only. ⚠️ Every `tileGround.js:NNN` citation below was measured STALE on 2026-09-06 (`node scratch/claims-doc-code-citations.mjs`) — do not follow one.
# The Data Wall
