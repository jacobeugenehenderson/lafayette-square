# ARCHIVED — HANDOFF: Freeze the curb in the First Bake (as written 2026-06-17)

> **Archived 2026-07-31.** Superseded, not deleted. This is the Jun-17 brief **verbatim**, kept for
> its investigation trail — the five ruled-out approaches, the instrumented findings at Lafayette ×
> Mississippi, and the authoring-integration requirements.
>
> ⛔ **Do not work from this document.** Its central framing — that the curb is re-stroked from
> chains *"live, downstream, every frame"* — was **false by 2026-07-14/15** and, left uncorrected,
> caused a session on 2026-07-31 to report that working infrastructure did not exist.
>
> **Live doctrine → `_handoffs/HANDOFF-freeze-the-curb-in-the-first-bake.md` (rewritten 2026-07-31)**
> and `cartograph/WALL.md §31` (the SSOT for what is and is not frozen).

---

**State:** ⭐ **the divided "d" is FIXED (`fd38c70`, 2026-06-16; baked `2a40d50`)** — NOT via this brief's Phase-1a recipe (now OBE: `offsetRingVariable` already does the per-edge offset), but at the **frame/`derive.js`**: ramp each carriageway's OUTER half-width onto the spine's outer line → straight through, median opens inward (Jacob's doctrine). mark#3 96°→9°. See the **§LANDING** section. The **broader curb-freeze** (freeze the curb polygon in prebake; the Phase-2 **block-local edit loop**) **remains OPEN** — and it's the lever for the **Section/authoring perf** (the rest of this brief is that standing architecture work). dispatch-ready, architecture-level. **Supersedes** `HANDOFF-divided-transition-block-tongue.md` (retired to git, 2026-06-14) (that brief's premise — "fix the transition in the frame/skeleton" — was right about the layer but wrong about the artifact; see "What this session ruled out" below). **Read this whole brief before touching code** — it is the distilled result of a long session that bounced off this bug from five wrong angles. The reframe at the end is Jacob's, and it is the actual diagnosis.
