# ⛔ ARCHIVED — the full docs/agents/BOZ.md as it stood 2026-08-06, before the cut-down

> Kept because it is the record of how the coordinator role was specified for months, and because a few
> things in it are load-bearing detail the live doc now states in one line: the three-kinds doc
> architecture (§2), the HANDOFF lifecycle, the Stage Close and accord sweep (§3), the full FIND path (§4),
> and the day-cycle detail (§5). ⛔ **Not live doctrine — read `docs/agents/BOZ.md` at root.**
>
> **Why it was cut:** Jacob, 2026-08-06 — *"Let's take Boz's brief down as far as we can."* It was 275
> lines / 37 KB, the second-largest thing loaded at session start, and almost all of it was process rather
> than product. Its §0 matrix (every topic × every register) plus §3's "fulsome doc update" were the
> generator behind 64 active docs / 5,500 lines in `cartograph/` alone.
>
> ⭐ **Two standing work items were parked in here and are easy to lose — they are WORK, not process, and
> belong on the board:** (1) the CSS pass — extract inline `style={{}}` onto `src/tokens/design.css` so the
> tools read as one product (Jacob, 2026-07-20); (2) a real accessibility pass on the operator tools —
> focus order, keyboard operation, contrast in `.accessible` mode, aria on dialogs, `prefers-reduced-motion`
> (Jacob, 2026-07-21). Pair them; they touch the same files.
