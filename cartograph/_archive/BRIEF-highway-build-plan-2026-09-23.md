> **Archived 2026-09-25** (docs pass, Gantry). This is the H-3 build plan **as dispatched** on 2026-09-23; its steps are built. The live state — what is built, how each part is proven, and what is still open — is `docs/briefs/BRIEF-highway-build.md`. Rulings live in `references/registry.json` by id.
...
- **At grade** (a ramp end shares a vertex with a town street). H butts at the town centreline. The town asphalt draws over it: 2D `renderOrder` `PRI.residential+1` sits under asphalt's 8, and the 3D stack in `bake-ground.js` has `highway` below the ribbons. Check 5 asserts the butt lies inside the town asphalt.
  - **Where H is wider than the street, the street FLARES** (`r-ramp-terminal-flares`): widened by the corner's overhang at the node, tapering over 10× that (`f-caltrans-terminal-widening-taper`, [U] — its truck-volume qualifier, and a mainline lane drop, are not what the passage covers). Frozen per corner at the mint (`protopolygon.flares`, `rampTerminalFlares`); ② draws it in the town street's own measure. ⛔ **Not drawn yet** — check 5 reads the frozen flare and stays red, *declared, not drawn*, until ② does.
...
| 5 | `claims-ramp-ends-classified.mjs` | the ribbons (the points H is swept on) + the stencil | data | Every in-disc degree-1 highway end is one of: at-grade · gore (on a highway interior vertex) · off-rim · a named exception. Each butt corner lies inside the town asphalt on its side, or is received by a frozen flare. | Nudge one endpoint 1 m. |
