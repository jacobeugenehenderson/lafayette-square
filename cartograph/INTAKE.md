...
**Draft auto-save** is implicit: debounced 500 ms `saveNeighborhood` → `neighborhood.json` (name/blurb/`draftRadius`/exclusions), restored on scene open. ⭐ The draft is not the applied disc: the radius's one home is `neighborhood_boundary.json`, written only by Bake (▶ `node checks/claims-radius-has-one-home.mjs`). **Implicit auto-save for cheap edits, explicit Bake for the heavy re-pour.**
...
| **Microsoft Global ML Building Footprints** | ML-derived building footprints, UNIONED with OSM's (`building-union.mjs`: MSBF's geometry where both have the building, OSM's where MSBF missed it) | `cartograph/fetch-msbf.js` (`minedbuildings.z5.web.core.windows.net`) | `raw/msbf.json` + `msbf-index.csv` | CDLA Permissive 2.0 (`intake-rows.mjs#MSBF_CDLA`) |
| **Overture Maps — buildings** (release `2026-01-21.0`) | buildings (incl. MSBF lineage) | `config.py` / merge — ⚠️ no fetch script in the tree | — | **ODbL** *(was "ODbL/CDLA ⚠️ confirm"; confirmed 2026-09-20 — the attribution page gives the buildings theme a "License for theme: ODbL" line)* |
| **Overture Maps — places** | **an external LISTINGS base** — businesses and landmarks for a town whose OSM POIs are thin | `cartograph/fetch-overture-places.js --scene=<id>` (STAC → parquet range reads; no creds, no native deps) | `raw/overture-places.json` | ⛔ **PER RECORD — there is NO theme licence.** CDLA Permissive 2.0 · Apache 2.0 · CC0 1.0 by contributor; derived per town in `cartograph/overture-licence.mjs`. ▶ `node checks/claims-overture-licence-table-is-current.mjs` |
| **The town's assessor** (declared per town in `data/<scene>/sources.json`; LS's is St. Louis's `ASSESSOR/Assessor_Public_Parcels/MapServer/11`) | **official parcels + assessor ROW** | `cartograph/fetch-parcels.mjs`, run by the Extent fetch | `raw/<declared file>` — LS `stl_parcels.json`, huron `oh_parcels.json` | per well |
...
| **USGS 3DEP LiDAR** | **building heights** (max_Z − ground_Z) **+** ground **terrain** DEM | `scripts/_archive/fetch-lidar-heights.py` (heights) · **`fetch-dem.mjs` acquires** (National Map products API → the COG URL list; ⛔ nothing is downloaded) · `bake-terrain.js` range-reads it | heights → `buildings.json` `size[1]`; DEM → `terrain.{json,bin}` | public (USGS) |
...
4. **Cleanup, not gaps:** ⛔ **`elevation.js` is NOT superseded** (this line said it was, and the two have been misread for each other since): **two jobs, two resolutions, both live.** `elevation.js` is the USGS **EPQS point query** on a ~55 m grid, giving every BUILDING its `elev` — it cannot make a dune, there is no dune inside a 55 m sample. `fetch-dem.mjs` + `bake-terrain.js` are the **1 m lidar raster** the ground's relief comes from. ⚠️ `pipeline.js` still imports and runs the first unless `--skip-elevation`, which the Pour passes for every non-default scene — *whether a poured town should carry pipeline elevation is OPEN* (`serve.js`, the elevFlag note). `enrich-*.mjs` / `match-facades.py` invocation is unclear — confirm or archive. No curb/pavement-edge dataset exists locally (osm2streets notes the same) — not pursuable.
