...
**Installation-agnostic (kit):** no St. Louis defaults in the flow — ZIP-driven geocode, OSM everywhere, timezone from center. **hipointe-demun** and **altadena** were both onboarded, poured and baked this way. ⭐ **Altadena is the first truly *end-to-end* intake→pour product** — LS and HPDM predate the intake regime and carry bolt-on protections (source hardwires, the re-center guard, legacy-inclusion-polygon drops); a hood poured fresh through the intake regime needs none of those convolutions. ⚠️ **Frame tighter than feels natural** — an early wide (~5.4 km) test fetch produced large artifacts; a real ~3 km hood is ~10× smaller.
