...
> ⚠️ **PARTLY SUPERSEDED (2026-06-30 doc-canon sweep).** The body below describes the **figure-ground** construction path (`buildBlockGeometryV2`, `chainPavementRing`/`chainStripBand`, `cornersAtIx`, `StreetRibbons`, `derive-toy.js`/`toy-ribbons.json`) as live — that path is **DEAD**. The live construction is the **TILE model** (`src/lib/tileGround.js`; canon **`RIBBONS.md`**, `SURVEY.md §3`); the figure-ground module is slated for deletion at T4 (`DOC-CODE-COHERENCE.md` C3/C4, `BACKLOG.md` tile-T3-authoring). Read this doc for its **fixture-coverage roadmap** (what topology cases the toy must exercise) — *not* for the V2 internals, which are historical. The "use toy as a spike surface" doctrine is `docs/agents/AGENT-VALIDATION-SURFACES.md`.
