...
| `laneProfile` / `length` / `weldedFrom` | *(welded highways)* lanes per member span `[{s0,s1,lanes,from}]` in metres on the raw polyline, of total `length` (rescale after RDP); `lanes:null` = untagged. `weldedFrom` = the member ids. |
...
7. **Unnamed ways — class decides, not name** (`A19`). A street class (`STREET_CLASSES` in `skeleton.js`; a `_link` counts as its parent) → a street with a synthetic name; everything else → `paths[]`. Prints the promoted census by class and any class in neither set.
   **7b. `weldHighwayChains`** *(H-3)* — after the numbering: same-class grade-separated highway chains meeting tail→head at a degree-2 node become one; ⛔ no angle gate (a degree-2 same-direction joint is curvature). Divided carriageways and two-real-name joints are printed, not welded. ▶ `node checks/claims-highway-no-visible-joint.mjs`.
...
> *The 2026-06-04 through-junction straightener (tried, reverted — the doglegs were a datum defect, fixed 2026-06-08): ▶ `_archive/SKELETON-through-junction-straightener-2026-06-04.md`.*
