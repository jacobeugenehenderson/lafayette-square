...
> **Status: v0.3 (2026-07-04) — additive; + the directional-corridor kit pass (§3 step 14).** Consolidates notation that was previously scattered across `PIPELINE.md`, the old `FEATURES.md` divided/corner sections (since migrated to `RIBBONS.md §3.1` / `ARCHITECTURE.md`), `ARCHITECTURE.md §2.1`, `OSM-FORENSICS.md`, `skeleton.js` code comments, and ~70 HANDOFFs. Those sources stay (this is additive, not destructive); this doc is the *index + the artifact reference*. Part of the cartograph quintet's orbit (`FEATURES` / `ARCHITECTURE` / `BACKLOG` / `NOTES` / `RIBBONS`); read alongside `PIPELINE.md` (the execution spine).

> **🔬 Forensic basis (read before trusting the doctrine below).** This design is *evidence-grounded*, not asserted — the proof lives in `OSM-FORENSICS.md` (the Osteopathologist) + `OSM-FORENSICS-EVAL.md` (the build eval). The load-bearing findings, with their homes (⛔ read the counts there, not here — neither forensic doc carries a re-run command, so any digit copied out of them is already unreproducible prose): **(a)** the frame was *strictly poorer than raw OSM* — "OSM or us?" (`OSM-FORENSICS` Part 5); **(b)** interior T-junctions sat in raw OSM at exactly 0.00 m and were deleted by the old junction-*blind* `simplify()` (Part 3.1) — this is *why* §3 step 8 is junction-protected; **(c)** extractability **proven** on a re-runnable prototype: a large vertex reduction while keeping every junction, vs production losing junctions at a shallower reduction (Part 3.5); **(d)** tags are *present-and-discarded* at P1 (`lanes` · `surface` · `maxspeed` · `median width` already computed) — "stop dropping it" is most of the enrichment (Part 2); **(e)** the **Dolman→18th** name-transition is the canonical regression case, and fixing the frame deleted two `derive.js` scars (the West-18th densify + LaSalle magic-coord hacks; `OSM-FORENSICS-EVAL`). The downstream damage these caused — the fuzzy re-projection snap (`IX_SEG_SNAP`), the blunt-and-pray cap — all trace to the junction loss.

---

> ⛔ **THE NARRATIVE LIVES IN `PIPELINE.md` — read it first.** This doc holds **detail only**:
> the mechanism, the schema, the open defect. **What the skeleton is FOR** is now said once, in `PIPELINE.md` step 2. ⭐ §0.1 below is a RULING, not a description — it stays.
...
| **Survey** | operator intent — chain measures/caps/anchors in `clean/overlay.json` (**5.3 KB**, `{version, streets}`, 52 LS streets — **no `loops`, no `blockCustoms`, no corner overrides**); the per-fe SHAPE SSoT in `looks/<id>/design.json` (**~30 KB** — `blockCustoms`, `cornerRadiusScale`, `cornerCornerRadiusOverrides`, `alleyCap`, `surveyDefault`) | `clean/overlay.json` · `looks/<id>/design.json` | 5.3K · 30K |
| **⟦WALL⟧** | the frozen per-tile shape (`_shapeArtifact`) | `public/baked/<id>/shape.json` | **1.03 MB** |
...
| `points` | `[{x,z}]` — the centerline, **junction-protected-RDP-simplified** (eps 1.0 m; loops 0.3 m). ⚠️ **On a street that carries `segments`, these are BEZIER CONTROL POINTS, not a plain polyline** (§3.5 CURVE-FIT). |
| `segments` | ⭐ **the curve primitive** — `bezier`|`line` per span, written by the curve-fit pass. A minority of LS chains carry it (▶ `node -e "console.log(require('./cartograph/data/lafayette-square/clean/skeleton.json').streets.filter(s=>s.segments&&s.segments.length).length)"`). Absent ⇒ `points` is a plain polyline. |
| `throughId` / `through` | canonical through-road identity across name transitions — **on every chain**. Read so a name-transition seam is a THROUGH node, not a corner. |
| `sources` | the OSM way ids welded into this chain (grade is summarized over **all** of them). |
| `seed` | seeded cross-section → default widths. **⭐ Width-sourcing priority (target): custom (`raw/survey.json` — operator-measured, "centerline→sidewalk-centerline"/ROW; most LS streets measured, a minority `default`) → OSM (`lanes`/`width`) → AASHTO/NACTO-by-class (`seedSection`).** ✅ **LANDED — the custom tier IS in the skeleton.** `skeleton.js` carries a titled block `[E1] Custom width base — survey.json → per-side seed enrichment` that reads `raw/survey.json` and stamps per-side seed; the committed artifact has a minority of chains with `seed.widthSource === 'survey'`** and `seed.left/right.source = 'survey'` (e.g. mississippi-avenue: left 3.33 / right 4.74) — ▶ `node -e "const s=require('./cartograph/data/lafayette-square/clean/skeleton.json').streets;console.log(s.filter(x=>x.seed&&x.seed.widthSource==='survey').length,'of',s.length)"`. `derive.js` documents the resulting order in place: *"base (E1): skeleton.js bakes custom (survey.json) → OSM lanes → AASHTO"*. ⛔ **Do not budget this as an open kit move and do not add a second custom-width tier in derive** — the frame would then carry two. |
| `lanes` / `surface` / `maxspeed` | raw OSM tags, carried through *verbatim* on named streets when present. Distinct from `seed`, which *interprets* them into widths. |
| `layer` / `bridge` / `tunnel` | raw OSM grade tags, summarized over `sources`. |
| `gradeSeparated` | operative flag = `entirelyOffGrade ‖ (isLimitedAccess(highway) && !twoWayMotorway)`. Consumers **exclude these from the face graph** (else the 2D crossing bowties `extractFaces`). Surface bridges (Mississippi) stay `false` and keep bounding blocks. (`gradeFields`; live home `ARCHITECTURE.md §7` "Grade separation".) |
| `twoWayMotorway` | `true` only when every source way of a motorway(`_link`) says `oneway=no` (untagged = one-way, as OSM implies). Not limited-access — H-3 ruling (a). Printed every pour. |
| `ref` | voted over `sources`, on **every** chain; `null` if none or tied ⇒ the pour prints *"Interstate status unknown"*. Selects a highway's section — ⛔ never a weld key (`US 6;SR 2`). |
| `phase` | `{ kind: 'divided'|'single', role: 'carriageway-A'|'carriageway-B'|'spine', corridorName, pairKey, chainGap?, startNode?, endNode?, spineAtStart?, spineAtEnd? }`. Carries the divided-road structure downstream so derive/knit don't rediscover it. `chainGap` (renamed from `medianWidth`, D1) = the carriageway chain-to-chain gap — **= the median width exactly**, since the chains sit at the carriageways' inner edges (Alidade 2026-06-05). `startNode`/`endNode` = endpoint node coords, stamped post-normalize (after the canonical-direction flip). **`spineAtStart`/`spineAtEnd`** = the spine street's `id` at that endpoint's divided↔undivided transition (matched by shared endpoint-node + `corridorName`), stamped once as a **frozen frame fact** (commit `61930d7`) — the link the prebake/corner work reads *without* node-matching (`PREBAKE.md §5`, `§5d/§5e`). |
...
| `corridor` | *(when present, KIT)* a shared tag unifying **opposite-prefix, same-base** chains (North/South X, East/West X) that one road carries under two directional names, when they meet **head-to-tail with continuous heading** — so downstream reads them as **one road** (Big Bend N/S, Skinker; Rosebury↔Big Bend on HiPointe). **Distinct** from `phase.corridorName` (the *divided*-road carriageway pair). Stamped by the §3 step 14 directional-corridor pass; guarded against perpendicular same-base crossings (South 18th × West 18th). |

> **Note — unnamed streets** (any street class, §3 step 7) are built *inline*, one chain per OSM way, not via `makeStreet`: synthetic `name` (`<highway> <n>`) + `synthetic:true`, `osmIds` + `sources` + raw `tags`, **a `seed`**, grade fields computed as for any street (`true` only for limited-access or wholly off-grade), and **no `phase`**. ⛔ The `<n>` is positional and the slug is an authoring key — pre-A19 classes are numbered first so their ids hold. ▶ `node checks/claims-authored-skelids-keep-their-ways.mjs`.

### `junctions[]` — the node typology
`{ x, z, degree, kind }`. Built by `buildNodeGraph` from **incidence degree** (endpoint +1, interior +2): every coord with **degree ≠ 2** is emitted — through-bends/name-transitions (degree 2) are skipped, **but dead-ends (degree 1) ARE included.** `kind = kindOf(degree)` = `deadend`(1) / `T`(3) / `cross`(4) / `Y`(5+). Counts per scene: ▶ `node -e "const c={};for(const j of require('./cartograph/data/lafayette-square/clean/skeleton.json').junctions)c[j.kind]=(c[j.kind]||0)+1;console.log(c)"` (⛔ not `OSM-FORENSICS` 1.1's raw-OSM counts). **Shared-vertex only** — grade-separated crossings share no vertex, so junctions carry **zero false grade crossings** (don't hunt grade bugs here). ⚠️ **Distinct from the RDP-protection set** (`junctionKeys`, §3 step 8) = coords owned by **≥2 distinct streets** — a different test that *excludes* dead-ends.

### `nameTransitions[]`
`{ x, z, from, to, fromId, toId }` — where one named street becomes another at a shared node. Marrow the figure-ground path used to drop.

### `paths[]`
Unnamed ways whose **class** is not a street (`PATH_CLASSES` — footways, service drives, …) — `{ id, highway, tags, coords, osmId }`. Render pavement-only, no measure authoring. A class in neither `skeleton.js` set lands here and is printed ⛔ every pour.
...
7. **Unnamed ways — class decides, not name** (`A19`). A street class (`STREET_CLASSES` in `skeleton.js`; a `_link` counts as its parent) → a street with a synthetic name; everything else → `paths[]`. Prints the promoted census by class and any class in neither set.
...
14. **Directional-corridor unify** *(KIT, 2026-07-04)* — after the `nameTransitions` loop, unify **opposite-prefix, same-base** streets that a single road carries under two directional names (North/South X, East/West X). `DIR_PREFIX = /^(North|South|East|West)\s+/i` + an `OPPOSITE` map find the candidate pairs; a pair that meets **head-to-tail with a continuous heading** (endpoint-adjacent, unit-tangent **dot < −0.6**) is stamped a shared **`corridor`** field so downstream reads them as **one road** (Big Bend N/S, Skinker; Rosebury↔Big Bend on HiPointe). **Guarded** against perpendicular same-base pairs (LS's *South 18th* × *West 18th* — a real crossing, not a corridor). Part of **the kit** (installation-agnostic, Jacob confirmed). ✅ **VERIFIED LS byte-identical** — `git diff` clean, **0 corridors** on the park (no opposite-prefix continuous pairs there).
15. **Write** `skeleton.json` via `writeIfChanged(…, {touch:false})` — content-aware, and (unlike chain *outputs*) it leaves mtime **alone** on a no-op run: `skeleton.json` is a needsRebuild **input**, so an mtime bump would force a spurious full downstream rebuild. (`e3ec84a`; `ARCHITECTURE §7`.)
...
**The frame is SPARSE by design.** `skeleton.json streets[].points` is junction-protected-RDP at **eps 1.0 m** (§3 step 8) — polygon-ready, *deliberately* coarse for offset-safety + an early Data Wall (§6, the First Bake). **The skeleton is never densified or smoothed in place** — but ⭐ **since the CURVE-FIT it IS curved at the source, as a PRIMITIVE rather than by densification** (§CURVE-FIT below). ⛔ **So the per-vertex turn angle no longer means what this paragraph used to say it meant** — the curvature lives in the bezier, not between the vertices, and the old *"West 18th↔Dolman ≈ 15.6°/vertex"* regression datum is not comparable to a measurement on today's frame. On a street with no `segments`, the sparse-frame consequence still holds: its concentric offset (the curb) facets, and the ribbon that reads off that curb fragments (`RIBBONS.md §1`, the Derivation Chain).

### ⭐⭐ THE CURVE-FIT — the curve is a PRIMITIVE on the frame, and it is ON by default

*(Added 2026-08-04. This capability shipped and **no doc in the frame cluster described it**, while three others said it was off — so the committed LS frame could not be reproduced from the documented build command, and the artifact disagreed with every doc that mentioned the flag.)*

- **`skeleton.js`: `const CURVE_FIT = process.env.CURVE_FIT !== '0'` — ON by default.** Set `CURVE_FIT=0` to disable. *(It defaulted OFF until `d15d7dd2`, which is what made the committed frame unreproducible; with it on, `skeleton.js` reproduces the committed frame byte-identically — 217 streets / 52 bezier / 976 verts.)*
- **What it does:** the curve-fit pass is the **only** writer of `street.segments`, and it **mutates `s.points`**. A fitted street's `points` are **bezier control points, not a plain polyline** — the single most consequential thing about the current frame, and the reason a consumer written against the §2 table alone will misread it.
- **Artifact:** a minority of LS chains carry `segments` (`bezier`|`line`) — ▶ `node -e "const s=require('./cartograph/data/lafayette-square/clean/skeleton.json').streets;console.log(s.filter(x=>x.segments&&x.segments.length).length,'of',s.length)"`.
- **Consumers already branch on it.** `derive.js` tessellates `street.segments` into a dense polyline — which is why `ribbons.json` has **0** streets with `segments` and 1168 dense verts against the skeleton's 976 — and both overlays read `st.segments ? st.points : smoothChain(...)`.
- ⛔ **`CURVE_FIT` and `STREET_SMOOTH` are DIFFERENT THINGS; do not re-conflate them.** `CURVE_FIT` = the curve **primitive at the frame**, grid-safe, guarded, eye-approved twice (`7c49349`, `4273ce8`). `STREET_SMOOTH` = **retired render-time** smoothing, pinned **0** since 2026-06-14 (no miter clamp ⇒ needle/spur degenerates). The correctness detector's gate is *named* "curve-fit" but *tests* `STREET_SMOOTH`, which is why "the CURVE-FIT gate is RED" read as "curves are broken" for weeks.
- **Consequence for §3.5's follow-ons.** The three open items this section lists (name-transition seam, part-b fillet, persist-past-the-Wall) were written against the render-time smooth and are moot at `STREET_SMOOTH = 0`; the ix-index blocker it names ("home (b) requires recomputing `intersections.ix`") is what the primitive design **avoids** — it is baked at the source as a curve, not as densification.
...
╚════════════════════════════════════════════════════════════════════════════════════════════════════╝
```
⭐ **The SHAPE first appears in the SURVEY tool, LIVE off the skeleton** (ribbons → the live `buildTileGround`; in Survey `sectionFrozen=false`, so `tileGeos` renders). The **Wall** freezes it to `shape.json`; every **non-Survey view is a pure consumer** of that frozen shape — Section/Measure **and** the neutral Design view (`sectionFrozen = !surveyActive && frozenShape` → `sectionGeos`, broadened from `measureActive` 2026-06-16 to kill the idle-redraw perf leak; the curb cannot move there — and the live `buildTileGround` is gated **OFF**: `tileGeos` returns null when `sectionGeos` exists). Only Survey live-strokes.
...
> ✅ **CONFIRMED instance (2026-06-15, Jacob) — the junction-curb bump.** Fixed entirely on the **Survey/SHAPE** side (the canonical `roadId` so a `continuesAs` seam is a through-node, + the through-road width reconciliation — `§5a` curb-side completion, `HANDOFF-curve-primitive-skeleton.md`). The **Section** ADA-fragment / broken-ribbon symptom then **cleared entirely as a downstream consequence — zero shape re-derivation in Section** (the FILL just reads the now-correct frozen curb + the frozen `roadId` identity). *"As predicted, the Section issue is fixed as a result of fixing the Survey"* — the SHAPE-ahead-of-the-Wall / derivation-chain doctrine (`RIBBONS §1`), validated live.
...
**The curve-fit (the smooth curve) is a CONSUME-TIME map, not a frame edit.** `src/lib/smoothCenterline.js` `smoothChain` — interpolating centripetal Catmull-Rom, **corner-protected** (`CORNER_TOL 30°` splits sharp corners out as hard vertices → "smooth bends, keep junctions sharp"), **junction-pinned** (`breakKeys` = `junctionKeysOf`), **arc-length-uniform** (density-robust; no scallop on sparse input — the dead scalloping fear, `HANDOFF-curve-primitive-skeleton.md` Law 3). Applied **where streets are consumed**, and per the `derive.js` freeze doctrine it must **never bake into the frozen artifact** ("an interpolating, junction-pinned geometric map that cannot change topology and must never bake into the artifact").

⭐ **THE ONE KNOB 2026-06-14. ⛔ IT IS PINNED TO 0 — this whole path is DORMANT.** `smoothCenterline.js:150` is `export const STREET_SMOOTH = 0`, reverted 2026-06-14 because `smooth > 0` exposed map-wide fold-spikes — **the offset self-intersects on tight bends.** ⛔ **NOT "a missing miter clamp"** — that prescription is **provably dead** (`ROADMAP A4`, `b378a450`): the clamp tests distance from the apex and in that branch the distance is bounded by construction, so it can never fire; both branches are per-vertex **local** tests and self-intersection is **global**. ⭐ **This knob does not un-pin until that is solved** — the fold-strip inside the offset runs only when `smooth > 0`, so **the cleanup is disabled by the condition it exists to handle.** `smoothChain`'s own header says *"render-time smoothing is currently OFF — smooth=0 — so this whole path is dormant."* ⭐ **The curve you can actually see on the frame is the CURVE-FIT primitive (§3.5), a different mechanism** — do not conflate them. The description below is the design of the knob, not a statement that it is engaged. `smoothCenterline.js` exports **`STREET_SMOOTH`** (the single tension) + **`junctionKeysOf`** (the single pin-set), read by EVERY consumer that renders the centerline curve — same constant + same keys ⇒ ONE smooth curve, **concentric by construction** (the two-source desync is structurally impossible). An **internal constant tuned once on the eye, NOT a UI slider** (a curve's offset-safe sample density has a correct answer, not a preference — the old Smoothing slider was retired 2026-06-04). *(Before: `smooth=0`; the eps-1.0 frame + the `derive.js` Goldilocks ≤5 m densify were judged enough, but the curved through-roads still offset-faceted, 2026-06-14.)* Wiring, by render path (mind the Wall, above):
...
- **Anchor: Center vs Inner-edge.** `center` (default) = ribbon grows **symmetrically**. `inner-edge` = ribbon grows **outward only from the median-facing edge** → the median emerges as the **walked face between the carriageways** (NOT a constructed polygon — `RIBBONS §3.5`, the derived median). **Auto-detected** from corridor pairing (`innerSign ≠ 0` means a paired chain was found); the operator can override. *(Historical note: the affordance was once described as "creates a median polygon"; the median is now derived as a face, never a constructed ring.)*
- **`inner-edge` is the prebake WIDTH-SIDE NORMALIZER, not an authoring mode** — ⚠️ **corrected 2026-08-11; the "authoring mode / chain at carriageway center" reading was the REFUTED center-chain model** (`_archive/DIVIDED-CORRIDOR-PLAN §95` retired it when D1 landed `innerEdgeAssign`). ⛔⛔ **CORRECTED 2026-08-14 — `§61`'s "the chains sit at the carriageways' INNER EDGES" is WRONG, and it is the one clause of this line that is.** `anchor:'inner-edge'` is a **FLAG** (and a *render* instruction — the runtime draws the visible centerline at the inner edge); the chain POINTS are at carriageway **CENTRELINES**. Measured: read as inner-edge-anchored, every LS divided road would carry a 7–20 m median; centreline-anchored gives 1.5–3.5 m. ⭐ **This line's own next sentence implies it** — both sides carrying the same non-zero half-width is only coherent about a centreline. ⭐⭐ **And the sentence after that is the one that matters and was RIGHT ALL ALONG: the "seeds inboard `pavementHW=0`" model is live on ZERO carriageways.** A coordinator re-derived that the hard way on 2026-08-14, built a datum-convergence plan on the opposite belief, and had it struck — **because the topic felt like ribbons and nobody opened `SKELETON.md`** (`RIBBONS §1`, `fb8420ba`). Live rule: `derive.js` `innerEdgeAssign` sets **both** sides to `surveyHW / 2` — the corridor's surveyed half-width, deliberately overriding the authored broadcast widths — and zeroes the inboard **ped** (`treelawn=0, sidewalk=0, terminal:'none'`). **Measured on LS, 2026-08-11: all 38 carriageways carry `anchor:'inner-edge'` + `innerSign:-1`; the ped zeroing is present on all 38; the once-documented "seeds inboard `pavementHW=0`" is present on ZERO of them** (both sides carry the same non-zero half-width). ⛔ The anchor is stamped at prebake, **not** in `overlay.json` — reading the operator's overlay for it reports "unset" on every carriageway and is how this was first mis-measured. **The median emerges by construction, never authored** — and in the live tile model it is an emergent **geometric face** from `tileGround.extractFaces`. **The two-carriageway model is LOCKED:** no pair synthesis, no median couplers, no collapse to a single spine. `setAnchor` is pair-aware (mirrors onto the `pairId` mate).
...
> ⭐ **The NAME-TRANSITION subclass — RESOLVED 2026-06-13 (`c4cb191`), eye-approved.** A *sibling* of the dog-leg with a different cause: at a **name-transition** (a degree-2 node where two differently-named chains meet, `continuesAs`), per-chain RDP (§3 step 8) pinned the seam node and **dropped the dense rounding shoulders**, facetting a gentle, densely-sampled curve (raw ~15°/vertex) into one hard kink (**West 18th↔Dolman 46.5°**). Two corrections to the obvious story: (1) it is **NOT** the §5a off-chord straighten — the road genuinely curves there, so the fix *preserves* the rounding rather than straightening it; (2) **un-pinning the seam did nothing** — the lever is the **tolerance**, not the pin. **Fix = simplify the through-ROAD, not the named chain:** follow `continuesAs` across the seam, RDP the concatenated raw polyline ONCE with a finer `RDP_EPS_TRANSITION = 0.3` so the rounding survives, then split back by name (`skeleton.js` — the `rdpKeep` helper + the transition-aware pass replacing the per-chain RDP loop). 46.5° → 15.6° (the honest raw turn); connectivity intact; retained ⊆ raw (the path can't move). **Gate:** the `name-transition` invariant in `scratch/correctness-detector.mjs` (+ `--simNT` self-test) — RED-until-true, GREEN after. Forensic `SPAR-SKELETON-FORENSIC.md`; brief retired to git (landed). *The principle (Jacob): a name is a label; the road is the line on the ground — simplify the road, attribute the names after.*
>
> ⛔⛔ **The CURB-SIDE completion — BUILT 2026-06-15, and BOTH HALVES ARE CURRENTLY DEFEATED. Re-measured 2026-08-13 (agent Wend, `1cadbceb`) on the same West-18th↔Dolman seam this entry cited as proof.** *(It read "RESOLVED … eye-approved" until then; the eye verdict was real, the mechanism has since been shadowed.)* `c4cb191` smoothed the **centerline** through the seam; the **curb** still bumped, and the two intended cures were: **(1)** a canonical **`roadId`** (a `continuesAs` union, frozen in `derive.js` + the frozen `runMeta`) so `cornerAt`/`isNameTransition` read the seam as a **through-node, not a corner** — ⛔⛔ **REGRESSED: `throughId`, which is a NAME, was later placed AHEAD of `roadId` in `cornerAt`'s key, so the cure is computed, frozen and never consulted at the one seam it exists for. Restoration, not invention — a through-node if EITHER union agrees; LS blast radius 6 stations** (`RIBBONS §3.3` owns it; do not restate it here); **(2)** a **name-aware width reconciliation** (`derive.js`) giving each `roadId` one base `pavementHW` per side, MAX across its chains — ⛔ **it compares by literal side label, and a U-seam joins a *left* run to a *right* run, so the two values that physically meet are never compared** (`side` is point-order-relative). Result at that seam: a **1.4369 m unauthored width step**, bevel-clamped into a 3.34 / 5.59 m curb excursion. Detector: `through-width` + `curb-bump` in `scratch/correctness-detector.mjs` — ⚠️ **it flags this today and the hit sits unjudged in the "grid false-positives" bucket.** Residual kit-gap, separate: a per-fe `blockCustoms` `pavementHW` override still wins over the base, so a deviating through-road override needs correcting in the Survey/SHAPE SSoT (`blockCustoms`).
...
### 5b-bis. Name-aware weld — the along-chain fortification *(folded from `name-logic-skeleton-pass`, 2026-06-22)*
A sibling skeleton-fortification gap, **distinct** from the across-intersection organ above (it's along-chain, not at the node): a substantial share of named streets are fragmented into >1 chain (weldable by name where geometry agrees — ▶ `node -e "const s=require('./cartograph/data/lafayette-square/clean/skeleton.json').streets;const b={};for(const c of s)(b[c.name]=b[c.name]||[]).push(c);const n=Object.keys(b);console.log(n.filter(k=>b[k].length>1).length,'of',n.length)"`), and a handful of within-name dog-legs kink a single named street at its own seam — **St Vincent 88° / Papin 146°** read as **artifacts** (weld-seams to straighten); **Park Place 17° / Papin 25°** are **real gentle bends** (leave). The pass: **name as a prior on top of geometry** — weld only where continuity + heading agree (never over-weld genuinely-separate same-name streets), junction-protected. **Partly addressed** by the `roadId` / `continuesAs` union (`HANDOFF-curve-primitive-skeleton.md`); the same-name *fragment* weld remains **open**, and the brief's sequencing gate — *"AFTER §Wall lands"* — **has been satisfied for weeks.** Deep brief, dispatch-ready with guards + acceptance gate → `cartograph/_archive/handoffs/HANDOFF-name-logic-skeleton-pass-FOLDED-SKELETON-2026-06-22.md`.
...
>
> - ▶ **The typing OSM-FORENSICS calls for is measured and unbuilt: `highway=turning_circle` on the tip node.** `INTAKE §Cul-de-sacs` — 20 in the LS bbox, **13 of 17 placeable at 0.00 m on a degree-1 tip, `carroll-street-1` · `caroline-street-0` · `henrietta-street-0` among them**, i.e. it independently answers three of the census's candidates the way Jacob did. `fetch.js:97` asks for the tag; `:123` drops `el.tags`. That is **`ROADMAP A09`**, and it is this section's only live descendant.
> - ⛔ **What remains TRUE and is the reason to weld at all:** 35 of 113 named streets emit >1 chain, and the *within-name dog-legs* above are real. **Weld for frame quality, never as a dead-end cure** — and never across a severance.
...
> ⭐ **DEEPENED + GENERALIZED — 2026-06-06, Voussoir's E3 spike → `_archive/JUNCTION-CURE-PLAN.md`.** "Wrong legs" was the corner *facet* of a broader root: **the junction silhouette is never CONSTRUCTED** — it's emergent from independent constant-width **butt-capped** chain strokes (`tileGround.js` `extractFaces` L297 walks raw taper runs as face boundaries → `strokeOpen` L919 → blind `filletRing` L88). The trigger is **width discontinuity at the node, not perpendicularity**; **53 instances map-wide** (32 same-corridor, 16 pendant-tip asymmetry, 5 name-continuation), not just the 4 park corners. This is the **one root** shared by the fold-at-join, the false corner, and the perpendicular-join protrusion (T-bulges + Benton-stem). **Band-fold (Section `iW`) and doglegs (frame) ruled OUT.** The cure = **construct the junction at prebake**: continuity-paired curbs (`spineAt*`/`continuesAs`/name) + de-taper windows + **one apron per node** + corner identities → consumed by identity (the E2 pattern). Decomposition = E3.1 map → E3.2 strokes/aprons → E3.3 fillet identities (the "wrong legs" fix below) → E3.4 datum repair. (E2 cured the *needle*, not the join — 18/24 transition nodes still stepped post-E2.) **✅ E3.2 (`b853217`) de-tapered the strokes (0 folds at all 24 nodes); E3.3 (`6a4f611`) fixed the corners BY CONSTRUCTION** — assert the keep-out quadrant at the two identified curb lines' intersection P, then `filletRing` rounds the right legs (a leg-gate alone leaves the false geometry *unrounded*, not absent). Mississippi×Lafayette 1.76→1.39 m (= the datum bound); stub fillets gone. **✅ FULLY RESOLVED 2026-06-07 — intersection-everywhere (`9c275ce`).** The osm2streets grounding (`OSM2STREETS-GROUNDING.md`) confirmed **E3 = the standard trim-back algorithm rediscovered**, and the marks were **TWO drawing bugs**: `filletRing` minting corners from Clipper **quantization micro-edges** (→ `dedupeRing` + `MIN_CORNER_LEG`) + **one width per run** (→ through-node split + per-fe width + trim + blend). Corners now come from **leg-adjacency at every node** (`junctionMap` 86→238); `innerSign`=face-adjacency. The artifact family (spurious corner / spur / internal-corner) is gone; **data was right, only the drawing was wrong** (Jacob). Residuals deferred (Survey paused): the not-totally-fixed nodes (the finish) + the new **curve-and-cut** class (`BACKLOG`).
...
**⭐ Regression provenance (it only broke recently — confirmed 2026-06-04):** figure-ground **already solved this** — `buildBlockGeometryV2.js#cornersAtIx` *skipped the corner record entirely* at the median-wedge park-corner IXs (the `cornersAtIx` skip, shipped **2026-05-16**, NOTES "A.6"; `NOTES:284` names the Mississippi×Lafayette NW/SW failures it fixed). The **tile re-pour** (~2026-06-01, "figure-ground dead-in-place") moved LS onto `tileGround.js` → **`filletRing`/`extractFaces`, which NEVER referenced `cornersAtIx`** and so **never inherited that skip.** That is the recent break. The tile path must build the **right** corner from the clean corridor legs (figure-ground simply omitted it; we want the clean one).

> ⚠️ **LIVE LOCUS = `tileGround.js` `filletRing` (~L90) + `extractFaces` (~L303); NOT `cornersAtIx`.** `cornersAtIx` lives only in the **dead** figure-ground `buildBlockGeometryV2.js` — every doc that frames this fix around `cornersAtIx`, "intersection consolidation in `skeleton.js`", or `osm2streets` prong-4 (`PIPELINE §5 (the Wall)` prong 4 + P7, `OSM-FORENSICS` Part 5, `_archive/RENDER-PATH-CENSUS`, `NOTES:17/49`, 5b below) is a **RED HERRING for the live tile defect** — it describes the dead path or a wrong (skeleton) layer. The centerlines/legs are already clean (§5c); only the tile corner-builder's leg choice is wrong. ⛔ Supersedes 5b's "intersection consolidation in `skeleton.js`" and all asphalt-stroke framings.

> **🔧 Implementation status — the corner-patch was KILLED (2026-06-05, Jacob's call).** The scoped approach (consume `phase.spineAt*` in `tileGround`, reconstruct the true corner from the two straight corridor curb lines, subtract a keep-out quadrant) was a **chain-patch** — it repairs a polygon by reaching back into carriageway/centerline reasoning, the forbidden *"patch chains deeper"* move (§5.1). It is **not shipping.** The false corner dissolves the right way when Survey becomes **polygon-first** (the prebake polygon-ization, §5.1 / `SURVEY.md §6`). The brief `_archive/handoffs/HANDOFF-divided-false-corner-KILLED-2026-06-05.md` and `scratch/divided-false-corner-WIP.patch` are retired (git keeps the validated-but-abandoned detail). *(Lesson: a fix that moves the corner "the right direction" by re-deriving from the centerline is still a chain-patch — the layer is wrong even when the vector is right.)*

### 5f. ⭐⭐ CORRECTION (2026-06-09) — the divided-transition "block tongue" residual is NOT a corner-leg fix; it's the unfrozen CURB

The residual "d" bulge at the divided→undivided transitions (the thin block tongue at Lafayette × Mississippi / Park × S-18th / Park × S-Jefferson) is **not** another instance of §5e's "wrong legs," and it is **not** fixable by tuning the junction construction. A long session (2026-06-09) ruled out, with evidence, every construction-layer lever: the frame de-taper nose (inert — changing it left the baked `iA` byte-identical), moving the frozen face-ring vertex (D2 topology — wrong boundary; the curb is the *stroke*, not the face offset), `cornersAtIx`/§437 (the dead figure-ground path; LS's Survey curb is `buildTileGround`, not `buildBlockGeometryV2`), and "the corner is missing" (it isn't — the E3 window + cross-street corner both fire at these nodes; `Mississippi × laf-6` builds a keep-out at `P=[173.3,215.6]`).

> ⚠️ **PRECISION FIX 2026-07-31 — "every frame" is FALSE as of the 2026-07-14/15 work.** Only **Survey** live-strokes (it edits the SHAPE — not a wall violation, though the current WHOLE-MAP scope of that re-stroke is a known perf defect, D6c); **every non-Survey view reads the frozen `shape.json`** (`PIPELINE §5 (the Wall)`, `PIPELINE.md` §5 (the Wall) ⚠️ *(was cited as `WALL.md §31` — a section that never existed)*). What remains true, and is the whole point below, is the **PRODUCER**: `shape.json` is minted by `buildTileGround` and snapshotted, so it is a photograph of a chain-stroke rather than a function of the frozen frame (**Check C, RED**). Read the diagnosis that way.

**The real diagnosis (Jacob's):** the curb is **produced by stroking the chains** (`buildTileGround`), and that union *can* bow it — building geometry from chains downstream is exactly what the skeleton exists to abolish. A correct curb is, by definition, `chain ⊕ halfWidth` (parallel offset) with corners as offset-intersections — a pure function of the skeleton — so it **belongs in the frozen body, not in a live re-stroke.** D2 froze the *topology*; the *curb* is the unfrozen half. **The fix is to finish the First Bake (freeze the curb), not to keep patching the live construction.** Full diagnosis + seam-to-cut: `PREBAKE.md §4.1`/`§5` + `HANDOFF-freeze-the-curb-in-the-first-bake.md`. Litmus for correctness: every straight-run curb is **parallel to its chain**; measure deviation from that, don't reconstruct corners from nodes.

### 5g. ⭐ §5d, applied to AUTHORING — a width drag operates on the FRONTAGE, not the junction-cut segment (2026-06-09)

§5d says the block-facing edge of a *through* street is **street-simple**: ONE continuous frontage between **REAL corners**. That same boundary now governs **authoring scope**, not just construction. The width handle (amber, `pavementHW`) is dragged per-block; the curb under it is the per-edge parallel offset (`tileGround.js` `offsetRingVariable`, §5f — `chain ⊕ pavementHW`).

**The symptom (Jacob's eye):** dragging a long avenue frontage (the Lafayette Park frontage is canonical) **bends/steps in the middle**, exactly where a side street T's into the avenue *on the far side*. The operator expects the whole frontage to move as one.

**The root — the SAME corner test, two layers out of sync.** The avenue chain is segmented into per-block `fe`s at **every junction node**, including a far-side T (`naturalSegments` cuts at every coord-shared IX). The block-ring walker, by contrast, cuts a frontage `fe` only at **REAL corners** (different-street adjacency, `buildFrontageEdges`) — so one frontage `fe` legitimately **owns MULTIPLE `segOrds`** across the through-nodes (e.g. `lafayette-avenue-3` left = `segOrds [2,3,4,5,6]`). The curb offset reads `pavementHW` **per run = per `segOrd`** (`runMeasure`); a per-block drag wrote only the `fe`'s representative (`min`) `segOrd` (`feCustomKey`). So the far-side runs kept the chain default → two depths meet at the through-node → the offset draws a **real width STEP** (the "bend"). The offset is faithful; the width difference shouldn't exist.

**The rule.** *Author the continuous frontage, not the junction-cut piece.* A side street T'ing in on the far side is a **through-node** — it does **not** break the near frontage. A width drag **fans the dragged `pavementHW` across every `segOrd` the anchor `fe` owns** (`SurveyorOverlay.applyAsphaltDrag`, and the mirror in `revertAsphaltHandle`), so every run reads one depth → one uniform curb, no step.
...
- **⭐ Everything is a best guess, and everything is overridable (revised 2026-07-02).** The SHAPE layer is **machine-poured** — a strong first draft from fetchable sources — and the operator may override any of it. A curated centerline is one of two things: **automation-debt** (the pipeline *could* derive it — improve the skeleton so the override becomes unnecessary) or **idiosyncratic authoring** (a feature no source holds — LS's private "Places", a contested edge — *kept*, first-class, not a bug). Automation's job is to make the guess better, not to drive overrides to zero. The two levers that shrink the *automation-debt* share: **stack every free authoritative source** + a **more sophisticated skeleton** interpretation. → `NEIGHBORHOOD-INPUTS.md §0.0/§1.1`. *(Supersedes the "hand-authored SHAPE is a defect, target 0" invariant.)*
...
- **⭐ Author the FRONTAGE, not the junction-cut segment (§5g).** §5d also scopes *authoring*: a width drag operates on the continuous frontage between REAL corners and **fans across every through-node run** the `fe` owns (a far-side T does not break the near frontage). The `fe`'s `segOrds` (block-ring corner test) define the run; the offset's `cornerAt` is the same test. True corners stay independent.
- **⭐ Directional-corridor unify — the kit (§3 step 14).** Opposite-prefix, same-base chains (North/South X) that a single road carries head-to-tail with continuous heading (tangent dot < −0.6) get a shared **`corridor`** tag → downstream reads them as one road (Big Bend, Skinker). **Guarded** against perpendicular same-base crossings (South 18th × West 18th). Installation-agnostic; LS byte-identical (0 corridors). Distinct from `phase.corridorName` (the divided-carriageway pair).
- **⭐ Width sourcing — custom > OSM > AASHTO, baked into the frame. ✅ BUILT** (`skeleton.js` `[E1]`; 139/217 LS streets carry `seed.widthSource === 'survey'`). The skeleton is **born with the best-available base widths** (operator-measured `survey.json` first, then OSM lanes, then AASHTO seed) so the operator only *tweaks* on top, never authors from scratch — a **kit** capability, any place supplies its own if available. A sanity guard (`plausibleSwDist` / `plausibleRowHalf` + a `rejected[]` log, the "[Benton guard]") rejects implausible measured values rather than baking them.
- **⭐⭐ The divided-corner root (§5e): the corner-builder corners the WRONG LEGS.** A clean corner = two straight curb legs (centerline ⊕ pavementHW) meeting at one rounded corner — what the tools already make everywhere. A divided road adds carriageway *stubs* at the node; the builder rounds a stub instead of the corridor's clean outer-edge leg → the false corner. **Fix = round the two corridor outer-edge legs, treat the divided corridor as one road at the corner.** Not skeleton, not asphalt, not a clamp. *(That cured the original false **corner** — live, `9c275ce`. The residual transition **"d" bulge** is a separate, deeper issue: the **unfrozen curb** re-stroked live from chains — §5f.)*

---

## Cross-references
- `PIPELINE.md` — execution spine (§"The stages, in order", §Wall, P1–P3); this doc is the deep chapter §skeleton points into.
- `RIBBONS.md §3.1` — divided-road inner-edge anchor (the live, authoritative model). *(The old figure-ground specs — corner-Q + park-corner skip, asphalt-rect simplification, formerly FEATURES §437/§439 — are superseded; git history holds them.)*
