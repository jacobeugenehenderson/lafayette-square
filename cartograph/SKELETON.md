...
`{ x, z, degree, kind }`. Built by `buildNodeGraph` from **incidence degree** (endpoint +1, interior +2): every coord with **degree ≠ 2** is emitted — through-bends/name-transitions (degree 2) are skipped, **but dead-ends (degree 1) ARE included.** `kind = kindOf(degree)` = `deadend`(1) / `T`(3) / `cross`(4) / `Y`(5+). **329 typed nodes on LS** *including* dead-ends — measured on the committed artifact: **deadend 100 · T 136 · cross 83 · Y 10** (degrees {1:100, 3:136, 4:83, 5:8, 6:2}). ⚠️ *This row used to read "≈103/141/84 per the forensics" — those are the **raw-OSM** degree counts from `OSM-FORENSICS` Part 1.1, a different population, presented as the post-fix output. `OSM-FORENSICS-EVAL` states the skeleton's numbers correctly. Corrected 2026-08-04; any before/after on junction typing was starting from the wrong baseline.* **Shared-vertex only** — grade-separated crossings share no vertex, so junctions carry **zero false grade crossings** (don't hunt grade bugs here). ⚠️ **Distinct from the RDP-protection set** (`junctionKeys`, §3 step 8) = coords owned by **≥2 distinct streets** — a different test that *excludes* dead-ends.
