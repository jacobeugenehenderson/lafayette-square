...
| `seed` | seeded cross-section → default widths. **⭐ Width-sourcing priority (target): custom (`raw/survey.json` — operator-measured, "centerline→sidewalk-centerline"/ROW; 61/68 LS streets measured) → OSM (`lanes`/`width`) → AASHTO/NACTO-by-class (`seedSection`).** Today only the AASHTO tier lives in the skeleton; the custom `survey.json` tier is applied later (`derive.js:679`). **Kit move: bake the custom source into the skeleton** so the frame is born as-close-to-reality-as-possible and the operator only *tweaks*. Custom street data is imported at onboarding (BACKLOG "Custom street data — kit"). |
...
- **`inner-edge` is an authoring MODE, not a geometry override** (`RIBBONS.md §3.1` — the live divided model): the chain stays at carriageway center; the flag (a) flips `measure.symmetric=false`, (b) seeds inboard `pavementHW=0`, (c) `innerEdgeMeasure` zeroes the inboard ped zone (no sidewalk along the median). **The median emerges by construction, never authored** — and in the live tile model it is an emergent **geometric face** from `tileGround.extractFaces`. **The two-carriageway model is LOCKED:** no pair synthesis, no median couplers, no collapse to a single spine. `setAnchor` is pair-aware (mirrors onto the `pairId` mate).
...
- **⭐ Width sourcing — custom > OSM > AASHTO, baked into the frame.** The skeleton should be **born with the best-available base widths** (operator-measured `survey.json` first, then OSM lanes, then AASHTO seed) so the operator only *tweaks* on top, never authors from scratch. The custom data is imported at onboarding and is a **kit** capability (any place supplies its own if available) — see BACKLOG "Custom street data (kit)." *(Here the data is good — the job is to load it with the right priority and propagate it onto every chain, incl. divided splits.)*
