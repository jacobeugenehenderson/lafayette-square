...
**Define a new neighborhood by eye, then pour it in one click — "the next town pours itself" made real.** The **Extent** tool is the front door: land on a labeled aerial photo and frame the neighborhood you want. A circle fits itself around it — and here's the insight that makes it effortless: **a neighborhood is a disc with a few bites taken out, not a shape you trace.** Everything inside the circle belongs *by default*; the operator only draws around the **exceptions** — loop the commercial strip that isn't really part of the hood, the far side of a big street, the hillside past the last house. Those loops are proper editable curves you come back and refine forever, and you can wave a single building in or out by hand for the ragged edge. Then one button — **Bake** — runs the entire build (re-center the world on the neighborhood, derive the geometry, carve it to the circle-minus-your-loops, bake the 3D slab) and drops the operator straight into the finished, rendered neighborhood. No terminal, no scripts, no perimeter to hand-draw. Drawing what to *leave out* is a fraction of the work of tracing a boundary — and it's exact, not a guess. This is the kit ambition's sharpest test — *a different operator, a different city, almost no hand-work* — turned into a tool you drive by eye. **Altadena, California is the first neighborhood poured end-to-end through this front door** — framed, fetched, excluded, and baked with none of the special scaffolding Lafayette Square and Hi-Pointe needed as the tool grew up around them. *(As-built: `ARCHITECTURE.md §8` "The Extent tool & the Pour" / `INTAKE.md §0.5`; the operator flow: `OPERATIONS.md §Extent`.)*
