...
- **Arborist** (`/arborist.html`) — tree species library and Grove (visual gallery). Operator browses, rates, and curates GLB tree variants. Bakes `public/baked/default.json` (tree placements with assigned variants) consumed by `InstancedTrees`. Tree placements come from `src/data/park_trees.json` (Python ETL, point-in-polygon-filtered against the real park boundary). The arborist is the single source of truth for what trees appear; it runs across all Looks (per-Look only changes the bark/leaves *atlas*, not which trees go where).
  - **Procedural-variant fallback (v1).** Arborist publishes a *generated* species family — `procedural_broadleaf`, `procedural_conifer`, `procedural_ornamental`, `procedural_columnar`, `procedural_weeping` — alongside scanned/vendor GLBs. A pure parametric generator function (resurrected from the pre-`43c4aa3` `ParkTrees` branching algorithm) emits N variants per morphology as a multi-node source GLB; the existing `publish-glb.js` pipeline LODs + manifests them; `bake-look.js` atlas-packs them into the Look like any other species. InstancedTrees substitutes from the same-category roster pool exactly as it does for real species — **the runtime sees no fork**. The generator is built parameter-first now (CLI today, sliders later); the eventual procedural-authoring mode in the Arborist app is a UI surface bound to the same parameters with no algorithm change. SpeedTree GLBs will replace these by raising quality ratings on roster entries — no code change at swap time. The slab carries procedurals exactly the same way it will carry SpeedTrees; v1 ships through this channel.
