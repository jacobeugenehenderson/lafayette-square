...
**Buildings stay live everywhere (Stage + Preview + production, as of 2026-05-26).** `LafayetteScene` reads `_allBuildings` from `src/data/buildings` for per-building interactivity (place state, neon, click handlers — downstream LS-app concerns that don't survive a flat merged-mesh bake). Neon draws from active data, so the live-data path is load-bearing. Stage, Preview, and production all mount the same live `LafayetteScene`. The `bake-buildings.js` merged mesh (`buildings.json`) + its `BakedBuildings` consumer are **orphaned** — Preview used to mount the merged mesh for GPU-perf proof, but the parity pass moved Preview onto the live mount so the profiler measures what production actually ships. **Open decision (L1.3, `HANDOFF-buildings-bake.md`):** revive the merged bake as a real optimization via a *hybrid* (merged geometry + per-building index sidecar) so production can consume the slab without losing per-id identity. Until then, `src/data/buildings` is a runtime dependency of the deployed app — a known slab-completeness gap.
...
| Preview (Slab preview, post-marriage) | `src/preview/PreviewApp.jsx` → `CanvasContents` | `<SceneNeon>` via `<LafayetteScene>` (neon layer toggle; `forceNeonOn` for inspection) | **Live `LafayetteScene`** — same as production (parity pass 2026-05-26) |
| LS production (lafayette-square.com) | `src/components/LafayetteScene.jsx` | `<SceneNeon>` mount, gated by `openPlaces` open-by-hours filter | Live `_allBuildings` + `scene.json.neon.values` read once via `useSceneJson(lookId)` |
...
**Preview is no longer a separate render path (2026-05-26 parity pass).** It now mounts the live `LafayetteScene` — the exact buildings + foundations + neon production renders — so the GPU profiler measures what actually ships. The old `BakedBuildings` merged-mesh consumer is retired-in-place (zero consumers); whether to revive the merged bake as a real optimization is punchlist **L1.3** (`HANDOFF-buildings-bake.md` — hybrid mesh + per-building index). Preview keeps its profiler/phone-frame/layer-toggle bolt-ons over the production render; that's the only divergence.
