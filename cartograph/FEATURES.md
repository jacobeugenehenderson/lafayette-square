...
- **Arborist** (`/arborist.html`) — tree species library, per-species workstage, and **Grove** (visual roster + curation gallery). Operator browses, rates, and curates GLB tree variants. Bakes `public/baked/<look>.json` (placements: `{x, z, url, scale, rotY, species, variantId, ...}`) consumed by `InstancedTrees`. Placements come from `src/data/park_trees.json` (Python ETL, point-in-polygon-filtered against the real park boundary). Placements are universal across Looks; **roster + atlas are per-Look** (`public/looks/<id>/design.json#/trees` + `public/baked/<id>/trees-atlas-*` + per-Look UV-rewritten GLBs under `public/baked/<id>/trees/`).
  ...
  - **Procedural authoring in-Arborist + skeleton-first redo (v1.5, Phases A + D + B-core shipped 2026-05-15).** Phase B core (photo-PBR bark + retint shader infra) joined A+D in this arc; the original Phase B brief's "5 GLSL bark patterns" approach was DROPPED before code landed (visual-bar risk + single-shader-program risk under Bloom). The actual Phase B core lands as: 5 CC0 photo-PBR tileable materials under `public/textures/bark/<materialRef>/`, per-species `bark` spec in the published species manifest, 3 uniform-driven retint slots (`uBarkTintBase`, `uBarkTintJitterRange`, `uBarkRoughnessOverride`) on the shared tree material gated by a per-vertex `aBark` attribute baked at runtime-merge time in `InstancedTrees`. Per-Look palette override: `scene.materialColors[<species>]` wins over the species default `tintBase` at runtime — no rebake needed. Single compiled shader program preserved across every (species, draw call). Workstage Bark panel + Stage debug overlay deferred to **Phase B.1**. Pipeline survives SpeedTree migration unchanged. v1's free-form recursion + single-leaf cards + flat bark is insufficient at any distance. v1.5 rebuilds in 7 phases per `NOTES.md` maxi-brief: in-Arborist dice+adopt UI **(Phase A — landed)**, Space Colonization + tropism for broadleaf/weeping/columnar/ornamental skeletons **(Phase D — landed)**, then monopodial-whorl (conifer skeleton), geometric polish (flanges, root flare, non-linear taper), procedural bark shader (per-species pattern + atlas-budget unlock), leaf cluster cards + 2-stop tint ramp, Sugar Maple proving pass. Same `generateTreeMesh()` params contract throughout; pipeline (publish-glb / bake-look / bake-trees) unmodified. **Phase A surface:** open `/arborist.html` → Procedural button → species dropdown → per-slot 🎲 dice (live preview, no file write) → ✓ adopt (persists seed to `arborist/state/<species>/seedlings.json`, gitignored) → Re-publish species (rebakes through publish-glb + fires per-Look atlas auto-bake). **Phase D surface:** the 4 SCA species' slot cards now expose Envelope (profile dropdown of 5 named revolution curves + width/height/asymmetry/Y-offset sliders) and Tropism XYZ panels; silhouettes diverge correctly (weeping curtains 3m below trunk via envelope `offsetYFrac=-0.6` + Y-tropism=-0.4; columnar narrows to W/H≈0.29; broadleaf rounded-oval; ornamental broad-low). Conifer still routes through the v1 free-growth path until Phase E. See `cartograph/BACKLOG.md` "Trees — Procedural v1.5" + `cartograph/NOTES.md` 2026-05-15 maxi-brief.
