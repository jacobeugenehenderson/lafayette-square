...
**Divided-pair endpoint coalesce in `cornersAtIx` (2026-05-16).** At an IX where two paired carriageway chains (cross-referenced via `chain.pairId === mate.skelId`) both terminate, their endpoint legs point nearly the same way (the mouth opening — typically 2–10° apart). Treated as independent CCW siblings, those legs produce a degenerate ~5° wedge → near-parallel corner Q → no visible plug. Phase A.5 detects this case (symmetric pairId cross-reference + endpoint check + 15° angle threshold) and synthesizes a single "composite" leg from the pair for corner-pair purposes only — composite tangent = unit-averaged member tangents, outer half-width = `max` of members so Q clears both pavements. Data layer (`chain.points`, `chain.pairId`, `ribbons.json`) untouched; coalesce is local to one `cornersAtIx` call. Future readers: V2 handles divided-pair endpoint IXs at the four LS park corners (Lafayette × {Mississippi, Missouri}, Park × {Mississippi, Missouri}) and Toy's Waverly couplet endpoint IXs without leg-pair-naïve degeneracy.
