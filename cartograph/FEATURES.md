...
### Divided-road inner-edge anchor — chain represents the median-facing pavement edge

Divided roads stay as TWO separate centerlines per the locked positive-carriageway model — no pair synthesis, no median couplers, no collapse to a single spine. Each carriageway's chain carries `anchor` (`'center'` or `'inner-edge'`), `innerSign` (±1; which perpendicular side faces the median), and `pairId` (matches its mate). Skeleton emits these via the phase-aware welder; derive passes them through to ribbons.json untransformed.

**`anchor: 'inner-edge'` semantics:** the chain represents the inner (median-facing) edge of the carriageway pavement. Inboard cross-section is zero across the board — no pavement, no curb, no treelawn, no sidewalk. Outboard side carries the full stack. The polygon between paired carriageways' chains IS the emergent median; widening the inboard pavement on one carriageway is impossible by construction (it's pinned at zero), so the median can only be carved from the *outboard* side or by re-aligning the chain itself.

**Single transform site (2026-05-14).** `buildBlockGeometryV2` (`src/lib/buildBlockGeometryV2.js` ~line 1295) applies `innerEdgeMeasure(measure, innerSign)` to every chain whose `anchor === 'inner-edge'` at the top of the function — `street.measure` AND each entry of `street.segmentMeasures`. Both consumers (`bake-ground.js` and Designer's `BlockGeometryV2Debug.jsx`) inherit the transform automatically; no per-call-site audit needed. `MeasureOverlay.jsx:360` calls `innerEdgeMeasure` separately for the operator's drag-handle preview (different code path; same source-of-truth helper).

**Pair-aware authoring.** `useCartographStore.js:setAnchor` mirrors the anchor flip onto the pair mate (lookup by `pairId`). One operator action → both carriageways flip → median appears between them. Width and segment-measure authoring stay independent per carriageway (asymmetric real-world widths like S Jefferson 7.72 / 9.16m must remain authorable per side).
