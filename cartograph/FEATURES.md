...
| LS production (lafayette-square.com) | `src/components/LafayetteScene.jsx` | `<NeonBands>` mount, gated by `openPlaces` open-by-hours filter | Live `_allBuildings` + `scene.json.neon.values` read once via `useSceneJson(lookId)` |
