...
**The pedestrian cross-section is authored, and the default is already correct.** The operator adjusts **sidewalks and tree lawns** per block edge — swap which side the walk sits on, tune the depths, or flip a cul-de-sac's whole cap — and can **return any edge, or the whole scene, to the default at any time.** The default is not blank: the kit reads each street's measured width to decide whether that frontage really has a tree lawn, then lays the strips out at **ADA-compliant depths**. So the map arrives correct and the operator *corrects*, rather than drawing a city from scratch. *(Operator manual: `OPERATIONS.md §Section`; as-built: `SECTION.md §3.1`.)*
