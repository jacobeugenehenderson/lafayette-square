...
> ⚠️ **Status (2026-06-05) — model being reconceived (`DIVIDED-CORRIDOR-PLAN.md`, Alidade).** The inner-edge *authoring workflow* (the Anchor knob + `setAnchor` mirror + runtime ped-zeroing) is slated for **removal**, replaced by a **median constructed at prebake** (the emergent/residual median was fragile). The two-carriageway **data model STAYS** — chains at the inner edge, `chainGap`/`innerSign`/`pairId`/`spineAt*` kept. The sections below describe the workflow being retired; read the plan for the target.
