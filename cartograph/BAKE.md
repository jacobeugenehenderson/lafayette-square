...
2. **The bake is where the second wall stands.** The pipeline has **two walls** (`PIPELINE.md` §5 (the Wall), `[[project_two_bakes_two_walls]]`): the **first** is the Survey freeze (chains die → polygons), the **second** is the bake (the look **store** dies → `scene.json`; the live geometry → `ground.json/bin`). Past the bake, nothing authoring-side exists — the runtime sees only frozen files. **The bake captures a snapshot; it does not author.** Its correctness is therefore exactly the correctness of its inputs (see §4 — the curb-freeze gate).
...
**Dirty-skip.** `runIfDirty(label, inputs, outputs, cmd)` skips a step when every output is newer than every declared input, its own `.js` source included; `?force=1` forces a full rebuild. Writes go through `io.js`'s `writeIfChanged` (`ARCHITECTURE.md §7`), which touches mtime on a byte-identical write so the graph doesn't cascade.
- **The pour's code inputs are its import closure** — `pipeline.js` and everything it imports (`importClosure`), the ① mint and the coastline included. **The registry counts by content:** `map.json.registryRead` records the entries the pour read, and only a change to one of those re-pours (`registryReadChanged`; a missing record is dirty).
- **A re-pour caused by CODE asks first:** the route answers 428 with the changed files, and BakeModal offers *Re-pour and bake* / *Cancel*. An authoring edit re-pours without asking.
- **Every step runs through `runStep`**: state, elapsed time, the last output line and a real fraction from `[progress] <what> i/n`, shown by BakeModal; the list is `bakePlan()`, read from the route itself. **No step is killed on a clock.** *Cancel bake* stops the running step and names it; a failure names its step.
- **Layer visibility is a bake lever**: a hidden layer (`layerVis[id] === false`) skips its sub-bake; re-showing it dirties `design.json`.
▶ `node checks/claims-the-bake-watches-its-code.mjs` · `node checks/claims-bake-progress-shows-the-route.mjs`

The steps, in execution order:

| # | Step | Script | Reads | Writes | Notes |
|---|---|---|---|---|---|
| 1 | **pipeline** | `pipeline.js` and its import closure | raw set + `overlay/skeleton` | `clean/map.json` | every town with OSM; re-derives the frame from the latest edits |
| 2 | **promote-ribbons** | `promote-ribbons.js` | `map.json` | the scene's ribbons (`src/data/ribbons.json` for LS, else `clean/ribbons.json`) | extracts `layers.ribbons` (the First Bake artifact) |
| 3 | **ground** | `bake-ground.js` | `map.json`, `design.json`, `ribbonsGeometry.js` | `ground.json` + `ground.bin` **+ `shape.json`** | calls `buildTileGround(ribbons, { emitArtifact:true })` (the Section FILL construction) → merged, triangulated, indexed ground plane, one group per material/face-use. **Non-street ribbons** (alleys + footway/cycleway/steps/path) are stroked by `buildPathRibbons` (clipped to parcel interiors = block − curb − treelawn − sidewalk; park excluded — its paths render gravel-shaded via `LafayettePark.jsx`) and added as their own `mat:<kind>` groups, `layerVis`-gated like every other group — the **same** geometry the Designer renders live (`BlockGeometryV2Debug`). *(This call was orphaned in the dead V2 bake path until 2026-06-12 — `DOC-CODE-COHERENCE C13`; the slab shipped without alleys/paths.)* **Also emits `shape.json`** — the frozen per-tile shape the Section consumer reads chain-free (the **WALL artifact**, `PIPELINE.md` §5 (the Wall)). Y=0 always (flat; terrain lift is a runtime shader). |
| 4 | **buildings** | `bake-buildings.js` | `map.json` (poured) / **render ledger** (LS), `design.json` | `buildings.json` (v2) + `buildings.bin` | merged building mesh + render-scoped per-building index (`SLAB-CONTRACT.md §6`). Gated on `layerVis.building`. **Render ledger (2026-07-05):** `loadBuildings(scene)` reads a per-scene **render record** at `data/<scene>/buildings.json` for every scene — the `scene==='lafayette-square'` source hardwire (`:62`) is retired (keyed on data, not the name). LS's ledger is the render-field projection of its authored `src/data/buildings.json` (`derive-ls-render-ledger.js`); LS bakes byte-identical. Poured scenes without a ledger fall back to `map.json`→`adaptMapBuildings`. **Membership cull (2026-07-20):** belt-and-suspenders re-application of the pipeline's membership — `((polygon − exclusions) ∪ activate) − hide`, where the **circle is only the fallback** when the scene has no polygon — reads `neighborhood_boundary.json` (circle + exclusions) + `building-overrides.json` (git-tracked) at bake time, so **curation only reaches the slab on a re-bake** (`OPERATIONS §Extent`). ⚠️ A re-bake used to DROP `nb.polygon` (`24323ab2`) — **retired 2026-07-20**; the polygon is now first-class and preserved. See `PIPELINE.md step 3 (prebake)`. *(Was: the boundary-street polygon; before that, drop any footprint point outside the circle.)* |
...
| **`shape.json`** | **`PIPELINE.md` §5 (the Wall)** | **Section** (`sectionOpen`), bake-time only — **not** a runtime artifact |

`shape.json` is the one output the runtime never sees: it is the WALL #1 freeze, consumed only by the Section FILL pass at the next bake. It is the load-bearing proof that `ground.json/bin` was built chain-free (`PIPELINE.md` §5 (the Wall)).
...
- ⚠️ **NOT derived *from the frozen frame*, therefore captured-with-its-defects:** the **curb geometry**. `bake-ground.js` calls `buildTileGround`, which **produces the curb by stroking chains** and then snapshots it (`ROADMAP A03`, Check C RED — `PIPELINE.md` §5 (the Wall) is the SSOT). The bake faithfully freezes whatever that stroke produced. **The slab is exactly as correct as the construction the moment the bake ran.** ⚠️ *Corrected 2026-07-31: not "re-strokes every build" for consumers — every non-Survey view reads the frozen artifact; this is a **producer** statement.* ⛔ And do **not** cite the "divided-transition bulge" as a *measured* symptom — the measurement behind it compared the curb to **un-authored** widths and largely scored the operator's own edits (`POLYGON-FIRST §2` Check A; the follow-on "collapsed rings" census was withdrawn for the same reason). **The producer defect is architectural — the artifact is traced rather than derived — and is NOT currently backed by a trustworthy geometric measurement.** `ROADMAP A05` builds the honest one.
...
- **`PIPELINE.md` §5 (the Wall)** — the freeze doctrine; `shape.json` schema; why frozen-wrong-data is odious.
...
- **`PIPELINE.md step 7 (bake)`** + the `P14`/`P15` addresses — the execution-ladder view.
