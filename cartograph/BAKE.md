...
| 4 | **buildings** | `bake-buildings.js` | `map.json` (poured) / **render ledger** (LS), `design.json` | `buildings.json` (v2) + `buildings.bin` | merged building mesh + render-scoped per-building index (`SLAB-CONTRACT.md §6`). Gated on `layerVis.building`. **Render ledger (2026-07-05):** `loadBuildings(scene)` reads a per-scene **render record** at `data/<scene>/buildings.json` for every scene — the `scene==='lafayette-square'` source hardwire (`:62`) is retired (keyed on data, not the name). LS's ledger is the render-field projection of its authored `src/data/buildings.json` (`derive-ls-render-ledger.js`); LS bakes byte-identical. Poured scenes without a ledger fall back to `map.json`→`adaptMapBuildings`. **Membership cull (2026-07-05):** belt-and-suspenders re-application of the pipeline's **polygon + `activate`/`hide`** membership (keep if centroid-in-`nb.polygon` **or** activated, and **not** hidden) — reads `neighborhood_boundary.json` (polygon) + `building-overrides.json` (git-tracked) at bake time, so **roster curation only reaches the slab on a re-bake** (`OPERATIONS §7`); falls back to the circle if no polygon persisted. See `PIPELINE.md §prebake`. *(Was: drop any building with any footprint point outside the boundary circle.)* |
