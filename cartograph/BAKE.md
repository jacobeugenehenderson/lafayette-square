...
| 4 | **buildings** | `bake-buildings.js` | `map.json`, `design.json` | `buildings.json` (v2) + `buildings.bin` | merged building mesh + render-scoped per-building index (`SLAB-CONTRACT.md §6`). Gated on `layerVis.building`. **Hard boundary cull (non-default scenes, 2026-07-03):** after `loadBuildings(scene)`, if the scene has a `neighborhood_boundary.json`, DROP any building with **any** footprint point outside the boundary circle (strict `every`) so nothing pokes past the boundary. Belt-and-suspenders with the pipeline's centroid clip (which drops on centroid-within-R); this one is a per-*point* test. (KIT step; demo 2038 → 2025.) |
...
| `default.json` | `SLAB-CONTRACT.md §8` | `InstancedTrees.jsx` |
...
- ✅ **Scene-generic bake (2026-07-03/04).** A poured neighborhood bakes a full slab (ground/lightmap/buildings/scene/shape) from its own OSM via the Pour tool; the "scene-specific pipeline not yet implemented" comment was conservative (§1). Non-default scenes with a `neighborhood_boundary.json` get a **hard building boundary cull** in `bake-buildings.js` (drop any building with a footprint point outside the circle) so nothing pokes past the boundary (step 4).
