...
7. **Edit buildings (curate membership — optional).** The footprint overlay shows every building; those whose centroid is inside the **boundary-street polygon** are the neighborhood (solid amber), the rest are excluded (faint ghosts while editing). Click **"Edit buildings"** → click a ghost to **re-activate** a stray that belongs (e.g. a rim building just past a boundary street), click an inside building to **hide** it — a **toggle-ghost** gesture; pan stays live (select is a click, pan a drag). Saved per scene (`building-overrides.json` = `{ activate, hide }`) and applied **at the pour**, so it persists through the 2D Designer *and* the baked slab. (`NEIGHBORHOOD-INPUTS §5.2`.)
