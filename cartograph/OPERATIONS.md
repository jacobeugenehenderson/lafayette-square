...
- **Stage drag semantics** — Browse: LEFT-drag = pan, ⌥+LEFT (or RIGHT) = orbit. Hero/Street: LEFT = rotate, ⌥+LEFT = pan (they're inspection shots). **Hero is the exception: its controls are handed to the flight while playback runs, and free the rest of the time** (above). Designer's "Stage →" lands on whichever Stage shot you were last in (`lastStageShot`), so bouncing between Designer and Stage returns you to the same place each time.
