...
- **The two rows read the live sites, they do not remember what you did (2026-08-29).** *Staging* and *Live* each say **"showing your latest"** or **"behind"**, derived on every mount by fetching what that site actually serves (`scene.json#bakedAt`) and comparing it to yours. ⭐ That is a truer answer than a commit count: it measures **the artifact being served**, so it stays right when a deploy fails silently, and it survives a refresh — the rows used to be written only by a button press, so a hard-refresh made them (and the Visit links) vanish while the deploy was live and fine. **Visit → is always there**, on both rows; every town has its own staging site, so it is a property of the look and not of a push you happened to watch.
