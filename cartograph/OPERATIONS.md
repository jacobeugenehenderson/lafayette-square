...
3. **⚠️ Frame TIGHTER than feels natural.** A wide fetch makes **huge** artifacts and can OOM the pour. The demo was framed at ~5.4 km and produced a 180 MB map; a real ~3 km neighborhood is roughly **10× smaller**. Frame to the actual hood — a ring of margin, not half the metro.
...
6. **Corners resolve automatically** from the skeleton's intersections — where your **consecutive named sides** actually meet (junction clusters, nearest-origin, area-weighted centroid). You don't place corners; you name the streets and the intersections between them define the polygon, its geographic centroid, and a **containing circle** (the neighborhood disc). *(Corners come from the streets on the ground — never from hand-dropped marks.)*
7. **Commit extent.** The heavy geometry step: re-centers `geography.json` to the centroid, reprojects, rebuilds the skeleton, and writes the boundary disc (`neighborhood_boundary.json`, centered at origin) + the neighborhood record (`neighborhood.json` — name, sides, radius, `committed:true`). It also parks the Designer camera on the hood so Stage opens framed. The draft (sides + radius) auto-saves debounced as you work, and restores when you reopen the scene; **Commit** is the deliberate "lock this extent."
8. **Pour → Designer.** Appears once committed. **One click runs the whole build** — derive (pipeline, boundary-clipped) → promote ribbons → ensure a Look bound to the scene → bake the slab (ground / buildings / lamps / scene / AO) → drop you into the Designer on the finished 3D. **The pour is LONG** — the *derive* is the bottleneck (it processes the whole fetched area before clipping to the boundary), so a wide fetch is slow *and* heavy (another reason to frame tight, step 3). It's guarded per-scene so a double-click can't race two pours.
