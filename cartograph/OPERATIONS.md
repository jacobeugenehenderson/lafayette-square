...
- **The bake shows its steps while it runs.** Each step is listed with its state (waiting, running, done, skipped, failed) and elapsed time. The running step shows its last output line and, where it knows its own work count, a real fraction (the ground step: "flatten <layer> 14/40"). A time estimate appears only from this town's own previous run of that step. **Cancel bake** stops it at the running step and says which. No step is killed on a clock; a failure names the step and its reason. ⛔⛔ **AND THE BAKE NOW STOPS ON A MISSING INPUT THAT WOULD FALSIFY THE MAP** — it does not list it as skipped and carry on. Today that is **elevation**: a town with no lidar and no record of anyone looking fails the `terrain` step, and the failure prints the exact command for that town (`node cartograph/fetch-dem.mjs --scene=<id>`). ⭐ **The way past it is to look.** `fetch-dem` records **verified-absent** when it searches the whole ladder and finds nothing — *searched, none exists, do not search again* — and a town carrying that mark bakes flat deliberately, saying so in the bake, with a date and an author. **An absence somebody signed for is not the same as one nobody noticed**, and that distinction is the whole point: a dune town once baked as a plane and the only trace was the word "skipped". ▶ `node checks/claims-bake-progress-shows-the-route.mjs` · `node checks/claims-a-false-map-is-not-a-fallback.mjs`
