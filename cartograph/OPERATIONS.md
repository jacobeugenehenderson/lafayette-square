...
9. **Bake.** The one heavy button: **apply the extent, then pour and bake.** On a first Bake it commits (writes the disc to `neighborhood_boundary.json` and marks the town committed; ⛔ it **never** re-centers the frame or reprojects: `EXTENT-DESIGN §3.3`); on a committed town it re-scopes in place. Then it pours (derive → ribbons), bakes the slab and opens the Designer **in Survey** (▶ `node checks/claims-extent-lands-in-survey.mjs`). The pour is LONG: derive processes the whole fetched area, so frame tight (step 3).
   - ⭐ **What changed** shows beside the button: *"This Bake changed the street layout: tiles 138→132 · …"*. Pressing Bake is the decision to accept that change; it is never applied silently.
   - ⛔ **A failure shows beside the button too, naming the step**, and everything the Bake wrote is restored: boundary, draft, `map.json`, the ribbons and the Designer camera. One consistent state either way (▶ `node checks/claims-a-failed-apply-leaves-one-state.mjs`).
10. **Draft vs applied.** The radius slider is a **draft**, autosaved as `draftRadius`. The map draws the **applied** disc, whose one home is the boundary file. When they differ, the panel says *"draft X m · applied Y m → Bake to apply it"*, and the Bake button is live for that reason (▶ `node checks/claims-radius-has-one-home.mjs` lists every town with an unapplied draft). Membership and loops, too, reach the Designer and slab only after a Bake. **A committed hood cannot be re-centered**: the frame origin is frozen at the fetch center, and a moved hood centre is stored **off-origin on the disc**.
...
- **Trees — the one knob that matters is `lu-policy.json`.** Trees are excluded by two gates, and they are very different sizes (measured on LS, `scratch/tree-lu-exclusion-census.mjs`):
  ...
  - ⭐ **THREE GROUND KINDS, not two** *(ruled by Jacob 2026-09-20)*. `soft` = the census tree may stand. `hard` = the interior is forbidden to a tree — ⚠️ **and `hard` does not mean paved**: `beach`, `dune` and `bare` are hard because no census tree grows on sand or scree. `planted` = green, and the planting is **specified** rather than free — a cornfield grows corn, an orchard grows fruit trees in rows. A `planted` row carries its own species: `{"agricultural": {"ground": "planted", "with": ["zea_mays"], "pattern": "rows"}}`, and the species ids are the same ones the roster and the Arborist already speak.
...
- **Surfaces — which generator paints a land-use class** (`cartograph/surfaces.mjs`: grass, sand). Override per town in `design.json#surfaces`: `{"classes": {"<lu>": "sand" | "grass" | null}, "params": {"<surface>": {"<param>": <value>}}}`. A parameter a surface needs and lacks is named in the console, never defaulted. Preview a remap in the lab first: `lab.html?look=<id>&at=class:<lu>&as=<surface>`.
...
- **The only deliberate "save" is forking a named Look** ("＋ Save as new Look…"). Every other tweak autosaves the active Look — always the open neighborhood's own — to `design.json`; there is no "save the bake." A neighborhood no Look belongs to shows a red **No Look for <town>** bar, and its Designer edits do not save.
- ⭐ **"Kit Default" is the 0-state Look, and it is deliberately EMPTY** *(2026-09-19)*. It is what a new pour seeds from, so it holds **no town's anything** — no widths, no camera, no sky. It binds no scene and **cannot be baked**; the Stage panel opens it showing the kit's own defaults for every channel. ⛔ With a town open it cannot be selected (close the town to edit it), and it cannot be deleted. **A fresh pour therefore opens on kit defaults, not on Lafayette Square's**. *(If you want a town's look as a starting point, fork from that town's Look explicitly.)*
...
- The bake is incremental (dirty-skipped); `?force=1` (or ⌥-click a bake button) forces a full rebuild. **It asks before a re-pour caused by CODE** — the pour's import closure (the ① mint included), or a registry value that town's last pour read — and names the files; an authoring edit re-pours without asking (`BAKE.md §2`).
- **The bake shows its steps while it runs** — state, elapsed time, the running step's last line, a real fraction where the step counts its own work, an estimate only from this town's previous run — and **Cancel bake** stops it at the running step, by name. No step is killed on a clock; a failure names its step. ▶ `node checks/claims-bake-progress-shows-the-route.mjs`
- ⛔ **The bake STOPS on a missing input that would falsify the map** — it does not list it as skipped and carry on. Today that is **elevation**: a town with no lidar and no record of anyone looking fails the `terrain` step and is told the command to run. **The way past it is to LOOK** — a search that finds nothing records **verified-absent**, and a town carrying that mark bakes flat deliberately, with a date. *An absence somebody signed for is not the same as one nobody noticed.* ▶ `node checks/claims-a-false-map-is-not-a-fallback.mjs`
- ⭐ **The revetment says how much of the shore it covers, every bake** — one line, four different kinds of fact, never merged: what it **built**, what the **lidar declined** (*"not at the water"*), what was refused for other reasons, and what lies **outside the drawing** (beyond the disc — our own frame, not a fact about the town). ⛔ **A low number here is usually CORRECT, not a bug:** where the mapped coastline and the lidar water surface disagree — common on a sandy or accreting shore — the kit **trusts the lidar**, because that is what the stone would sit on *(ruled 2026-09-25)*. The line exists so a town that armours an eighth of its coast says so.
- **"The 2D map is out of date"** after a bake means the pour changed the ribbons the page loaded with; **Reload** draws what was poured.
...
- **Ground-contact effect knobs.** *Bake-time* (`bake-ground-ao.js`, edit + re-bake): lamp pool `POOL_RADIUS_M` · `POOL_RING_POS` · `POOL_RING_SHARP` · `POOL_SHADOW_FRAC`; contact shadow `TREE_SHADOW_RADIUS_M` / `_STR`, `LAMP_SHADOW_RADIUS_M` / `_STR`. *Live*: trunk-base blend `uTrunkBlend` / `uTrunkBlendTop` (`treeAtlasMaterial.js`); contact-shadow strength `uShadowStr` (`grassMaterial.js` + `BakedGround` FadeMesh); the pool takes the **lamp colour** (the Surfaces lamp swatch); pool intensity and arch uplight are TOD channels (Lamps / Arch Lighting cards). ⚠️ Pool diameter/blur are bake-time; promoting them to panel controls is unbuilt.
...
- The server runs under `node --watch`: saving `serve.js` **or anything it imports** (incl. `src/instances/*.js`) restarts it and **kills a bake in flight**. ▶ `node scripts/bake-in-flight.mjs` before saving (exit 1 = wait).
