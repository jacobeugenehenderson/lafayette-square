...
- **The bake shows its steps while it runs.** Each step is listed with its state (waiting, running, done, skipped, failed) and elapsed time. The running step shows its last output line and, where it knows its own work count, a real fraction (the ground step: "flatten <layer> 14/40"). A time estimate appears only from this town's own previous run of that step. **Cancel bake** stops it at the running step and says which. No step is killed on a clock; a failure names the step and its reason. ▶ `node checks/claims-bake-progress-shows-the-route.mjs`
