...
- 🔴 **Only SURVEY live-strokes — and that is BY DESIGN**, because Survey is the tool that *edits* the SHAPE. A live re-stroke there is not a leak.
...
- ⛔ **NEW, live, unfixed (2026-07-31) — a FALLBACK inside the wall itself.** If the `shape.json` fetch fails, the consumer **silently falls back to a live build** (`BlockGeometryV2Debug.jsx:589` — *"Section falls back to live build"*); same for a scene with no freeze yet (`:595`). On a town where the freeze failed or never ran, the operator sees a plausible map and **never learns the freeze did not happen** — the exact silent-substitution class excised elsewhere on 2026-07-31 (`CLAUDE.md` Layer 0; `BRIEF-ls-bleed-excision`). The wall's guarantee must fail loudly, not degrade.
