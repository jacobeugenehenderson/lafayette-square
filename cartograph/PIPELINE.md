...
- **Refs:** ⭐ **`SKELETON.md`** (the keystone artifact's home doc — schema, build stages, affordance catalog, the across-intersection gap) · P1 below · §Wall · `[[project_skeleton_is_the_first_bake]]` · `OSM-FORENSICS.md`.
...
> **⭐ ACTIVE 2026-06-04 — the §Wall debt, made visible (the "better bones" program).** The thorn / bulge-bow / degenerate-corner cluster on the live 2D Survey render is **this debt, not separate bugs**. Operator's target (Jacob): *the park block = ~4 corner points; the map scrubbed of thorn-points BEFORE Survey.* **FOUR prongs, separated:**
> 1. **Centerline over-densification** — `skeleton.js simplify` was a timid local filter (perp<0.2m AND turn<2°, useless on curves; Benton 29 pts where ~5 suffice). **✅ FIXED:** aggressive **junction-protected RDP** (⛔ the junction-*blind* simplify deleted 79 interior Ts — `OSM-FORENSICS`), merged `d1d70a2`, baked `13d8195` at **`smooth=0`** (clean RDP frame needs no render-smooth — `smooth=0` replaced the planned arc-length pass). Benton fixed on Jacob's eye; 64/66 curvy chains coarsened. *(`derive.js:1146` curve-densify was wrongly blamed as a re-noising layer — verified inert post-RDP; corrected.)*
> 2. **Band-fold** — at a T-mouth the deep `iW` offset folds where a wide avenue necks the tile (the per-tile G12 guard sleeps; tile globally in-spec); + thin-tile slivers. 🔧 **Option A = LOCAL capacity clamp** ("G12 made local"). In-flight (Bollard). `HANDOFF-band-fold-fix.md`.
> 3. **No-mouth-side dogleg** — at a degree-3 T, the side with **no street** doglegs even though the through-centerline is straight (a through-vertex is corner-treated when it should pass straight). Distinct from the `iW` fold. Folded into Bollard's pass.
...
> **⚠️ STATUS — the centerlines are now polygon-ready; the remaining debt is at the INTERSECTIONS (2026-06-04).** Its job per §Wall is an **extremely simplified, polygon-ready** dataset (a clean block = ~4 corners). **The over-noding is FIXED for centerlines** — the old `simplify` was a timid local filter (perp<0.2m AND turn<2°, useless on curves: Benton 29 pts where ~5 suffice); replaced with aggressive **junction-protected RDP** (Chord, merged `d1d70a2`, baked `13d8195` at `smooth=0`; Benton fixed on Jacob's eye; junctions held). The lines on screen are now clean/straight. ⚠️ **The remaining degeneracy is NOT the centerlines — it's downstream INTERSECTION/corner construction:** the band-fold at T-mouths (`tileGround sectionPass` `iW` fold), the no-mouth-side through-vertex dogleg, and **over-noded complex IXs** (`cornersAtIx` derivation) → the **intersection-consolidation** prong (the highest-leverage, still open). **Don't conflate "geometrically-correct/clean centerlines" with "the corners are clean"** — the lines can be straight while the *junctions* degenerate (Boz conflated these 2026-06-04; corrected). See §Wall note + `HANDOFF-band-fold-fix.md` + `HANDOFF-name-logic-skeleton-pass.md`. (Historical: `derive.js:1146`'s curve-densify was wrongly blamed — verified inert post-RDP, 64/66 curvy chains survive.)
