...
### intake — onboard a place  (`◎ Extent` tool → `ExtentApp.jsx`) — **THE EXCLUDER PEN (2026-07-14)**
- **What / job:** author a place front-to-back with **no JSON hand-editing, no CLI**: ZIP → Locate → **frame** the hood on the global aerial → **Fetch this view** (frame-then-fetch: OSM + buildings + skeleton) → **the auto-fit circle IS the boundary** (buildings IN by default; radius handle adjusts the disc) → **draw bezier exclusion loops** to carve the strays (`BezierPen`) → per-building override for the margin → **Bake** (Commit re-centers `geography.json` to the boundary centroid, writes `neighborhood_boundary.json` = circle + `exclusions`; then one-click Pour into the 3D Designer, §pour).
- **STATUS:** **LANDED** (on trunk `curb-offset-draw`; **not yet in prod** `origin/main`). Membership = **inside-circle − exclusion loops + per-building overrides** — no traced perimeter. Installation-agnostic (kit); hipointe-demun and **altadena** were onboarded, poured and baked this way — **altadena is the first fully end-to-end intake→pour hood** (LS/HPDM predate the regime and carry bolt-on protections). **Never geocode for geometry** — search is the fetch bootstrap only.
