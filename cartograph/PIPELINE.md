...
- **STATUS:** **LANDED** (was "deferred; LS already has its `osm.json`"). The Extent tool IS this stage — the operator-driven realization. Corners come from `skeleton.json junctions[]`, never marks (`feedback_real_path_not_fast_path`). Installation-agnostic (kit); hipointe-demun was onboarded, poured and baked this way. Committed + shipped to staging; the 3D framing bug is **RESOLVED** (content-centering + reproject-all-raw fix — see `HANDOFF-neighborhood-perimeter-builder.md`).
...
- **⭐ The boundary clip = the Data-Wall neuter (`pipeline.js`, after `deriveLayers`, before `map.json`).** A **KIT** step: if the scene has `neighborhood_boundary.json`, prune everything outside the hood. Three cuts: (1) **Drop** features entirely outside the circle + street-fade margin (`keepR = streetFade.outer + 30 ≈ 1441 m`) — top-level layers **and** ribbons faces/tiles/medians/corridors/junctions/nameTransitions (inclusive `touches` test so edge refs survive). (2) **Building MEMBERSHIP (2026-07-05)** — the neighborhood is the area inside the **boundary-street polygon** (`nb.polygon`, re-resolved + persisted on commit in the re-centered frame), *not* the circle: keep a building if its **centroid is in the polygon**, *plus* the roster editor's overrides (`activate` forces an outside building IN, `hide` forces an inside one OUT — `building-overrides.json`, now **git-tracked** so the curation is reproducible source; §5.2 / `NEIGHBORHOOD-INPUTS §5.2`). Applied HERE so `map.json` is the **single filtered source** both the 2D Designer (which reads `map.json`) and the bake inherit. Falls back to centroid-within-radius **R (≈1251 m)** if no polygon was persisted — ⚠️ this fallback is the operator-visible **"whole circle shows in 2D/3D"** symptom for a scene committed *before* the polygon-persist landed (the boundary file carries no `nb.polygon`); the fix is to **re-Commit** (persists the polygon), then re-pour + re-bake (`OPERATIONS §7`). Note `activate` is inert without a polygon — it re-includes an *outside-polygon* building, so with nothing excluded there's nothing to add back. *(Was: centroid-within-R circle clip.)* (3) **Streets/alleys/paths are polyline-CLIPPED, not kept whole** (`clipRun`): trim each polyline to the circle, keep the longest inside run. **This is where an overshooting named arterial gets neutered** — South Big Bend ran **3882 m** across a **2502 m** hood (Forsyth 3677, Wydown 2905); kept whole they skewed the content bounds SE. Post-clip the ribbons street bbox is symmetric (`x[-1439..1434] z[-1441..1441]`, center ≈ origin); map.json 180→52 MB on a wide (~5.4 km) test fetch. ⚠️ The **3D ground mesh is stencil-bounded to ±1461 (a clean disc) regardless** — the clip changes the *content/ribbons* bounds, not the ground mesh. *(Belt-and-suspenders: `bake-buildings.js` re-applies the same polygon + activate/hide membership on the already-filtered `map.json`.)*
...
- **STATUS:** **LANDED + scene-generic.** The old bake-route comment "scene-specific pipeline not yet implemented" was **conservative — it works** (uses the scene's OSM buildings, OSM land-use, STL parcels if in STL). Guarded per-scene (`_seedsInFlight`). **2026-07-05:** the pour now applies **polygon + activate/hide building membership** in `pipeline.js` (the single filtered `map.json` source for 2D Designer + bake; §prebake cut 2, `NEIGHBORHOOD-INPUTS §5.2`) and flushes the roster overrides before building. ⚠️ The pour is **long** (derive processes the whole fetch *before* clipping — the bottleneck; clipping the INPUT for speed is OPEN); a too-wide fetch OOM'd the dev stack once — **frame tighter**. Extent base on `curb-offset-draw`; the roster/membership arc on branch `roster-editor`.
