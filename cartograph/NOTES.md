...
**Phase B (core) — Photo-PBR bark + retint shader infra** — **SHIPPED 2026-05-15** (commit pending)
...
**Phase B.1 — Workstage Bark panel + Stage debug overlay** (deferred from Phase B core)
...
- Stage debug overlay: `renderer.info.programs.length` readout + active bark uniform values for the focused species; toggle-able dev surface so visual gates (criterion 5: WebGLProgram count; 6: per-Look pink-tint reload; 7: per-instance jitter visual) close mechanically.
- UV-scale shader uniform: today's shader samples atlas UV unchanged; tighter per-cylinder tiling needs an additional uniform + per-cylinder UV multiply at the vertex-shader level. The manifest `uvScale` field already exists; B.1 wires it through.
