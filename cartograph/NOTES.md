...
## 2026-05-15 — Procedural trees v1.5: in-Arborist authoring + skeleton-first roadmap

Migrated 2026-05-18 to [`arborist/NOTES.md`](../arborist/NOTES.md). The 2026-05-15 maxi-brief is the load-bearing architecture record for the procedural-trees v1.5 arc per [[feedback_notes_md_holds_architecture]] — read it end-to-end before any tree work. Phase status, the post-Phase-C/C.1/C.1b doctrine deltas, and the cross-phase orchestrator notes (scope-drift / skeleton-first / PS-authored leaves / ProceduralWorkstage single-focus layout) all live there. See also `arborist/{ARCHITECTURE,BACKLOG,FEATURES}.md`.
...
#### Phase 2-arc — Lafayette Park band cusp guard — 2026-05-17

**Status:** SHIPPED. Fixes the opaque-black ribbons at Lafayette Park's 4 corner T-intersections (visible regression Jacob reported post-Phase-2.1). Validity probe (`scratch/park-band-validity.js`) confirmed Outcome (A) — 4 of 8 park frontageBand entries had self-intersecting 34-vert sidewalk rings, all at the four arc spans wrapping the park's corners.

**Root cause.** The park-side of Lafayette Park has authored chain measures (`tl≈3`, `sw≈4`, `cw=0.45`) totalling `~6.6m` of ped-zone depth, which marginally exceeds the authored corner radius (`R≈6.4m`). The Phase 2 regime emitter offsets the arc inward by `totalDepth = cw + tl_avg + sw_avg`; when that exceeds R the offset arc cusps (folds onto itself) and `closeBandRingV2`/the ramp-branch concat produce a self-intersecting ring, which triangulates as opaque artifacts. Symptom: aerial-through-translucent-ribbon failed to read through at park corners.

**Fix (~25 LOC in `src/lib/buildBlockGeometryV2.js`).** Threaded `R` (corner radius) through `arcMeta` entries emitted by `applyRoundCornersToRing`. In `buildFrontageBandsV2`'s arc-span branch, before the asym/sym/ramp branching, added a cusp guard: if `max(cw+tl+sw)` for either leg exceeds `0.9 * arcR`, scale tl/sw proportionally so the deepest ring stays inside (R − 10%·R), with `cw` preserved as the minimum. All three regime branches (asym, ramp, sym-no-ramp) then consume the clamped tl/sw — no per-branch changes needed.

**Bake delta vs Phase 2.1 (`b9cb11c`).** Default look: 384,925 → 384,749 verts (−176, −0.05%), 639,781 → 639,859 tris (+78, +0.01%), 12,008.3 → 12,007.1 KB (−1.2 KB). Determinism preserved (sha256 re-bake match). Imperceptible delta, consistent with bands at park corners getting slightly shrunk near the cusp.

**Surface for coordinator + Jacob:**
1. *Other blocks still SELFINT.* Repo-wide scan: 70 self-intersecting rings remain across all blocks (mix of arc-span and straight-span). The arc-span residuals (e.g., blockKey `854.5,16.5`, `-757.0,369.5`, `696.5,166.0`) are tiny (<5 m²) at blocks with even smaller R; my 0.9× clamp leaves a thin cusp survivor. The straight-span SELFINTs are a separate failure mode (likely emanating from `closeBandRingV2` on chains with sharp turns) and untouched by this fix. Surfaced not fixed — outside the park-bug scope.
2. *D.7a drift on frontageBands is real but not causal here.* `blockKeyFromRing(rounded)` = `2.5,0.0` but `blockKeyFromRing(sharp)` = `3.0,0.0` on Lafayette Park; bands emit at the rounded key, FEs are post-pass-2-backfilled at the sharp key. `sharpFeByKey` lookup at line 1411 misses (because the FE-pass-2-backfill on FEs doesn't mirror onto bands), so band `edgeOrd` and `segOrds` are undefined for the park's bands. Designer rendering doesn't filter by `edgeOrd` so this is silent today, but per `feedback_d7a_blockkey_drift` it's a latent gremlin for any future consumer that does key by `(blockKey, edgeOrd)`. Deferred.
3. *Park corner radii are tight enough that even after clamping, the inner sidewalk converges close to the corner's arc-center.* Visually the band becomes a "thick sidewalk wrapping a tight corner". If Jacob wants the band to feel less pinched at the corner, the right fix is to author wider park corner radii (per the corner-radius authoring kit) rather than to redesign the offset emitter.
...
Migrated 2026-05-18 to [`arborist/NOTES.md`](../arborist/NOTES.md). v1 ship doctrine + pre-ship design memorial preserved verbatim there.
