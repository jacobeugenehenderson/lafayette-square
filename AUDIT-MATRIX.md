...
- **Mobile.** `src/lib/isMobile.js` is the confirmed single *sensing* source (6 importers, dup regex already removed). Open = the *policy* (which layers ship + global quality) → split across slab/`design.json` (inclusion, per-Look) + `INSTANCE.mobileQuality` (global). Cartograph owns authoring/preview-mobile; LS owns the shipped regime.
  > ⛔ **"Policy" undersells it — the RENDER is the open part, not just who authors it** *(measured 2026-09-02)*. Production drops **five** post-FX passes on mobile — `ao` · `pyramid` · `dof` · `bloom` · `aerial`, all `platform: 'desktop'` in `renderPipeline.jsx`'s manifest, filtered at `:259`. **A phone renders `grade` + `smaa` + `grain`.** The `render-pipeline-install` arc was justified on converting that on/off drop into a **resolution bracket** and shipped everything *except* that (`plans/clean-for-handoff.md §W1` banner) — so the binary moved from `PostProcessing.jsx` to the manifest rather than going away. ⭐ And **Preview cannot show it**: inspecting installs the whole desktop pipeline regardless of tier (`cartograph/PREVIEW.md §0.1`), so the phone modes are a desktop render in a phone bezel. ▶ Board home: **`ROADMAP.md H1`**; the design is `_handoffs/HANDOFF-mobile-profile.md`, ⭐ **tracked as of 2026-09-02** (it and `HANDOFF-preview-measurement.md` are the two exceptions to the `_handoffs/` ignore — H1's pointer used to resolve only on Jacob's machine).
