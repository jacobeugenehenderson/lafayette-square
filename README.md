...
| **Dead-ends** (cul-de-sac · stub · spike) | ⭐ **`POLYGON-FIRST.md §2.1` Check 5** (the diagnosis + the enforceable gate) · **`SECTION.md §6.3`** (the class, the scoreboard, the cap-as-end-COUPLER LANDED 2026-07-22) · `_handoffs/HANDOFF-deadend-face-resolution.md §C0` (the live task) · `OSM-FORENSICS §1` | ⭐⭐ **THE MISSING PIECE IS THE CORNER** (2026-07-30). A corner is built where **two different streets meet** (`cornerAt(a,b)` iff `a !== b`). The doubled-back ring visits a spur's mouth vertex **twice** — 1st pass `cross-street → spur` **is** a corner, 2nd pass `spur → spur` **is not** — so **a dead-end spur gets a corner on ONE side of its mouth and NONE on the other**: one leg is bounded corner-to-corner, the other runs through unbounded. That is the whole triad: **edit lands on a SEGMENT not the LEG** (no far corner to fan to) · **partner flips** (same line; sides labelled from walk direction ⇒ inverted 34/34) · **neighbouring corner + cap move** (the mouth *patch* is defining them). ⭐ **Naming ≠ bounding** — the walk-ordinal coupler named the two legs correctly and still could not finish, because there was no second mouth corner to couple to. ⛔ **Test any proposal with: does it CREATE THE SECOND MOUTH CORNER?** A notch (road-with-width subtracted from the disc) does. ⚠️ **NOT "nothing to click"** — 191 of 198 walk slots *are* clickable, only 7 are not (`coupler-fe-coverage.mjs` — ⚠️ every `coupler-*.mjs` runs **only from the `polygon-asks-stamp` worktree**; on trunk they print a silent, lying `0`, see `POLYGON-FIRST §2.1`); the defect is **BOUNDING, not EXISTENCE**. Also settled: render **clean WOVEN** with their authored cap, **the ribbon folds, not the chain**, **the bulb has no halves**; 3 defects fixed at source (cap slope · leg flip · width collapse); legs 56→93/100; 10 asymmetric caps ACCEPTED. ⛔ The **MOUTH** is a **co-claim, not a gap** — never FILL-patch it. ⛔ The pendant-prune was tried **twice** and reverted (`dd4ddb6d`; asphalt is tile-sourced → pruning deletes the road) — this is the **third** pass at the class |
