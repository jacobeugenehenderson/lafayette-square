...
**Pipeline order (Jacob's):** `intake → skeleton → prebake → survey → ⟦WALL⟧ → section → bake → stage`. The open **SHAPE frontier is skeleton + survey**; **intake now has its operator front** — the Extent tool + one-click Pour (2026-07-04, `ExtentApp.jsx`; see the row below + the feature index). Recent sessions have also run on the downstream **look / stage / publish** side (TOD day-cycle, per-shot look cascade, the Preview publish gate). Trunk = `curb-offset-draw` (→ staging via `staging.yml`; the old `cartograph-looks-pass-ab` trunk was retired 2026-07-08).
...
|---|---|---|
| **Extent / intake tool + the Pour** | Intake is an **operator tool, not hand-edited JSON**: `◎ Extent` (`ExtentApp.jsx`) frames a hood on the aerial → **visually selects the boundary streets** (order-independent; ring resolves from shared skeleton junctions) → **auto-radius** containing circle → **Pour** runs pipeline→promote→bake **scene-generically** (Data-Wall boundary-clip neuters overshooting arterials). Concerted redesign **SHIPPED 2026-07-10** (visual selection replaced the 4-cardinal model; empty workspace; commit/rollback; §11 living re-scope). **Divided-road weld** (a divided arterial won't close) fixed in the skeleton via corridor-propagation (2026-07-11). **Finish work open:** legible labels · end-at-corners trim · §11 fold → **`HANDOFF-extent-finish.md`**. | **`cartograph/INTAKE.md`** + **`HANDOFF-extent-finish.md`** (finish work) · archived brief `cartograph/_archive/handoffs/HANDOFF-extent-tool-concerted-logic-2026-07-11.md` |
...
| **Building roster + render ledger** ⭐NEW | Per-building **membership curation** (§5.2's first slice, BUILT 2026-07-05): the neighborhood is the **boundary-street polygon**, *not* the circle (the circle stays the slab disc). Toggle-ghost **"Edit buildings"** in the Extent tool re-activates outside strays / hides inside ones (`{activate,hide}` per scene), applied in `pipeline.js` so **`map.json` (2D Designer) + the bake both inherit it** — one filtered source. Buildings load from a per-scene **render ledger** (`data/<scene>/buildings.json`), retiring the LS `bake-buildings.js:62` source hardwire (LS bakes byte-identical; "LS is a look at the render level"). *(branch `roster-editor`.)* | **`NEIGHBORHOOD-INPUTS.md §5.2` / `§5.1`** (+ OPERATIONS §7 · PIPELINE/BAKE wiring); brief archived → `cartograph/_archive/handoffs/HANDOFF-building-roster-editor-2026-07-05.md` |
...
|---|---|---|
| **intake** (sources → cleaned frame; now an **operator tool** — `ExtentApp` extent + one-click Pour, no longer hand-edited JSON) | **`INTAKE.md`** (data provenance — every source, fetch, transform; authoritative vs. OSM-default) · `HANDOFF-neighborhood-perimeter-builder.md` (the Extent tool + the Pour) · `PIPELINE.md §intake` | `scripts/` (the fetch pipeline) · `OSM-FORENSICS.md` |
...
|---|---|---|
| **Extent tool + the Pour** (intake step-0 — frame → visually select boundary streets → commit → one-click pour) | **`cartograph/INTAKE.md`** · `HANDOFF-neighborhood-perimeter-builder.md` · `NEIGHBORHOOD-INPUTS.md §10/§11` (spec, §11 pending fold to the shipped model) · **`HANDOFF-extent-finish.md`** (open finish work) | `ExtentApp.jsx` (`◎ Extent`) frames the hood on the aerial, **visually selects the boundary streets** (ring resolves from **skeleton junctions**, order-independent); Commit re-centers + `reproject-raw.js` + writes `neighborhood_boundary.json`; the **Pour** bakes **scene-generically** (pipeline→promote→bake), Data-Wall boundary-clip neutering overshooting arterials. Concerted redesign **SHIPPED 2026-07-10** (archived: `cartograph/_archive/handoffs/HANDOFF-extent-tool-concerted-logic-2026-07-11.md`); **divided-road weld** fixed in-skeleton 2026-07-11. |
