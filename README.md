...
|---|---|---|
| **Extent / intake tool + the Pour** | Intake is an **operator tool, not hand-edited JSON**: `◎ Extent` (`ExtentApp.jsx`) — **search a PLACE** (the searched place sizes the fetch envelope, +1000 m and squared, because the disc is concentric and a circle can't fit a rectangle narrower than its diameter) → **Fetch** (the one irreversible step; the envelope is drawn on the aerial and the disc-vs-data containment is checked) → **author the BOUNDARY** (gazetteer ring · click boundary streets · draw) → exclusions + per-building overrides as *correction* → **Bake**. ⭐ **Membership = `(polygon ∪ activate) − (exclusions ∪ hide)`** — the disc RENDERS, the polygon DECIDES. **This supersedes the 2026-07-14 "excluder pen, settled, do not re-open" entry**, which was wrong and cost two agent passes (`INTAKE §0.5`, retracted 2026-07-20). Fetch builds a street INDEX, not the skeleton — *the Skeleton is the First Bake* and belongs to Bake. | **`cartograph/INTAKE.md §0.5`** · `NEIGHBORHOOD-INPUTS §5.2` · `PIPELINE §intake/§prebake` |
...
|---|---|---|
| **intake** (sources → cleaned frame; now an **operator tool** — `ExtentApp` excluder pen + one-click Bake, no longer hand-edited JSON) | **`INTAKE.md §0.5`** (the Extent tool + the Pour; data provenance — every source, fetch, transform; authoritative vs. OSM-default) · `PIPELINE.md §intake` | `scripts/` (the fetch pipeline) · `OSM-FORENSICS.md` |
