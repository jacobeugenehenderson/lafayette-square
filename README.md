...
|---|---|---|
| **Extent / intake tool + the Pour** ⭐NEW | Intake is now an **operator tool, not hand-edited JSON**: `◎ Extent` (`ExtentApp.jsx`) frames a hood on the aerial → names 4+ boundary streets → **corners resolve from skeleton junctions** (not marks) → **Commit** re-centers/reprojects + writes the boundary → **one-click Pour** runs pipeline→promote→bake **scene-generically**. The boundary-clip is the Data-Wall neuter of overshooting arterials. ⚠️ **UNDER CONCERTED-LOGIC REDESIGN (2026-07-10)** — the built **name-derive / 4-cardinal** model is being replaced (legible perfect-fidelity fetch → **visual boundary-segment selection** → **auto-radius**, slider=margin); the old "browse framing" item folds into this. | **`cartograph/INTAKE.md`** + **`HANDOFF-extent-tool-concerted-logic.md`** (the redesign) |
...
|---|---|---|
| **Extent tool + the Pour** (intake step-0 — frame → name streets → commit → one-click pour) | **`cartograph/INTAKE.md`** · `HANDOFF-neighborhood-perimeter-builder.md` · `NEIGHBORHOOD-INPUTS.md §10/§11` (the spec) | `ExtentApp.jsx` (`◎ Extent`) frames the hood on the aerial, names 4+ boundary streets, resolves corners from **skeleton junctions**; Commit re-centers + `reproject-raw.js` + writes `neighborhood_boundary.json`; the **Pour** bakes a scene **scene-generically** (pipeline→promote→bake), Data-Wall boundary-clip neutering overshooting arterials. ⚠️ **UNDER CONCERTED-LOGIC REDESIGN 2026-07-10** — model replaced (legible fetch → visual segment-selection → auto-radius); see `HANDOFF-extent-tool-concerted-logic.md`. |
