...
|---|---|---|
| **Extent tool + the Pour** (intake step-0 — frame → fetch → the circle is the boundary → draw exclusion loops → one-click Bake) | **`cartograph/INTAKE.md §0.5`** · `NEIGHBORHOOD-INPUTS.md §5.2/§11` · `FEATURES.md` / `OPERATIONS.md §Extent` | `ExtentApp.jsx` + `BezierPen.jsx` (`◎ Extent`): the auto-fit **circle IS the boundary** (buildings IN by default), the operator draws **bezier exclusion loops** to carve strays (membership = circle − exclusions + per-building overrides); **Bake** re-centers + `reproject-raw.js` + writes `neighborhood_boundary.json` (circle + exclusions), then pours scene-generically (pipeline→promote→bake), Data-Wall clip neutering overshooting arterials. **The excluder pen (2026-07-14)** superseded every trace-the-perimeter model; a **re-center guard** protects committed hoods. **Divided-road weld** fixed in-skeleton 2026-07-11. |
