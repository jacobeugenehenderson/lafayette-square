...
|---|---|---|
| **Extent / intake tool + the Pour** | Intake is an **operator tool, not hand-edited JSON**: `◎ Extent` (`ExtentApp.jsx` + `BezierPen.jsx`) frames a hood on the aerial → the **auto-fit circle IS the boundary** (buildings IN by default; radius handle) → the operator draws **bezier exclusion loops** to carve the strays + per-building override → **Bake** (Commit re-centers + reprojects; Pour runs pipeline→promote→bake **scene-generically**, membership = circle − exclusions). **Settled model 2026-07-14 (the excluder pen)** — supersedes every trace-the-perimeter model (geocode ring · snap-route · name-boundary-streets · inclusion polygon; all dead, briefs archived). A **re-center guard** protects committed hoods (LS/HPDM). **Committed on trunk, not yet in prod.** | **`cartograph/INTAKE.md §0.5`** (flow + why-excluder) · `FEATURES.md` / `OPERATIONS.md §Extent` · archived briefs in `cartograph/_archive/handoffs/` |
