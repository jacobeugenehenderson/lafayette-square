...
> ✅ **LANDED `c4cb191` (2026-06-13), eye-approved.** Node B **46.5° → 15.6°** (its honest raw turn). ⚠️ **CORRECTION to the diagnosis below:** Spar's proposed fix (un-pin the joint, RDP across) was **necessary framing but NOT the lever** — un-pinning alone left node B at 46.5° (verified empirically). The kink was never caused by the *pinning*; it was `RDP_EPS=1.0` collapsing a **densely-sampled** gentle curve (raw ~15°/vertex every 2–3 m, each <1 m off-chord). **The lever is the TOLERANCE.** The landed fix simplifies the through-ROAD as the unit (Jacob's principle: simplify the road, not the named chain) AND uses a finer `RDP_EPS_TRANSITION=0.3` so the rounding survives. See `SKELETON.md §5a` (the landed fact; the `through-road-simplify` brief was retired to git).
