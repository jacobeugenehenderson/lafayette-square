...
**Near-tier leaf technique (decided 2026-06-25 — reuses the bark toolbox). ⛔ PART BUILT, AND THE SPLIT MATTERS:** two layers, exactly mirroring bark — a **posterized substrate** (low-freq tone → the front/back/season recolor, `extract-bark-posterized.mjs` analog) + a **high-pass detail overlay** (high-freq venation/edge, Overlay-blend, gated ON only at the near tier — the same `extract-bark-detail.mjs` mechanism + the `step(0.5, uBarkShaderTier)` gate the bark already uses).

- ✅ **The backlit flash is LIVE** — `uLeafTransmission` (`treeAtlasMaterial.js`), a wrap/back-light term over every directional light (the moon included), tinted by the leaf's own colour. ⛔ **Not** subsurface scattering, by design.
- ✅ **Front/back two-tone is LIVE as of 2026-08-28** — `gl_FrontFacing`, driven by the `leaf.face` rubric axis, dossier-poured and operator-overridable. ⚠️ **It is a two-COLOUR select, not yet a two-PALETTE swap:** the front colour is the white-point the back is recast against, and the front face renders identity. Recolouring fronts would restyle 10 species nobody asked about. `arborist/ARCHITECTURE.md §leaf.face`.
- ⛔ **UNBUILT: the posterized leaf substrate and the leaf detail overlay.** `extract-bark-posterized.mjs` / `extract-bark-detail.mjs` are generic image ops behind three bark-shaped constants (their directory, `color.jpg`, the `perBarkRef` defaults key), so the extractor generalizes cheaply — but the shader's posterize + Overlay machinery sits behind the `vBark` gate and needs a mirrored leaf block with its own tile uniforms, plus its own manifest slots. **Until that lands, `leaf.season` has no home either** (both want the same substrate).

**Remaining eye-gate (operator, Street view):** how stylized the posterized tones read up close, and how strong the flash. Settled at the Salon Street view, not on paper.
...
- [x] **Botanical height — non-dossier fallback ships** — RESOLVED (`bac11a43`): `publish-glb#normalizeScale` lookup is now **dossier `required["chassis.size"].target` → `arborist/mature-heights.json` → category default**. The stopgap map gives the non-dossier roster species real mature heights (`oak_bur` 22 · `oak_white` 25 · `blackgum` 16 · `linden_american` 21 m) so they no longer read stunted at the 12m default. Republished-all → Grove shows the correct size ladder (oaks ~22–25m, birch 14m), no lone giant. **Reminder:** a botanical-height change still requires `generate-salon` (all) / a Grove regenerate to take — only republished species pick up the new scale.
- [ ] **Full dossiers for the remaining roster species** (`oak_bur`, `oak_white`, `blackgum`, `linden_american`, …) — Jacob endorsed authoring these to **professional-botanist depth** (the annotation is first-class kit IP, as valuable as the assets — [[project_dossier_annotation_is_first_class_ip]]). `mature-heights.json` is an explicit placeholder giving only `chassis.size`; a full dossier supersedes the map per species and feeds the whole rubric-forward matcher, not just height.
</content>
