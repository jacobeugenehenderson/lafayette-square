...
**Status: ✅ LANDED (consume-time) 2026-06-14 — the ONE KNOB.** The curve-fit shipped as a single shared smoothing knob: `smoothCenterline.js` exports **`STREET_SMOOTH`** (the tension) + **`junctionKeysOf`** (the pin-set), and BOTH render consumers read them — the curb (`buildTileGround` `opts.smooth`) and the navy editable centerline (`MeasureOverlay`). Same constant + same keys ⇒ one smooth curve, concentric by construction (true SSoT; the two-source desync is structurally impossible). It honors the two laws below (curve-FIT on the FRAME, applied at consume so the sparse frame + `intersections.ix` are untouched — ix-safe). **Open:** (1) tune `STREET_SMOOTH` on Jacob's eye; (2) the corner-aware fillet ("part b" — `vertR=0` on within-run curve vertices so the curb doesn't take a fillet at every densified point); (3) bake at the source (home (b), `derive.js`) once the constant locks. **Map: `SKELETON.md §3.5`** (frame→render flow + the two homes); doctrine `RIBBONS.md §1`. *(History below: 2026-06-13/14 burned a long arc proving what does NOT work and distilling the two laws; the through-road fix `c4cb191` is the live frame state; the leg-elimination is dormant/abandoned.)*
