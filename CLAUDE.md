...
4. **Name the layer before you fix.** SHAPE (the frozen polygon / curb — Survey · skeleton · prebake) vs FILL (the inward ped strokes — Section)? Ask **"is this chains again?"** (`PIPELINE §5 (the Wall)`). A wrong *silhouette* is upstream; how the *ribbon bends* is Section. Patching the wrong layer is the recurring waste — confirm the layer, in the canon, before editing.
   > ### ⛔⛔ AND HERE IS **HOW** THIS GATE FAILS TO FIRE — the trigger, named 2026-08-11 (Jacob: *"likely responsible for a LOT of miscommunication over this month"*)
   > **You resolve a measured COORDINATE to a street name by asking the STREET GRAPH** — *"which chain is nearest this point?"* — and the chain-world diagnosis arrives attached to the name (a T-junction, `cornerAt`, a through-node). ⭐ **It never feels like switching layers; it feels like LOCATING the defect**, which is exactly why the gate above stays silent.
   > - ⛔ **That query IS proximity recovery of a chain label — `A15`'s explicitly forbidden third recovery** (nearest-fe match and the walk-ordinal coupler both died of it). Done in a throwaway probe rather than in `src/`, so review never catches it.
   > - ⭐⭐ **THE POLYGON IS NAMED TOO — ASK IT.** `shape.json`'s `tile.runs` carries `skelId · side · segOrd` per arc and `iaEdge` binds every inset vertex to its owning ring edge. **The same street names are already there**, as attributes of arcs rather than nodes of a graph.
   > - ▶ **The move: coordinate → the tile whose RING contains it** (by geometry — ⛔ never by tile index; the live pass and the frozen artifact number tiles differently) **→ which ARC is unpainted and who owns it.** Name a street last, as a label for the human.
   > - ⭐ **Jacob naming streets is CORRECT** — authoring is street-keyed **because that is the product**. The error is converting his street-name *report* into a chain-world *diagnosis*. `[[feedback_a_street_name_arrives_with_a_chain_diagnosis]]`
