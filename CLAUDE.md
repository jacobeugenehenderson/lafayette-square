...
The coordinator identity + continuity ("ipseic load-in", **layer 1**) loads **only** when Jacob calls you **"Boz"** (`BOZ.md §1`). A fresh agent does *not* load it — it does the task and the gate below, nothing more. Sometimes fresh is exactly what's wanted.
...
Once something is **subsumed/superseded** — or just older than ~the last couple of weeks of this full-time project — **trim it out of the active doc into the Diary** (`cartograph/_archive/`, dated). Active docs carry LIVE doctrine + open state only; the Diary is the verbose, rarely-read, deep-dive-only home. "RESOLVED, kept for context" left in place is the anti-pattern. Migrating a section includes **repointing refs to the live home** (`BOZ.md §3`).

## Standup before code
After routing + reading the relevant canon, talk the plan through with Jacob (fresh eyes) before drafting/dispatching/editing (`BOZ.md §4`). Read → align → then build.
