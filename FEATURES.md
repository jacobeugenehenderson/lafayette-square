...
**Corner-pad ↔ face-clip parity (2026-05-06).** Corner-pad geometry is built in two places — `StreetRibbons.jsx:buildSidewalkPads` (rendered concrete) and `ribbonsGeometry.js:buildCornerPadClips` (face-clip silhouette so parcel grass carves to match). They MUST stay in sync; comments in both files cross-reference. `buildRibbonGeometry(ribbons, ...)` now consumes `ribbons.intersections` and threads it into `buildRibbonUnion`, which adds the per-corner annular sectors to the silhouette before subtracting from face polygons. Without this, parcel faces show their natural sharp L corner peeking through the corner pad. See `HANDOFF-corner-plugs.md` for full state and the open SW back-leg-flip bug.
