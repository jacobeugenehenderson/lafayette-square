...
1. **`src/instances/<look>.js`** — self-contained identity: `lookId`, `geography` (lat/lon/timezone/bbox — timezone drives weather/sun/TOD), `locale` (`{language, units, clock}` — the i18n/localisation-service track), `name`, `branding`, `legal` (`documents`: the legal-document id it runs under, from `src/instances/copy/index.jsx#LEGAL_DOCUMENTS` — omit it and the /privacy and /terms pages say "not declared"; the About text is `src/instances/copy/<map>.jsx`, likewise never inherited), `profile` (Layer-0: name/population/founded/tagline/about/landmarkName), and `modules` (see Stage 4). Mirror `hipointe-demun.js`. **References NOTHING about any other installation.**
