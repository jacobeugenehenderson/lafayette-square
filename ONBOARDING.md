...
3. **Per-scene isolation.** All scene data lives under `cartograph/data/<scene>/` and `public/baked/<scene>/`. Add the `.gitignore` un-ignore block (mirror the HPDM/`ksi-y-m-yn` blocks) so config/content/census are tracked and heavy generated geometry (`clean/map|skeleton|ribbons|terrain`) stays ignored. ⛔ **`public/baked/<scene>/` is ignored wholesale (2026-09-01) and needs no un-ignore rule** — the pour uploads it to R2 and that upload, not a commit, is what puts a new town in front of a visitor. Verify it landed: ▶ `node scripts/verify-baked-in-r2.mjs --look=<scene>`. Mechanics: `PUBLISH.md §6`.
