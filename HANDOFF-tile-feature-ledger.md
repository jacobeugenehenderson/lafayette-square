...
| G3 | Divided-carriageway **median** (`anchor:'inner-edge'`, `innerSign`, `pairId`) | ✅ | T2.3 via `effectiveMeasure`; median is a thin tile flooding bare |
...
| G12 | **Capacity guard — thin-feature degeneracy** (W-past-inscribed-reach): when a tile's interior pinches below the band depth `WB=cw+tl+sw`, the inward offsets collapse past the medial axis → degenerate spurs `filletRing` rounds into **"thorns."** ⚠️ **SYSTEMIC — ~100 instances across the map** (Jacob, 2026-06-02): every thin loop / narrow median / sliver / tight wrap, not just Benton (Benton was only the easy exemplar). | ✅ **landed in code** (`d325c4b`, 2026-06-02) · pending eye | **DONE as the general per-tile guard** (corrects this row's day-old "❌ NOT ported" — verified against live code 2026-06-04): the shape pass binary-searches each tile's inscribed reach and **freezes `cap = 90%`** (`tileGround.js:936-938`); `sectionPass` + the curb clamp **every** inward offset depth to `cap` (`:498-506`) → over-capacity tiles "degrade to a clean truncated ribbon" — the whole thin-feature class in one move (NOT site-by-site, NOT a corner-R clamp — `feedback_no_corner_radius_clamps_in_emit`). **Remaining = Jacob's eye** that the ~100 thorns are gone across medians/slivers/loops (the consolidated eyeball), then port the doctrine to RIBBONS §3.9a. |
