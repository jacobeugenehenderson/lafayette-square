...
- **Preset cameras** (top-left, second row — Brief 13 Vantage 2026-05-23). Three buttons (Overhead / Hero / Street) snap the viewport camera to generic studio-inspection distances matching Brief 10's three bark-shader tiers: Overhead is ~150m+ above looking down (aerial silhouette verification), Hero is the existing studio framing (~mid-distance Browse default), Street is ~5m at human eye level (close-up bark detail read). Single-select; default `hero`; per-session UI preference (no persistence). NOT coupled to `uBarkShaderTier` — operator authors camera and tier independently so they can debug "what does street tier look like from overhead" mismatches. Camera mechanics preserved: Option+drag still cranes/turntables from any preset, wheel still zooms. Acts as the review tool for Brief 10's tier work and any future Salon tree-quality decision that needs the right viewing distance.
