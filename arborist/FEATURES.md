...
**What ships today, exactly.** Every tree in Lafayette Square paints — nothing is dropped. The **impostor is the foundation**: each placement renders as a captured canopy billboard, and the **tallest 15%** keep real `lod1` mesh geometry as anchors sprinkled through the scene, so articulated branch-motion and parallax read against a canopy sea that breathes with the same wind. Two impostor constructions are live, split by viewing hemisphere — the **overhead** 3-slice snapshot for browse (zoom out and the whole scene swaps), the **hero** azimuthal canopy bands for the low side-on pan. Both are render-to-texture captures of the actual tree through the shared atlas, so they match their mesh neighbours in colour and season. The geometry budget is a dial (`?heroGeom=`).


**The canopy is lit, not just dimmed (2026-09-03) — `?litCards=1`, default off.** A tree impostor is a flat card, and flat cards had no way to know where the light was: they carried a global *dimmer* keyed to the weather, so on a clear afternoon every crown in the neighborhood was the same value and the canopy read as one green mass. Now each card is shaded by the scene's **actual key light** — the sun by day, the sun→moon blend after dusk, the same light the buildings and the mesh trees answer to — with the baked occlusion channel deciding how much of it reaches deep into a crown. Individual trees separate: sunlit tops, shaded flanks, a canopy with depth in it. ⭐ **The change redistributes light without changing brightness** — by construction, not by tuning — so the trees you signed off on are the same trees, differing only in how light falls on them. It costs **no new baked pages, no re-capture and no re-bake**: a card's light-facing shape is synthesised from the card itself, which also sidesteps the failure mode a captured normal map invites (lighting a crown from the wrong side *while looking plausible*). The trunk/ground joint returns with it — a card's base takes the colour of the shadowed ground beneath it instead of meeting it as a hard edge. ⛔ **Ships as a knob defaulting to today's look**, so no town's map moves until someone turns it. ⛔ Still owed: trees neither cast nor receive real-time shadows. ▶ `node checks/claims-cards-light-from-the-scene-key.mjs`

**The impostor pool ships transcoded (2026-08-29).** Every hero + overhead page is KTX2/ETC1S, encoded **at the capture handler, on write** (`arborist/encode-ktx2.mjs`), so a town gets it from its first bake with nothing to switch on. ⛔ **The pages are BORN compressed, and that is load-bearing — it used to be the pour's "last step" and that was not last enough.** The GPU capture passes run in the BROWSER, after `POST /grove/bake` returns, and merged their own page paths into the manifest; nothing re-packed afterwards, so every bake that captured anything silently reverted the compression — worse the better the bake went. Measured 2026-09-03: 420 of LS's 426 declared pages had gone back to PNG, every `.ktx2` sitting un-referenced beside them (fixed same day). `pack-impostor-ktx2.mjs` remains as the idempotent net for pages captured before this. PNG is a *file* format the GPU cannot read — it decompresses to raw RGBA on upload — so the win is in memory, not just on the wire: LS's pool went **1,121 MB → 282 MB resident** (71 MB → 20 MB shipped), which is the difference between an embedded Ward a phone can hold and one the browser kills. Measured on the roster's hardest silhouette (`maple_silver az0_leaf0`, 4.4% opaque), alpha coverage at `alphaTest 0.5` moves 4.36% → 3.96% — a one-pixel boundary wobble, no silhouette loss. ⛔ **The leaf/bark atlas is deliberately NOT transcoded** and stays PNG: `treeAtlasMaterial#loadTexture` builds a CPU coverage-preserving mip chain so the far canopy keeps its shape instead of eroding to specks, and that pass cannot run on a texture whose mips live inside the file. ▶ `node scratch/claims-every-declared-page-ships.mjs`

**Not yet:** the load-streaming that orders GLBs along the pan, the Stage knob for the budget, and the atlas's own compression (it needs pre-generated coverage-preserving levels fed to the encoder — its own job). → `ARCHITECTURE.md "Tree-render reality at LS"`.
...
| `POST` | `/procedural/:species/publish?look=<id>` | **Authoring-only (Brief 14.1, Corbel 2026-05-25):** shells out to `node generate-procedural.js --species <id>` (which syncs the Look roster in its `main()`) + rebuilds the index. Stages species artifacts to the library; does **not** bake the slab atlas. ⛔ **`?look=` is REQUIRED** (400 without it) and is passed through as `--look <id>`: the publish writes the built variants into `<look>/design.json`, the authoring SSoT, and `generate-procedural#requireLookArg` refuses to guess which town that is (58a91317, 2026-09-20 — it had been editing Lafayette Square's roster from inside every town). Slab bake is still the explicit Grove gesture (`/atlas/bake`). |
...
| `POST` | `/salon/:species/publish?look=<id>` | **Authoring-only (Brief 14, Lintel 2026-05-23):** shells out to `node generate-salon.js --species <id>` + rebuilds the index. Stages species artifacts to the library; does **not** bake the slab atlas. ⛔ **`?look=` is REQUIRED** (400 without it) and is passed through as `--look <id>` — same rule and same reason as the procedural publish above (`generate-salon#requireLookArg`). Slab bake is still the explicit Grove gesture below. |
