...
- [ ] **Phase N.3 — Project: Li'l Vera Cycle 1 rev. 2** (DISPATCHED 2026-05-20 evening; brief at `scratch/phase-n2-lil-vera-observational-skeleton-brief.md`). Fresh build in new module `arborist/lil_vera_v2.py`, baby picks own name; Tycho's rev. 1 commits stand as baseline (5th alignment-oracle layer "Li'l Vera v1"). Brief restructured post-Fraunhofer-audit to be **tips-first, middle-out, adaptive-scan, growth-shaped**: ridge tracing removed entirely; precision-gated tip detection (six-gate conjunction) + RANSAC trunk give two confident anchor sets; bidirectional axonal *growth* (step-by-step probes, not cone-shot) from anchors through M_obs steered by species curvature priors, handshake fires on mutual recognition; adaptive scan via verdict-rate stopping (three guards: `batches_in_pass ≥ min_batches_before_stop=4` AND `eligible_fraction ≥ min_eligible_fraction=0.30` AND `batch_verdict_rate < 0.005`) — no fixed N, no geometric cluster detector. **Three load-bearing primitives**: (a) iteration loop with three-outcome elimination + masked rasterization (Rubin subtraction); (b) species-conditioned botanical priors as the third inference channel (hand-encoded `arborist/state/<species>/botanical-priors.json`; position-dependent expected-radius envelopes, expected-local-direction tangent field for growth steering, branch-angle distributions, Murray's-law joint constraints, hard rejections); (c) neuronal mutual recognition elevated to *primary* extraction primitive (not just connectivity-completer). Output is parametric splines (100–500 per tree, 3–10 control points each, `from_handshake` / `from_taper_only` / `orphan` flags per spline), NOT dense voxel nodes. CLI heartbeat (Std Req #10) for tail-f operator visibility during 1–2 hour runs. 5 stages: N.3.0 classifier+priors+tip-detector precision validation (the foundational gate before iteration or growth machinery) → N.3.1 adaptive scan + verdict-rate stop + masked rasterization → N.3.2 tip anchoring + axonal growth + handshake recognition (THE cycle gate) → N.3.3 pipe-model radii + taper co-determination → N.3.4 Rubin consensus-stability. ~6 days work + ~1.5 days operator validation = ~7.5 days end-to-end. New 6th alignment-oracle layer (orange-gold / deep-teal). v1 endpoints + state-dir untouched; v2 lives in parallel. Brief audited four times (Curie / Fraunhofer / Bessel / Doppler — see `arborist/NOTES.md` 2026-05-20 evening for the chain).
