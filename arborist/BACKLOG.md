...
- [ ] **Brief 21 — Normalize extreme-mis-scale chassis (the "empty"-roster un-hide).** Surfaced by the Brief 20 baby's chassis sweep + the operator's "the empties are just 300' tall" diagnosis (2026-05-25). **Root cause:** a pre-existing *extraction* bug — some chassis import at **900–1700m heights** (unit mismatch, almost certainly mm/m or ×100). They render **"empty" in the Salon preview** not because they lack geometry, but because `SpecimenViewport`'s framing distance scales with `treeH` (`studioFraming`: distance ∝ treeH/2) while Brief 13 clamped the camera at `D_MAX=300 / H_MAX=120` — so a 56–140× oversized tree can't be pulled back far enough to frame; the camera is trapped looking at the lower trunk slice → reads as blank. **Distinct from two other "empty" populations:** (a) *structurally* empty (zero-mesh, caught by `_quarantine-empty.js`'s `listMeshes().length>0` check — legitimately empty); (b) *normal* scale variation (12 vs 18m — can't auto-size per operator, stays the manual scale gizmo). The extreme outliers (>~100m) ARE auto-detectable as unit-bugs — **this is the one place auto-scale is justified.** **The fix:** detect chassis with height ≫ plausible-max (e.g. >100m) at the source (`survey-deleaf.js`) → rescale to real-world height, OR flag for operator review. Un-hides the roster chunk that's been invisible-and-assumed-empty. **Distinct from Brief 20:** recenter ≠ rescale — a recentered 900m tree is still 900m and still unframeable; Brief 20 does NOT fix this. Both operate at chassis-source; compose but **don't bundle** (Brief 20 is recenter-only, scoped). **Affected (Brief 20 sweep):** `acer_saccharum_a/c`, `gray_poplar`, `honey_locust`, + others at 900–1700m. **Related-but-separate:** `acer_saccharum_a/c` are multi-tree *forests* (several trees per file), a **curation/classification** issue (reject from the single-tree chassis pool via the Brief 1.5b curation surface, or reclassify) — NOT a scale fix; handle separately. **Open question for the brief:** rescale-to-canonical-height (needs a target — species-map canonical height? normalize bbox to plausible range?) vs flag-for-operator. Operator-requested 2026-05-25. Cold dispatch; composes with Brief 20 at survey-deleaf.

- [ ] **Brief 23 — Restore + make-durable the forest decomposition (Brief 20 `--clean` regression). SUBSUMES Brief 22.** Operator caught 2026-05-25 that Brief 20's `--clean` regen lost forest splits — multi-tree files reappeared as whole "group shots" in the Salon. **This is a data-integrity regression to repair, NOT a hide-forests job** — the splits belong in the library as individual single-tree chassis, and they used to exist. **Confirmed partial scope (forensic step extends it):** Riven's candicands ×12 survived; **gleditsia ×4 + populus ×2 appear GONE**; single-mesh forests (`acer_saccharum_a/c`) were never auto-split (Riven's root-detector can't see single-root forests — needs the spatial method); Sextant's `--clean` deleted **17 "off-origin orphans," some possibly legit splits.** **Subsumes Brief 22** (operator-confirmed merge 2026-05-25): the spatial single-mesh decomposer (old Brief 22 — trunk-cluster splitting built on Sextant's `surveyTrunkClusters`) is the *tool* for the single-mesh half, integrated INTO `survey-deleaf.js`'s regen path so splits are **durable across `--clean`** per `[[clean-regen-must-be-idempotent-complete]]`. **Two decomposition paths:** multi-root (Riven's `processBundleGlb` — repair why gleditsia/populus dropped) + single-mesh (the spatial decomposer). **Forensic-first** — alignment-check the regression map (which splits existed pre-Brief-20 via the `_chassis-curation.backup-2026-05-22.json` name-diff + Riven's documented 18, what process split each, what's recoverable) BEFORE restoration code; the fix shape differs sharply (re-run a path vs rebuild a lost process vs recover deleted data). **Composes:** Boz's `7eef6c5` already filters procedural + acer-LiDAR chassis from the Salon catalog; this restores the *vendor* forest splits. Interacts with Brief 21 (`acer_saccharum_a/c` also mis-scaled — decompose here, rescale there). **Recommend before 3A.** Full brief: `scratch/brief-23-restore-forest-decomposition.md`. Cold dispatch, solo.
