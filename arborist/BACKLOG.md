...
- [ ] **Brief 10 — View-aware bark rendering** (Phase V [trees]) — sub-phased A → D, each a separate ship.
  - [x] **10A — Aerial tier infrastructure** — **SHIPPED 2026-05-23 (Cork).** `uBarkShaderTier` module-scope uniform (mirrors `treeSwayUniforms`) added to `treeAtlasMaterial.js` with the aerial fragment branch (gradient LUT REPLACE at per-vertex `vBarkWorldYNorm`); hero (tier 1) is the default + current behavior; street (tier 2) falls back to hero until 10C. `aBarkWorldYNorm` per-vertex attribute computed at runtime-merge time in `InstancedTrees.jsx` (chassis-wide bbox) and in `stampTreeVertexAttrs` (Salon preview parity, chassis range passed by `SpecimenViewport`) — sibling to Sough's `aWindTier` per [[project_runtime_merge_vertex_attributes]]. Debug setter `window.__setBarkShaderTier(n)`; sub-phase D adds the Salon overlay; Brief 11 wires cartograph SHOT.
  - [ ] **10B — Posterization + hero substrate swap** — `extract-bark-posterized.mjs` pre-processing + `unifyAtlases` posterized sub-page + hero fragment retarget to posterized substrate. Atlas grows ~1–2MB per bark ref vs current vendor-color path.
  - [ ] **10C — Street tier full PBR** — `unifyAtlases` roughness (+ optional displacement) sub-pages + street fragment path. Atlas footprint guard at 8192² before ship.
...
- [x] **Brief 13 — Salon preset cameras** — **SHIPPED 2026-05-23 (Vantage).** Three preset camera framings (Overhead / Hero / Street) in `SpecimenViewport.jsx` matching Brief 10's bark-shader tiers. `presetFraming(preset, treeH)` mirrors `studioFraming`; new `camPreset` state defaults to 'hero' so existing workflow is undisturbed. Camera-state ref gained an optional `lookAtY` (Overhead tilts down to chassis base; horizontal otherwise). Crane H_MAX 60→120 and dolly D_MAX 150→300 to accommodate Overhead on mature 40m chassis. NOT coupled to `uBarkShaderTier` (operator controls independently — Brief 11 wires the cartograph SHOT driver). Button row sits below existing Studio/Worm row at top-left of viewport. **Doesn't fix:** Studio/Worm row's partial functional overlap with Hero/Street (Studio button's previously-NaN `studioFraming(targetCategory)` call silently corrected to use `topY`); animation between presets (v1.6 polish); auto-tier-binding (out-of-scope per brief).
