...
- [ ] **Brief 26 — Salon picker: Look-only toggle + browse-all + chassis species reassignment.** Surfaced 2026-05-25 (operator). Restructures the Salon Chassis picker around the real authoring workflow. **(1)** Retire the `Approved only` filter → **`Look only` toggle** (ON = active Look's roster species; OFF = full vendor single-tree library incl. un-approved bundle-splits, so the unlabeled singles decomposed from group packs are reachable). Approve/reject stays as a *label*, not a visibility gate. **(2)** **Chassis species reassignment** ("Generic Garden Tree" → "Weeping Willow"): a `speciesOverride` in `_chassis-curation.json` (name-keyed, survives `survey-deleaf` regen, inherits `source.species` when absent), honored by an `effective species = override ?? source.species` helper everywhere grouping reads species — `listSalonSpecies`, the chassis catalog, **and Brief 24's coverage join** (must be override-aware or a re-homed tree won't count). NEVER edits `meta.json`. Sub-phase 26a (data/override helper) before 26b (UI: toggle + reassignment control). **Forests stay suppressed (Brief 23a stays dormant — operator confirmed redundant); procedural/LiDAR stay excluded; no runtime/shader/slab touch.** Touches `SalonWorkstage.jsx` + `serve.js` + `generate-salon.js` → **serializes after Brief 3A commits**; coordinate effective-species helper with Brief 24. Full brief: `scratch/brief-26-salon-picker-look-toggle-reassign.md`. ~300–450 LOC.
