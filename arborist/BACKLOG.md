...
- [ ] **Brief 2 — Gradient-map bark + multi-stop tint editor.** Bark sampling currently reads color.jpg directly. Brief 2 introduces a per-composition gradient ramp: luminance(`color.jpg`) indexes a multi-stop gradient authored in the workstage (front + back, like the Phase F leaf gradients). Per-composition palette without re-baking the bark photo. NEW uniforms are NOT in scope — gradient LUT bakes into the published GLB's color tile at author time. Single shader program preserved.
