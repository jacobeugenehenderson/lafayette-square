...
▶ `node checks/claims-matched-axes-have-matrices.mjs` — the list, the term counts, and the
...
(`node checks/claims-leaf-pack-cells-agree.mjs`) and the stale part-index are all the same
...
- [ ] **⛔ `hipointe-demun`'s slab references 26 GLBs that do not exist** *(found 2026-08-29 by the new page/geometry check, not by anyone looking)*. **4,485 placement references** across `betula_pendula`, `magnolia_sp`, `nyssa_sylvatica` and others — the slab places trees whose baked geometry was never written, so every one of those loads 404s on a deployed build. ⭐ **This is the same class the LS bake hit and the reason the check now covers geometry as well as pages:** declared, absent, and invisible from the page itself. ▶ `node scratch/claims-every-declared-page-ships.mjs`
- [ ] **⛔ Three looks publish behind a stale cache-bust key** — `default`, `lafayette-square-staging` and `toy` (LS was among them until 2026-08-29 and is now clean). Cold consumers serve the PREVIOUS slab from cache **and no reload dislodges it**, which is exactly how a fixed bug keeps reading as broken. The pour stamps `scene.json#bakedAt` unconditionally now, so the cure is one re-pour per look. ▶ `node checks/claims-the-slab-freshness-key-is-not-stale.mjs`
- [ ] **KTX2/Basis the master tree ATLAS.** *(Narrowed 2026-08-29 — the impostor pool is done; the encoder, loader and transcoder all ship.)* The atlas is the part that could not simply be encoded: `treeAtlasMaterial#loadTexture` builds a CPU coverage-preserving mip chain (`scaleAlphaToCoverage`) so the far canopy keeps its silhouette, and basisu's own box-filter mips are exactly what that pass corrects. ⇒ the job is **pre-generating those levels and feeding them to the encoder as explicit mips**, not running the packer over it. LS `trees-atlas-color` + `normal` = 140 MB resident.
...
**OPEN / NEXT:**
- [ ] **⚠️ CREDIT THE LEAF PACKS** *(Jacob, 2026-09-13 — "they need crediting, but that's not an emergency")*. The barks are done and are the pattern to copy: `public/textures/bark/*/LICENSE.txt` names the source, the licence and the maps, per asset, and `public/library/INVENTORY.md` carries a `provenance` column for them. **The 6 scanned vendor leaf packs have neither** — `INVENTORY.md` says only `🟢 vendor`, which names nobody, and the hi-res sources in `assets/leaf-packs-2026/` are gitignored, so the terms exist outside the repo or not at all. ⛔ **The rest of the library is NOT in this gap — do not restate it as one:** `public/library/MANIFEST.json` accounts for all 266 parts as `authored` (253) or `procedural` (13), and the barks are CC0 (ambientCG · Poly Haven) with their files committed. ⭐ Kit-shaped fix, not a spreadsheet: the credits surface already exists and already reports what it cannot state — `cartograph/bake-sources.js` computes per-town credits and prints an `owed` list — it simply never reads `public/library/`. Point it there and the gap reports itself in every town, forever. ▶ Verify the claim before acting, it is two commands: `node -e 'const j=require("./public/library/MANIFEST.json");const s={};const w=o=>{if(Array.isArray(o))o.forEach(w);else if(o&&typeof o==="object"){if(o.source)s[o.source]=(s[o.source]||0)+1;Object.values(o).forEach(w)}};w(j.tree);console.log(s)'` · `ls public/textures/bark/*/LICENSE.txt | wc -l`
...
- [ ] **Trees stand on ground the map DRAWS as hardscape** — `bake-trees.js`'s own guard passes (it asks `makeZoneTester`, built from `shape.json` + `map.json` + the Look's `design.json`), but the ground the operator LOOKS AT is a different artifact by a different producer, and it disagrees: placements land on fence, cycleway, asphalt and pitch. ⛔ Two producers, one question — the defect class Layer 0 exists for. Uncited in any doc until 2026-09-03. ▶ `node checks/claims-no-tree-stands-on-drawn-hardscape.mjs` (re-run it; ⛔ never quote the counts).
