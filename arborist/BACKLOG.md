...
- [ ] **Brief 10 — View-aware bark rendering** (Phase V [trees]) — sub-phased A → D, each a separate ship.
  ...
  - [ ] **10B — Posterization + aerial/hero substrate swap** — `extract-bark-posterized.mjs` pre-processing + `unifyAtlases` posterized sub-page + fragment retarget so `<map_fragment>` reads the posterized sub-region under tier ≤ 1 (aerial AND hero); street stays on vendor color (10C). Operator-locked 2026-05-23: posterized is the substrate everywhere it doesn't hurt fidelity. Atlas grows ~1–2MB per bark ref vs current vendor-color path; aerial's file-budget savings compound (small posterized tile, no vendor sample).
