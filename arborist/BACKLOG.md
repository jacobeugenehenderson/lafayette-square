...
- [ ] **Phase N.3 — Project: Li'l Vera Cycle 1 rev. 2** (NEW 2026-05-20 PM; brief at `scratch/phase-n2-lil-vera-observational-skeleton-brief.md`). Fresh build in new module `arborist/lil_vera_v2.py`, fresh baby; Tycho's rev. 1 commits stand as baseline (5th alignment-oracle layer "Li'l Vera v1"). Adds **three load-bearing primitives** that rev. 1 missed: (a) iteration loop with three-outcome elimination + strictly shrinking working set + masked rasterization (the Rubin-subtraction primitive); (b) species-conditioned botanical priors as the third inference channel (hand-encoded `arborist/state/<species>/botanical-priors.json`; position-dependent expected-radius envelopes, branch-angle distributions, Murray's-law joint constraints, hard rejections); (c) neuronal mutual recognition as the connectivity primitive (bidirectional probe + reciprocal recognition, NOT MST closure). Output is parametric splines (100–500 per tree, 3–10 control points each), NOT dense voxel nodes. 5 stages: N.3.0 classifier+priors validation (foundational gate before iteration) → N.3.1 iteration loop + elimination → N.3.2 spline fitting + lock-in + mutual recognition (THE cycle gate) → N.3.3 pipe-model radii + taper co-determination → N.3.4 Rubin consensus-stability. ~5.5 days work + ~1.5 days operator validation = ~7 days end-to-end. New 6th alignment-oracle layer (orange-gold / deep-teal). v1 endpoints + state-dir untouched; v2 lives in parallel.
