...
- [ ] **Brief 20 — Recenter all chassis to origin (global lean fix + wind-frame fix).** Surfaced by Quartz (Brief 19 alignment-check, 2026-05-25) as the "Option 2" deliberately NOT bundled into Brief 19 — it's a separate decision with a much larger blast radius. **What it does:** bake a global recenter (dominant-trunk base → world origin) into every published chassis's geometry at publish time, so all trees ship origin-centered (trunk base at `(0,0,0)`, vertical). **What it fixes:** (1) the long-standing **lean issue** — Whittle's 141 single-tree chassis (Brief 0) were never recentered due to the byte-identity constraint, so they carry a baked-in positional offset in their inner mesh node (e.g. `TREE_00` at `T=[3.9,0,−3.8]`; broadleaf_rt3 base Y≈−1.3, trunk centroid X≈−3.0/Z≈−0.7) — the visible cause of decentered/leaning chassis (`survey-deleaf.js:1162`); (2) **Sough's wind-frame assumption** (Brief 9a) — `injectFoliageSway` computes sway assuming trunks sit at `X≈Z≈0, base≈0`; off-origin geometry makes the sway frame subtly wrong; recentering makes the assumption true. **Mechanism:** reuse Quartz's dominant-trunk finder (`generate-salon.js#computeAutoCenterPivot`, a documented port of `SpecimenViewport.jsx#computeDominantTrunk` — Brief 19 did NOT lift the two to a shared module since they read different representations, three.js scene vs gltf-transform doc; **a shared-helper lift per the classifier-lift doctrine is itself a follow-up candidate, since the two can silently diverge and break the bake-matches-viewport guarantee**) to locate each chassis's base, bake `translate(−base)` into POSITION at publish. **Compose order with Brief 19:** Brief 19 bakes the authored orientation/scale correction *in-place* (Option 1, about the trunk base, base stays put); Brief 20 then translates the corrected geometry so the (now-vertical) base sits at origin. 19 = orientation/scale; 20 = global translation — complementary, 20 runs after 19's bake. **Relationship to 3A:** NOT a hard 3A blocker (3A computes its deformer pivot from published geometry at merge time, so it handles off-origin fine) — but 20 simplifies 3A's pivot to trivially-origin and removes the wind-frame caveat, so landing 20 before 3A is cleaner. **Blast radius — the reason this is its own deliberate brief, not a 19 rider:** it **breaks byte-identity for every existing composition** (all geometry shifts), forces a **full-roster re-bake**, and could **move trees the operator already positioned** via authored posOffset (their posOffset meaning shifts relative to an origin-centered frame). Needs its own verification pass + operator sign-off to take the blast radius on purpose. **Watchout:** confirm what byte-identity break invalidates (existing baked slabs, Brief 1.5b curation file byte-refs, determinism contracts) before dispatch. **Operator-decision-required before dispatch.** Cold dispatch after Brief 19 ships.
