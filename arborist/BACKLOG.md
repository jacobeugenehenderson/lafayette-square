...
- [ ] **Brief 27 — Grove population from Salon compositions (retire the rating gate; update on Re-publish).** Surfaced 2026-05-25 (operator). The Grove is still the OLD regime — a "gallery of *rated* variants" (`qualityOverride` Fill/Mid/Hero set in a workstage → "All Rated") + a manual "Add to Look" toggle. Under the roster-driven regime (Brief 26), composing + Re-publishing a composition IS the intent; the rating gate + manual-add is obsolete. **Fix:** populate the Grove from **published Salon compositions in the active Look's roster** (via `syncLookRoster` on Re-publish — the decided timing, consistent with Brief 14's Adopt=author → Re-publish=stage-to-library → Grove-bake=ship-to-slab), retire the rating gate for *visibility* (keep per-Look membership toggle). **Decouples `qualityOverride` from Grove visibility only** — whether `bake-trees#pickVariant` still uses it for substitution (likely vestigial w/o procedural fillers) is a reported-but-separate question. "All Rated" scope retires/repurposes (coverage view + Salon navigator supersede it). **Depends on Brief 26; dispatch after it; Cadastre warm-natural** (owns Grove + 24 + 26). Touches `Grove.jsx` (+ maybe `serve.js`/`generate-salon.js`). Full brief: `scratch/brief-27-grove-population-from-compositions.md`. ~150–250 LOC; authoring-only.
