...
**Three inspection points before writing the transform (per `[[feedback_geometry_briefs_need_artifact_inspection]]`):**
1. **Normal var at the injection point — the load-bearing finding.** `objectNormal` is NOT usable at `<begin_vertex>`: in `MeshStandardMaterial` the normal chunks (`<beginnormal_vertex>`→`<defaultnormal_vertex>`→`<normal_vertex>`) all run BEFORE `<begin_vertex>`, so by the time `transformed` exists the normal is already baked into `vNormal`. The brief anticipated exactly this ("you may need to inject into both chunks"). **Resolution:** build the lean∘twist `mat3` in the `<beginnormal_vertex>` patch, rotate `objectNormal` there, and stash the matrix + wander offset as `main()`-scope locals (`cantRot`/`cantWander`) that the `<begin_vertex>` patch reuses on `transformed`. Cross-chunk locals are idiomatic — three.js itself relies on `objectNormal` flowing beginnormal→defaultnormal. Not a premise failure; the anticipated remedy.
2. **Instance-anchor accessor — confirmed, reused.** The wind block already derives `instWorld = vec3(instanceMatrix[3].x,0,instanceMatrix[3].z)` with a `modelMatrix[3]` non-instanced fallback (Cambium B7 added the fallback for the single-Mesh Salon preview). The deformer hash uses the same accessor — instance anchor, not the deformed vertex, so every vertex of one tree shares one signature.
3. **Base≈origin sanity — confirmed three ways.** Brief 20 (Sextant) in the log; `T_autocenter = translate(-trunk.x, -trunk.minY, -trunk.z)` at `generate-salon.js:783`; chassis meta `heightRange[0] == 0` (acer_saccharum_q, italian_cypress_o). Pivot = origin, trusted — no per-chassis pivot, no `uDeformPivot`, no merge-time pivot scan. This is the premise the brief waited on (originally drafted off-origin).

**The math.** `h = aTreeHeightNorm` (0 base → 1 top). Lean = rotate about the horizontal axis perpendicular to a per-instance compass azimuth, angle `leanAmt·h` (verified Rodrigues: rotating +Y about `axis=(leanDir.y,0,-leanDir.x)` tips the top toward `leanDir`). Twist = rotate about local Y by `twistAmt·h`. Compose `R = leanRot · twistRot` (twist first, then lean). Wander = `wanderAmt·sin(h·π·1.5 + phase)` along a per-instance direction, added to `transformed.xz` after rotation. Hashes: `dh5` (lean+twist magnitude, per brief's correlation spec), `dh6` (wander magnitude), `dh7` (lean azimuth), `dh8` (wander dir+phase) — all `fract(sin(dot(anchorXZ, k))·43758.5453)` with fresh `k`s uncorrelated with the fragment `jh1-jh4`. Identity is **bit-exact** when ranges are `(0,0)` (lc=1,ls=0 → identity `mat3`; wander=0), so every species without an authored deformer renders pixel-identical (AC#5).

**Surfaces (per `[[feedback_baby_must_surface_scope_drift]]`):**
- **applyBarkUniforms vs sibling:** added a SIBLING `applyDeformerUniforms(material, range, seed)` — bark + deformer stay separable and the preview re-roll seed is deformer-only.
- **Lean azimuth source:** third hash channel `dh7` (not authored) — direction varies per-instance independent of magnitude.
- **Wander freq/phase:** freq is a FIXED constant (1.5 half-cycles over the trunk); phase + direction are hashed (`dh8`). Not authored — kept the panel to 3 ranges.
- **Multi-instance preview — MVP limitation:** the Salon preview renders ONE tree → one hash sample. Added a `uDeformSeed` uniform (0 in LS) + a "Re-roll preview sample" button so the operator cycles across the authored spread. True multi-instance preview deferred (would need instancing the preview scene). Live binding: the deformer is uniform-driven (not atlas-baked), so the preview reads the LIVE authored range straight from workstage state — drag a slider, see it instantly, no preview-atlas re-bake.
- **Preview frame caveat:** the preview applies the operator's straightening tilt LIVE via the group transform (Brief 19 bakes it only at publish), so for a not-yet-straightened chassis the preview deformer composes in the pre-straightened orientation. For correctly-authored chassis (the norm post-Brief-20: Y-up, base at origin) preview and LS match exactly. Minor, edge-case only.
- **Reintroduced Cork's retired chassis-wide Y-scan** for `aTreeHeightNorm` — verified it doesn't conflict with what 10A removed: Cork retired the scan because `aBarkWorldYNorm` was a camera-angle-dependent BARK consumer; the scan itself was always sound. Height-norm is a legitimate consumer.
- **`deformerBySpecies` in trees-atlas.json:** AC#8 ("byte-identical") is about GLB geometry + atlas PNGs (the heavy artifacts) — none change. A re-bake adds one metadata key (`deformerBySpecies`, empty until a deformer is authored), exactly like barkDetailBySpecies/barkPosterizedBySpecies did. Runtime defaults a missing key to `{}`, so old bakes still load.

**Validation.** vite build transformed all 1040 modules clean (the failure is a pre-existing dangling `public/photos/.../other` symlink in the static-copy step, unrelated). node --check on bake-look.js + generate-salon.js. **GLSL only compiles in a real WebGL context — the operator-eye visual sign-off in Salon (AC#1/#3/#4/#6) is Jacob's to do; I could not run WebGL headlessly.** Single commit when the visual AC passes.

## 2026-05-25 — Brief 23: Chassis-library fortifications + forensic that overturned the regression premise (Mistral)

**Baby: Mistral** (a wind — off the saturated architectural/mineral/instrument namespaces). Boz drafted, Jacob dispatched. Solo, warm (forensic → re-scope → build, all one session).

**The forensic killed the brief's premise, and that was the deliverable.** Brief 23 was filed as a "restore the forest decomposition lost by Brief 20's `--clean`" repair. The forensic (AC #1, surfaced as an alignment check before any code) found **there was no regression:**
- **All 42 of Riven's multi-root splits are present.** Boz's brief said "Riven's 18" and "gleditsia + populus appear GONE." Both wrong: Riven's own report (line 14) says **42** decomposed chassis, and they're stored under **common-name** labels — `gleditsia_triacanthos`→`honey_locust_*` (12), `populus_alba_fall`→`poplar_fall_*` (4), `populus_canescens`→`gray_poplar_*` (12), candicands (12), platanus→`london_plane_*` (2). Boz grepped the binomial vendor names and didn't find the common-name outputs.
- **The 17 "orphans" Sextant deleted were all correctly stale** (matched the PRE→CUR curation diff exactly): `broadleaf_rt3_a/b` (now no-wood via Brief 6.2 classifier evolution), `candicands_b` + `poplar_fall_a..g` whole (pre-decomposition group-shots, superseded by the splits), `procedural_broadleaf_*` (different concern), `robinia_pseudoacacia_a_tree_*` (→ renamed `black_locust_a..d`, present). Zero legit splits lost → AC #2 + #6 satisfied by forensic, no restoration code.
- **The real "group shots" are the single-mesh forests** (`acer_saccharum`, `acer_saccharum_lowpoly`, `burnt_tree`) — never auto-split by any process (Riven's detector is root-based; these are single-root). Operator clicked through the Salon and saw none currently (they're approved-null/LiDAR-filtered) — so this is "unlock these as usable singles," not "fix a live bug."

**Second alignment check (operator-requested): mesh classification.** Per-WOOD-primitive trunk-cluster counting showed the forests are the HARD case — one bark primitive holds all 17–22 trunks (organized by material, not by tree). So splitting = per-triangle nearest-trunk segmentation with crown interleave. Operator's call: **sub-phase the segmentation → Brief 23a**, ship the tractable fortifications here.

**Shipped (4 fortifications, all in `survey-deleaf.js` + a `generate-salon.js`/`serve.js` consumer):**
1. **`--clean`** (new flag; didn't exist — Sextant's "clean regen" was a manual `rm`). Idempotent-complete per `[[feedback_clean_regen_must_be_idempotent_complete]]`: after a full regen, prunes a `_chassis/` orphan ONLY if its `meta.source.species` is a vendor dir still present + walked this run (genuinely-superseded output it just regenerated equivalents for). REFUSES + surfaces everything else — procedural source, LiDAR-seedlings source, source dir gone, or no meta. Verified with 4 synthetic orphans: stale-vendor deleted; procedural/absent-source/no-meta all refused with reasons. This is the guard the `procedural_broadleaf` loss should have had.
2. **Rescale** (folds old Brief 21). `>100m → divide by 10^round(log10(H/15))` (the power-of-10 toward a 15m target; never up-scales), applied post-recenter (base stays at origin; uniform scale leaves NORMALs). **Bundle path uses ONE divisor from the whole bundle's tallest root**, applied uniformly — so candicands' 56/87/110m siblings all ÷10 (→ 5.6/8.7/11, order preserved) instead of only the one >100m sibling shrinking (an inversion). honey_locust/gray_poplar/poplar_fall ÷100 → 6–12.5m. `meta.source.unitRescale` records the divisor. **Watchout:** rescaled chassis need `derive-leaf-attachment-tags.mjs` re-run (tags are mesh-space; survey-deleaf already resets them to `[]`, so this is the existing post-regen step regardless).
3. **Forest detection + worklist + suppression.** `surveyTrunkClusters` promoted from a Sextant console-warn to a real gate, run **per-WOOD-primitive** (keys on "N trunks merged in ONE mesh" — the hard-case signature — NOT whole-chassis count, so it can't sweep the 58 separable splits, which take the bundle path or are file-pre-split singles). Gate is **≥3 trunks** — 2-trunk readings are multi-STEM organisms (`sugar_maple_multistem`) or forked-base singles (`italian_cypress`/`blue_spruce`/`common_beech`/`western_juniper`), deliberately kept (operator's stem-vs-tree distinction; "don't sweep good chassis"). Writes `state/_chassis-forests.json` (producer-derived, regenerated each run, NOT operator state) = 7 forests (acer_saccharum a/c 17/22, sugar_maple_low_poly_forest a/c 17/22, burnt_tree a/d/f 7/4/8). `serve.js` `/salon/:species/chassis` excludes them via `generate-salon.js#listForestChassis`; verified the 58 splits + garden_mix stay.
4. **Curation reconcile.** Only one live orphan (`broadleaf_rt3_a`, permanently no-wood) — removed. No live `robinia_*` entry existed to migrate (the dangle was only in old snapshots). `--clean` now surfaces curation orphans read-only, preserving the never-touch-curation invariant (curation lives in `state/`, survives regen).

**Determinism (AC #4 + #8):** worklist + chassis byte-identical across runs. 241 chassis (unchanged count). Bundle-divisor + per-prim cluster gate are both deterministic.

**Surfaced for operator / Boz:**
- **Brief 23a may not be worth building.** The merged forests are a handful of stylized/burnt low-poly assets, none in active use, now suppressed + rescaled; individual Sugar Maples already exist via the LiDAR/procedural hero. The 3D segmentation is genuinely hard (esp. `burnt_tree`'s noisy 45-raw-cluster count). Recommend confirming a concrete need before dispatch — else leave them suppressed.
- **Brief 21 is effectively closed** (folded + shipped here). Its BACKLOG entry can be marked done.
- **`burnt_tree` at 109–124m → ÷10 → ~11m** is a borderline mis-scale call (a real tree could be ~110m, but burnt forest stock at that height is a unit-bug); uncurated/not-in-use, low-risk.
- **Slab propagation:** like Brief 20, the rescaled/suppressed library reaches the slab only after affected roster species are re-published then baked (Brief 14 decouple). The lafayette roster's mis-scaled chassis-derived species (honey_locust etc.) want a re-publish + Grove bake.

## 2026-05-25 — Brief 20: Recenter all chassis to origin at the SOURCE (Sextant)

**Baby: Sextant** (a measuring instrument — fitting for a brief about where the trunk base *measures* to; off the saturating architectural/mineral namespaces). Boz drafted, Jacob dispatched. Solo cold dispatch, serial-before Brief 3A.

**The fix.** `survey-deleaf.js` now recenters EVERY chassis so its dominant-trunk base lands at origin `(0,0,0)`, at chassis-creation time. The 141 single-tree chassis (`processGlb`) lose Brief 0's byte-identity carve-out — their baked node translation + cm/mm→m scale fold into POSITION via a new `bakeSceneTransforms` (mirrors `generate-salon.js#bakeAllNodeTransforms`: parentless-node walk, shared-mesh Set guard, orphan identity net, full TRS reset), then a `translate(−dominantTrunkBase)`. Bundle chassis (`processBundleGlb`) switch from bbox-XZ-center to the same dominant-trunk recenter. New helpers: `computeDominantTrunkBase` (the recenter pivot) + `recenterPrimsToDominantTrunk` + `surveyTrunkClusters` (diagnostic only).

**Port, not shared-lift (the brief's open call).** Chose to PORT the dominant-trunk algorithm a third time into `survey-deleaf.js` rather than lift a shared core imported by all three consumers. Reasons: (1) the brief's own logic — on a chassis recentered HERE, all three finders return ~origin and **agree by construction**, so the KEEP-IN-SYNC drift hazard goes quiet from the recenter ALONE, regardless of port vs lift; the shared-lift's headline payoff is already captured. (2) **No precedent** for the browser (`src/`) importing executable JS from `arborist/` — only a JSON file (`leaf-pack-bindings.json`) crosses today; `atlas-kind-classifier.js` is shared between Node scripts only. A shared core would introduce a new, untested Vite→Node-script import boundary AND force a refactor of the shipped, Brief-19-stabilized `computeDominantTrunk` in the browser path. (3) survey-deleaf already manipulates gltf-transform docs exactly like `computeAutoCenterPivot` — near-verbatim, one-file blast radius. (4) The shared-lift is the right *end state* and pairs with the deferred cleanup (delete-the-quiet-finders) — lift-then-delete-two is churn; lift when we delete. The new copy carries a ⚠ comment making the "agree by construction" reasoning explicit.

**Why a single recenter pass (not iterate to bit-zero).** The estimator is grid-quantized (0.5m XZ bins). For a trunk straddling a cell boundary, the densest-cell pick can flip under the very translation we apply, leaving a sub-grid XZ residual that **no translation can drive to exactly zero** — verified it's a period-2 limit cycle (iterating just flips the residual's sign at the same magnitude). Y is exact (not quantized) → always lands at 0. So we translate by the pivot the downstream finders ALSO compute (byte-identical estimator); their auto-center drops from the chassis's full multi-metre offset to just this residual — "goes quiet" / "≈ identity" per AC #3, not bit-zero. **Residual distribution across 241 chassis: 185 < 0.1m, 231 < 0.5m, 7 at 0.5–1.5m, 3 outliers** (the mis-scaled multi-stem `candicands_*_bark_144` at ~32m — see watchouts).

**Composes with Brief 19.** The viewport stacks `R·S·T_posOffset·T_autocenter·scene` (`SpecimenViewport.jsx:856-865`) and the Brief 19 bake conjugates `T_autocenter⁻¹·R·S·T_posOffset·T_autocenter`. On a recentered chassis `T_autocenter ≈ I`, so the conjugation degenerates to plain `R·S·T` about origin and authored `posOffset` stays valid — the recenter does at source what the viewport auto-center did at display. AC #4/#5 hold by construction (modulo the sub-grid residual above).

**Verification (artifact-level; no browser this session).**
- AC #1 — origin sweep: 231/241 chassis within 0.5m of origin, all `minY=0`.
- AC #2 — dominant-trunk not bbox: `american_linden_a` trunk (0.22, 0.66) vs bbox-center (0.26, 1.64); `london_plane_g` trunk is the anchored reference while bbox-center is (−0.49, 0.08). Confirmed.
- AC #3 — finders quiet: the residual the verify probe computes IS the pivot the downstream estimators return (identical math) → auto-center ≈ residual ≈ 0.
- AC #6 — determinism: byte-identical chassis aggregate sha across runs (`51f5c0b…`).
- AC #7 — curation: 37/38 keys resolve (name-keyed, survives regen). One stale entry: `broadleaf_rt3_a.glb` (now reclassified no-wood — see watchouts).
- AC #8 — bake: `bake-look --look lafayette-square` completes exit 0, 93 GLBs rewritten, `errors: []`. **Slab NOT committed** — reverted the `trees-atlas.json` change (baked from old published artifacts; the real slab is the operator's re-publish→bake→commit gesture per dispatch posture).
- AC #9 — gizmo tools intact (tiltX/Z, rotateY, scaleOverride, posOffset, oubliette) — those files untouched.
- AC #10 — no consumer depends on the off-center frame (grep hits were a log, timestamps, LiDAR run-data; all real consumers re-derive center at runtime).

**Watchouts surfaced (per `[[feedback_baby_must_surface_scope_drift]]`).**
- **Slab propagation needs re-publish FIRST.** Brief 14 decoupled publish from bake, so recentering chassis reaches the slab only after the affected roster species are **re-published** from the new chassis, *then* baked. A bare `bake-look` reads already-published (old-frame) artifacts. Operator's two-gesture loop: re-publish affected vendor/Salon species → Grove bake. The lafayette-square roster's chassis-derived species: `platanus_acerifolia` (london_plane — has residual), `acer_saccharum` (forest!), `cedar_generic`, `generic_leaf_tree`, `generic_tree_2`, `alaskan_cedar_2`.
- **Single-mesh FOREST chassis (operator-directed capture).** `surveyTrunkClusters` (connected-component labeling on the slab grid, ≥50%-of-dominant-mass + ≥2-cell threshold, tuned 2026-05-25) flags chassis where >1 distinct trunk cluster exists = several separate trees in one file → dominant-trunk centers the whole scene on one arbitrary stem. **This is a classification problem, not a centering one** — recentered best-effort, captured for curation + the forest-split follow-up (now filed as **Brief 22** — extends Riven's Brief 1.5c root-based detection with this spatial trunk-cluster counting, reusing the dominant-trunk binning). Flagged: `acer_saccharum` (skeleton-1: 17 clusters, skeleton-3: 11), `acer_saccharum_lowpoly` ("Sugar Maple low-poly forest", 11–19), `burnt_tree` (11–57, also mis-scaled), + a handful of borderline 2-cluster cases (Italian Cypress, Blue Spruce, Common Beech — verify in the follow-up). Multi-STEM organisms (gray_poplar, honey_locust) reach the path only via the bundle splitter (already root-decomposed) so are NOT flagged — correct per the operator's stem-vs-tree distinction.
- **Mis-scale is NOT this brief (→ Brief 21).** Several bundle chassis ship at absurd scale (candicands ~110m, gray_poplar ~983m, honey_locust ~1700m tall — un-applied cm→m or unit mismatch at extraction). Recenter is harmless on them (centers the base; a 900m tree is still 900m and unframeable). The `candicands_*_bark_144` ~32m residual is this class crossed with multi-stem — best-effort recentered, left for Brief 21 + the forest-split follow-up. All uncurated (not in use).
- **`broadleaf_rt3` is now no-wood (doctrine rot).** The brief named `broadleaf_rt3` as a spot-check target, but current classification scores all 4 of its prims LEAF → `skipped-no-wood`, so no chassis is emitted. The curation file still carries a `broadleaf_rt3_a.glb` entry (approved=null, "Broadleaf") — now an orphan (harmless; name-keyed curation preserves absent keys). `broadleaf_rt3` is also still in the **lafayette-square roster** but has no live chassis — a pre-existing inconsistency the operator may want to re-classify or drop.
- **Stale-orphan accumulation in `_chassis/` (pre-existing, fixed via clean regen).** `survey-deleaf.js` only WRITES chassis, never prunes, so the gitignored `_chassis/` dir had accumulated 17 orphans from older runs under since-changed naming/classification (`broadleaf_rt3_a/b`, `candicands_b`, `poplar_fall_*`, `procedural_broadleaf_*`, the old `robinia_…_tree_…` bundle names) — all off-origin (old code) and a quiet pollutant for any roster picking them up. Did a clean regen (rm gitignored `_chassis/*` → re-run) to land 241 = 199 single + 42 bundle with zero orphans. **Consider a `--clean` flag or generation-stamp prune in survey-deleaf** so future re-runs self-heal instead of accreting.

**Deferred (NOT this brief, per spec):** deleting the now-quiet `computeDominantTrunk` / `computeAutoCenterPivot`; collapsing Brief 19's conjugation to plain `R·S·T`; the shared-lift of the dominant-trunk core. All belong to the post-verification cleanup once recentering is observed working in the Salon.
...
**What shipped.** `decimateLeafPrimitivesConnectedMesh` in `decimate-tree.mjs` — a clone of Adze's Lever 5 (`decimateBarkPrimitives`) with two swaps: gate `bark`→`leaf`, and add a `maxVertUse > 1` connected-mesh check so it's disjoint from Spindle's Lever 3 (card-based, `=== 1`). `MeshoptSimplifier.simplifyWithAttributes` at `errorTolerance=0.02, targetRatio=0.20, uvWeight=1.0`. Idempotency flag `extras.gnomonDecimatedLeaf`. `leafDecimation` block added to `decimation-defaults.json` + `DEFAULT_CONFIG`; new immutable `decimation-defaults.defaults.json` backstop (matches the posterize/leaf-attachment `.defaults.json` precedent). Call site in `publish-glb.js` after Lever 5, before emitLod. Full survey: `scratch/brief-6.3-leaf-decimation-survey-gnomon.md`.

**Headline result — Lever 6 works, within its scope.** Linden leaf prim **416,888 → 83,377 tris (−80.0%)**, err 6.18e-3. UV sub-region preserved byte-tight: Linden's leaf + bark share ONE material (`EuropeanLindenBark_Mat`) in disjoint UV sub-regions (leaf U[0.445,0.989] V[0.619,0.770], bark U[0.008,0.425]); 0 of 173,658 post-decim verts drift into the bark region. uvWeight 1.0 earns its keep. Deterministic (sha1-identical across runs), idempotent, deployed-path verified.

**The big finding — the brief's premise about lod2 is wrong.** Adze fingered the leaf prim as Linden's lod2 floor-bearer; the brief assumed killing it would clear lod2. It doesn't. I probed per-prim: the **post-Lever-6 leaf collapses freely to ~100 tris** under emitLod — it's no longer a floor-bearer at any LoD. The remaining lod2 floor (~43K emitLod / ~57.7K attribute-aware) is **~99.8% the bark prim** (Adze's Lever 5 output), whose UV-locked topology floor is **error-budget-independent** (flat from error 0.02→0.4 — verified). `simplifySloppy` crushes bark to ~1 tri but obliterates atlas-UV addressing. So clearing Linden-class lod2 is a bark / Lever-4 / bracket problem, not a leaf problem. Filed **Brief 6.3-followup** (bark lod2 floor) with three candidate paths: bracket retune / impostor lod2 / sloppy-with-vertex-lock. Did **not** ship the retune (scope wall per brief).

**Second finding — Robinia is card-based, contradicting AC #3 + Adze's survey.** The active `robinia_pseudoacacia_a_tree_*` chassis ships leaves at maxVertUse=1 (card-based), so Lever 6 correctly skips them — they're Spindle's Lever 3 territory. This is *fortunate*: republishing Robinia on the deployed path showed **Lever 3 firing (leaf 324,202 → 217,648)** — the first deployed-path proof of Spindle's lever, which **closes Adze's open item** (he couldn't prove L3 fired in the bake chain because both his test species had connected-mesh leaves).

**Scope notes surfaced.** (1) `simplifyWithAttributes` has **no positionWeight knob** in meshopt 1.1.1 — `attribute_weights` weight only UV; position is the quadric base. The brief's `positionWeight: 1.0` is inexpressible; kept as a documented-inert defaults field, not passed. (2) Firing set is **broad** — 29 connected-mesh leaf prims across ~14 species (birch/red maple/aspen/white oak/tall pine/juniper/peach/white fir + Linden), not Linden-only; that's the real mobile payoff even with lod2 unbracketed. (3) Sugar Maple + Italian Cypress leaf prims are connected-mesh but below the 100K threshold → no-op (Cypress does have small leaf prims, contra the brief's "no leaf prims"). (4) **Operator-eye Salon visual sign-off (AC #4) outstanding** — I proved UV addressing survives at the data level but did not fake a visual pass; the Vantage Ground/Overhead "does it still read as a Linden" check is Jacob's per [[feedback_salon_preview_is_authoring_surface]].

**Verification hygiene.** All testing ran against throwaway `_gnomon_*` species dirs (publish-glb writes under gitignored `public/trees/`); removed them and rebuilt `index.json` back to 69 species/44 variants. No tracked-file pollution. Scratch probe scripts deleted. The other working-tree changes (posterize-defaults*, brief-6.2*, brief-14/18) are pre-existing — not mine, not committed by me.
...
- **Sub-phase B composes naturally — aerial uses posterized too (operator-locked 2026-05-23).** Posterization affects what `diffuseColor.rgb` contains before the luminance computation. Aerial and hero BOTH pick up the posterized substrate; aerial drops detail, hero keeps it. Fragment retarget is one line: `<map_fragment>` reads from the posterized sub-region under ALL tiers ≤ 1 (street's vendor-color stays for 10C). The "aerial on vendor color, hero on posterized" alternative is rejected — operator wants posterized as the substrate everywhere it doesn't hurt fidelity (i.e. everywhere but street). This also means the aerial-tier file-budget savings compound: aerial reads the small posterized tile, not the vendor color.
