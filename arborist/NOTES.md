...
- **Grove:** no-filler + **one-tile-per-species** dedupe (was showing procedurals + N variants of the same tree).

**B — the OPERATING MODEL (settled in prose; the aspirational direction). Full statement in `ORIENTATION.md §The operating model` + `BACKLOG.md`.**
- **Four surfaces, one judgment each:** **Intake seeds** the Look (mandatory-*real*: every census species → nearest real chassis or an honest gap, **never a filler** — the no-filler gate moved to Look-birth) → **Salon adds/composes** (add blind, one tree) → **Grove culls in context** (see it next to siblings; per-Look remove **and** global demote) → **Bake ships**.
- **Promote / Demote, NO ratings.** Kill the 0–4 Fill/Mid/Hero scale (false precision). **Promote** = vouch eligible (Salon). **Demote** = set aside, **reversible / non-destructive** (Grove). Untouched = default. No hero bit.
- **⭐ Categorize, don't recommend (Jacob's pivot).** A per-species matcher *score* is brittle ("Recommended (0)" everywhere). The rubric already **closes the set** — no scoring needed: **9 chassis HABITS** (`vase·columnar·oval·spreading·weeping·multi-stem·pyramidal·rounded·irregular`, rubric axis 0), **10 leaf shapes** (axis 10), **~8 bark types**. Parts live on **shelves**; each chassis is assigned ONE habit (a fact); a species declares its habit → lands on that shelf. **THE composition workspace is built on this, not a matcher.** Tagging = assign-1-of-9 (the ~80-chassis habit-untag backlog) — a curation afternoon, not ML. This supersedes the kit-matcher *recommendation* framing.

**C — Salon interface pass (LANDED: `SalonWorkstage.jsx` · `SpecimenViewport.jsx` · `ChassisPlate.jsx` · `generate-salon.js`).** Leaf Source → 3-way **`Bare · Native · Synthetic`** (Bare is an authored state that *ships bare*; dropped the preview-only Visible toggle). Chassis/Bark/Leaf **pack libraries → collapsible + called-out** (an `emphasis` chip style on the library headers). **Removed:** the Overhead (Browse) section, the Name/Reset/Set-Canary footer, the "SLOT·adopted" header, the "all adopted" counter, the species-header `Recommended / Show all` + `Mark not-available` cluster, the roster Todo/Done/N-A task tabs, the `canonical <slug> · composed` line + the roster-row state words, and the leaf helper-text. **Moved/changed:** species intro (ReferencePanel) → **top of the tools stack**; `1 tree / 3 variants` → **`Solo / Group`**; chassis thumbnails **re-centered** (subject-centered, not feet-on-floor); Fix-orientation button re-aligned. Canary *setting* removed from the Salon → the **Meteorologist** is its home (picker there is punchlisted; it still renders its last-set value).

**Open threads (verified, for pickup):**
- **Bark knobs are DEAD on Native trees** — and the code says so (`generate-salon.js:1571`): `tint / uvScale / roughness / jitter` are runtime shader uniforms keyed by a per-species **bark manifest entry**; no entry → identity uniforms → knobs do nothing. Native/substituted trees have no entry. Fix: **hide-when-inert**, or give every composed species a bark entry. (The bark **REF swap** *does* work everywhere.)
- **Yellow coverage → empty Salon.** A 🟡 (composite/substitute) species opens *blank* because the substitute is a bake-time routing, not a seeded composition. The **intake-seeds-the-Look** wiring closes it: 🟡 → pre-load the substitute · 🟢 → pre-load the literal · only 🔴 is empty.
- **Species-intro blank without a dossier** (~10 of 84 have one) — needs the light fallback (name · botanical · category · coverage), upgrading to the full dossier card when tagged.
- **Coverage dots** (the roster lights): 🟢 literal (right tree, genus-match) · 🟡 composite (covered by a substitute/cousin) · 🔴 gap (no tree). Expect **more 🟡** now (real substitutes replaced procedural fillers — honest, not a regression).
...
- `/salon/:species/publish` shells out to `generate-salon.js --look <id> --species <id>` — one species, on demand. ⛔ **It is no longer the ONLY regenerate-from-source path**: the fold landed 2026-06-25 (`15682e55`) and `/grove/bake` now runs the same tool over ALL species as step 1, which is why a bake cannot ship stale GLBs. Both callers pass `--look` — the tool refuses without it (2026-09-21).
...
**2. The stale-GLB trap (root cause of "miniature/missing leaves").** Published GLBs in `public/trees/<sp>/` were **May-25**, before the June-19/20 leaf model existed; `bake-look` only repacks them, so the slab/Grove showed pre-leaf-work geometry. Regenerating from compositions (`generate-salon`) is the fix — this is the "Re-publish" step that's hard to reach in the Salon UI. ✅ **Closed 2026-06-25 (`15682e55`):** the fold landed — `/grove/bake` runs `generate-salon` over all species as step 1, so a bake regenerates from source and cannot ship stale leaves.
...
2. **The allowed-zone got bypassed** (my regression, one commit later): re-baking via the bare CLI passed no `zoneShapePath`, so `makeZoneTester` was never built and trees placed in the carriageway and on rooftops. The tell was `0 forbidden-surface drops`. The CLI now resolves through `treeBakeInputsForMap` like the other two callers, and `bakeTrees` refuses to run unzoned when a baked `shape.json` exists. LS: **4,992 placed**, 892 forbidden drops, 1,405 surveyed trees nudged onto legal ground.

**⛔ The blank-capture failure mode — twice in one day, in both bakers.** A capture that renders nothing produces a *valid transparent PNG*: the manifest looks complete, the bake reports success, and the species silently stops existing. Measured: `platanus_acerifolia` has **all 18 hero layers blank** (442 placements — LS's 2nd most common tree — invisible on the pan while its ground shadow still prints) and 2 of 3 overhead bands blank; `linden_american` has a blank overhead canopy band. Root cause for the platanus: a species with **no `barkDetailBySpecies` record** renders with `uBarkTileScale (0,0)`, so every layer samples an empty atlas region — and only *Salon-composed* species get that record, which the merged London plane never was. Both bakers now **refuse to ship a species with any blank layer**. Meter: `scratch/overhead-band-coverage.mjs`. ▶ **The platanus needs a Salon composition** (it also has broken geometry — `skeleton-1` spans −72 m to +58 m).

**Also:** `linden_american` was publishing + shipping 3 byte-identical trees per scene (shading-group chassis → a node per sub-mesh → same-named siblings read as variants, kept by name); fixed at source and hardened downstream. `tree-anchors.json` is index-parallel to `trees.json` and the runtime discards it wholesale on a length mismatch — **re-bake anchors whenever placements change** (I broke this and re-baked; 745 → 4,995).

**▶ OPEN, from Jacob's eye on the render:** ground shadows print where no tree renders (the platanus blanks are a large part, but confirm the rest); Browse and Hero disagree on density when they must match; and the geometry-anchor rule wants **foreground**, not height — the brief's own build log flagged the "∩-foreground pan-distance axis" as still to come, and that is now the live question.
