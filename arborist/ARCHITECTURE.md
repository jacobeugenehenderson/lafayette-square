...
## Tree-render reality at LS (2026-06-25; UPDATED 2026-07-10) — mesh ship; geometry by ROLE-at-bake; overhead-Browse SNAPSHOT impostor LANDED (Hero-cross still parked)
...
- `InstancedTrees.jsx` chooses geometry via a `lodForRole(inst)` hook keyed on the baked role — **not** a runtime tier driver. The old runtime altitude-swap **`GeoTierDriver` is RETIRED** (`InstancedTrees.jsx` comment, ~line 542). The mesh path returns `'lod1'`; **the OVERHEAD-Browse snapshot un-parked this seam 2026-07-10 (`a4458f4a`)** — `captureTreeOverhead`/`OverheadTrees` bake a **3-slice snapshot** per unique tree and the runtime **selects it by camera height** (zoom-out → snapshot). The Street/Hero mesh path is unchanged; the **Hero (octahedral) impostor + `cull → drop` stay the parked future.**
