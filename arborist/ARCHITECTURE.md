...
**Salon preset cameras (Brief 13 Vantage, 2026-05-23).** Verifying tier work requires viewing the chassis at the camera distance the tier was designed for. `SpecimenViewport.jsx` carries three preset framings (`presetFraming(preset, treeH)`): Overhead (`{distance: max(150, treeH*6), height: treeH+50, lookAtY: 0}`), Hero (`studioFraming`), Street (`{distance: max(5, treeH*0.2), height: 1.8, lookAtY: 1.8}`). The camera-state ref contract gained an optional `lookAtY` so Overhead can tilt the camera down at the chassis base; absent or `=height` keeps the legacy horizontal lookAt. Crane `H_MAX` bumped 60→120 and dolly `D_MAX` 150→300 to accommodate Overhead on mature 40m chassis. Presets are NOT coupled to `uBarkShaderTier` — operator authors camera and tier independently (Brief 11 wires the tier from cartograph SHOT). Generic studio-inspection distances; not cartograph SHOT imports — Salon stays helper-internal per `project_kit_helpers_pattern`.
