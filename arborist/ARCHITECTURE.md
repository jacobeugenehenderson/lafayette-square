...
> ### ⛔⛔ SEATING: ONE ANCHOR PER TREE, LIFTED IN THE SHADER
> Every tier (mesh, hero card, overhead disc) lifts by `aGroundRaw × uExag` in the vertex shader —
> `aGroundRaw` is `tree-anchors.json`'s raw height of the DRAWN ground under the tree (`groundSampler`),
> and the exag is a per-shot tween a baked lift cannot follow. The slab's `y: 0` is a sentinel, never a
> height (`elevation.js#treeGroundRaw`). The anchor file names its placements (`placementKey`) and its
...
Both are **RTT captures of the real tree** — rendered from the actual `lod1` geometry through the shared atlas material — so colour, season and character match the mesh trees by construction. Both share one shader program, the wind uniforms, and the DoF/fog/grade the depth gauges apply. Both are **browser-GPU authored** on Bake→Slab and **carried** by `bake-look.js` — neither can be regenerated by the CLI bake, so a merge or a re-pour must preserve those manifest keys, never re-derive them.

> ### ⭐ THE CARDS NOW LIGHT FROM THE SCENE'S KEY — `?litCards=1`, DEFAULT OFF (2026-09-03)
> ⛔ **This line used to claim "full optical parity … weather relight" and that was ASPIRATION, not as-built.** The cards are `MeshBasicMaterial` by design — they cannot join three's light rig — and their entire light model was two scalars, `uAmbient + uSun · ao`: a **global dimmer**. A card was dimmed by the weather; it was never lit by anything, so the mesh tree beside it and the card disagreed about where the light was. The intent had been settled since 2026-06-25 (*"normal map, not just color — must-have for LS"*, priority 2 of 6, `_archive/BATON-tree-render-next-RETIRED-2026-07-22.md`) and simply never got built.
> - ⭐⭐ **ONE KEY, ONE PUBLISHER.** `CelestialBodies` publishes `keyDirection` off the very `primary.lightPosition` its `<directionalLight>` is built from → `useSkyState` → `OverheadLightDriver` → `uKeyDir`. ⛔ **Not `sunDirection`** — after dusk the key is the sun→moon blend, and a card lighting from the sun at midnight lights from below the horizon while every mesh lights from the moon. ⛔ Never recompute it at the consumer: two derivations of one physical fact is what produced both the tree-height bug and the capture-frame bug.
> - **The normal is SYNTHETIC, and deliberately so** — a hemisphere bulge off the card's own UV, built in the card's *current billboarded* frame. ⭐ **That is why it needs no azimuth rotation.** A baked normal page is stored in the frame of the azimuth it was shot at, so it must be rotated by (card yaw − `azimuthDeg`) — and getting that wrong lights the canopy from the wrong side **while looking plausible**, which is worse than looking broken. Costs no new pages, no capture change, no `CAPTURE_FORMAT` bump. A captured normal remains the upgrade if the eye asks for one.
> - **AO gates the directional term rather than replacing it.** Occlusion is geometry — a leaf deep in the crown stays dark wherever the sun is — so the baked AO page scales what the key delivers. It is the one channel that needed no capture work.
...
> - **The trunk/ground joint came back with it, on the bark card.** `injectFoliageSway`'s ground-colour blend was mesh-only, so with meshes at zero every card met the ground as a hard edge. The hero stack's `kind:'bark'` layer has its own material — the card's equivalent of the mesh's per-vertex `vBark` gate. Every hero card's frame starts at the ground (`heroCardFrame`), so the blend always has a trunk base to act on.
...
- **The pool is transcoded (2026-08-29)** — KTX2/ETC1S, encoded by the pour's last step. LS: **71 MB → 20 MB shipped, 1,121 MB → 282 MB resident**, which is where the phone budget was actually being spent. The anchor species still load their GLBs. ⛔ **The master atlas is excluded on purpose** — its coverage-preserving mip chain cannot survive an encoder that generates its own mips; compressing it would silently reintroduce the far-canopy erosion that chain exists to fix. Remaining levers, in order: the atlas (needs explicit coverage-preserving mips fed to the encoder), azimuth count, persist resolution.
...
⭐ **THE LIKELY GROVE (Jacob, 2026-09-25):** *"The point of the Arborist is to create likely groves."* Species come from the town's census, else **USDA FIA by county, ranked by live stems** (`scripts/15-fia-tree-mix.mjs`). ⚠️ Open: a sample-size rule (huron: 1 plot/species).
- **The system builds; the operator swaps**: `recommend-plates.mjs` ranks each plate **S > G > T** with a plain-word reason; `--build` composes the top pick as `auto`, never over an authored one. ⛔ Filenames never reach the operator (▶ `claims-no-filename-reaches-a-plate-label`).
- **A town's lists hold only its own grove** (*"The LS species should come out of ALL LISTS FOREVER EVERYWHERE"*): a Look's roster ⊆ its `tree-species-map.json`; a town with no routing holds nothing, loudly — never a borrowed grove (▶ `claims-a-look-holds-only-its-towns-grove`).
- **The Grove is only finished trees** (*"The grove is ONLY completed, ready, baked, ready to place trees. There is no such thing as placeholders."*): a species without a baked tree does not appear in the Grove. Its gap is loud where it is built — ranked and red in the Salon roster.
- **Undesirable = a sourced fact, the operator's call** (not built): the UNION of every list we can source per state (USDA PLANTS `PlantInvasiveStatus`/`PlantNoxiousStatus` + state lists, e.g. MA MDAR), provenance per list; uncovered reads **ABSENT**, never "desirable". The Grove draws it as its tree on a **red circle**; hiding it from the map is the operator's choice.
...
- **SelecTree** — queried by name; takes the exact species record or a cultivar of the SAME species, else **skips** and emits `_taxon_mismatch`, as USDA does (fixed 2026-09-25 — the old any-other-species fallback fed `Sorbus decora` into `sorbus_americana`; diary `arborist/_archive/ARCHITECTURE-selectree-fallback-2026-09-26.md`). ▶ `node checks/claims-no-dossier-rests-on-a-wrong-species.mjs`
