# Arborist BACKLOG — archived May-2026 brief arcs (Salon + Procedural v1.5)

> **Archived 2026-06-25** out of `arborist/BACKLOG.md` per the CLAUDE.md "Keep it trimmed" doctrine. These are the **May-2026 brief-numbered arcs** — the Salon composition arc (Briefs 0–31, 2026-05-21) and the Procedural v1.5 / skeleton-first redo (Phases A–G, 2026-05-15) + the Procedural-fallback-v1 / cross-helper / SpeedTree tails. **Almost entirely resolved (`[x]`) or folded (`[~]`).** The current front-end direction is the **Forest Builder kit-matcher** (BACKLOG.md top callout) which supersedes the Salon arc; the live open items that survived the cut were distilled into BACKLOG.md `§ Live open work carried forward`. Nothing is lost — the full brief-by-brief record (what shipped, the alignment-check findings, the schema deltas) lives here for deep dives. Companion narrative diary: `_archive/NOTES-2026-05-diary.md`.
...
- [x] **Brief 6.3 — Connected-mesh leaf decimation (Lever 6)** — shipped 2026-05-23 by baby Gnomon. `arborist/decimate-tree.mjs#decimateLeafPrimitivesConnectedMesh` runs BEFORE Lever 4 emitLod (alongside Lever 5) via `MeshoptSimplifier.simplifyWithAttributes` at `errorTolerance=0.02, targetRatio=0.20, uvWeight=1.0`; fires only on prims with `extras.atlasKind === 'leaf'` AND `vcount > 100K` AND `maxVertUse > 1` (connected-mesh — disjoint from Spindle's Lever 3 card-based `=== 1` path). `arborist/decimation-defaults.json` gains a `leafDecimation` sub-tree (+ immutable `decimation-defaults.defaults.json` backstop). Idempotency flag `extras.gnomonDecimatedLeaf`. **Numbers:** Linden leaf prim 416,888 → 83,377 tris (**−80.0%**, err 6.18e-3); UV sub-region preserved byte-tight (0/173,658 post-decim verts escape the leaf atlas sub-region into the bark region). Determinism sha1-verified; idempotent. **True firing set is broad** — 29 connected-mesh leaf prims across ~14 species (birch/red maple/aspen/white oak/tall pine/juniper/peach/white fir + Linden), not Linden alone. **Deployed-path verified** (Salon → publish-glb chain, `stampAtlasKind` → L3 → L5 → L6 → emitLod). **`positionWeight` scope note:** meshoptimizer's `simplifyWithAttributes` exposes no separate position-stream weight (position is the quadric base; `attribute_weights` weight only UV) — the defaults field is documentation, not passed to the simplifier. **Doesn't clear AC #2 (lod2 bracket) — and corrects the brief's premise:** with the leaf floor removed, Linden's post-Lever-6 leaf prim collapses freely to ~100 tris at lod2; the remaining lod2 floor (~43K emitLod / 57.7K attribute-aware) is **the bark prim** (Lever 5 output), whose UV-locked topology floor is error-budget-independent (flat from error 0.02→0.4). lod2 clearance is now a Lever-5/Lever-4/bracket problem, not a leaf problem. **AC #3 premise wrong:** the active Robinia chassis (`robinia_pseudoacacia_a_tree_*`) ships **card-based** leaves (maxVertUse=1) — Lever 6 correctly skips them; Robinia leaf reduction comes from Spindle's Lever 3. **Closes Adze's open item:** Robinia deployed-path republish shows **Lever 3 fires** (leaf 324K → 217K) — first deployed-path proof of Spindle's lever. **Operator-eye Salon visual sign-off (AC #4) still pending** — data-level UV preservation proven, but the Vantage Ground/Overhead silhouette read is the operator's to confirm. See `scratch/brief-6.3-leaf-decimation-survey-gnomon.md` + NOTES.md 2026-05-23 entry.

- [ ] **Brief 6.3-followup — Connected-mesh bark lod2 floor (mobile gate closer).** Surfaced by Gnomon (Brief 6.3). With Levers 3/5/6 all firing, Linden-class chassis still miss lod2 bracket (300-1500) because the **bark** prim floors at ~57.7K tris under attribute-aware (UV-locked) `simplifyWithAttributes` — the floor is error-budget-independent (verified flat from error 0.02→0.4), so it is NOT a tuning problem. `simplifySloppy` crushes the same bark to ~1 tri but obliterates UV/atlas addressing (unacceptable per [[feedback_atlas_subregion_uv_recovery]]). Three candidate paths, operator's call: (a) **bracket retune** — accept Linden-class lod2 floors at ~40-58K, bump `qualityBracket.lod2.maxTris` for connected-mesh species (defeats the hundreds-of-placements mobile budget); (b) **impostor/billboard lod2** — render the farthest LoD as a camera-facing card pair (true mobile win, but new runtime + bake path); (c) **sloppy-with-vertex-lock** — `simplifySloppy` with the atlas-sub-region boundary verts locked, trading interior topology for UV-region survival (needs prototyping — meshopt sloppy doesn't take a lock mask today). Lever 5 could also drop `targetRatio` 0.15 → ~0.08 to approach bark's 57.7K floor pre-emitLod, but that alone won't bracket. **This is the actual mobile-critical gate** the operator flagged; 6.3 cleared the leaf half of it. Cold dispatch candidate; parallel-safe.
...
- [x] **Brief 19 — Persist + bake authored chassis transform** — shipped 2026-05-25 by baby Quartz. The Salon gnomon gizmo (rotateY / posOffset / scale / tiltX-Z) was inspection-only — `SlotCard` held it in local `useState`, reset on every slot/chassis switch, never written to the composition (`SalonWorkstage.jsx:497` literally `// Inspection-only`). Operator's orientation corrections (e.g. the "Y-up trunk 90° X" flip for Z-up kit chassis) evaporated on adopt/publish. Now: gizmo `onChange` → `onParams({transform})` → `setSlotParams` → `composition.transform` in `compositions.json` (`{posOffset, rotation:[tiltX,rotationY,tiltZ], scale}`); hydrates back into the gizmo on slot/chassis switch (was reset-to-identity → persistence was write-only); and **bakes into the published GLB** in `generate-salon.js#bakeAuthoredTransform` (publish path only — the live preview leaves it to the gizmo, so no double-transform). **Alignment-check finding (load-bearing):** the brief assumed a plain `T·R·S`-about-group-origin bake, but `SpecimenViewport.jsx`'s `<Skeleton>` composes `R · S · T_posOffset · T_autocenter` — it auto-centers the dominant-trunk base (`computeDominantTrunk`) to the bullseye BEFORE the authored transform, so rotation/scale pivot about the trunk base, not the origin. The bake replicates this EXACTLY via the **conjugated** transform `v' = T_autocenter⁻¹ · R · S · T_posOffset · T_autocenter · v` (operator-chosen "in-place" Option 1) — matches the viewport AND keeps identity authoring byte-identical. `eulerXYZToMat4` verified byte-exact vs three.js `makeRotationFromEuler('XYZ')`; identity → byte-identical POSITION on a real 52K-vert chassis; 90°X flip swaps Y↔Z extents; determinism + writeBinary round-trip confirmed. **Deformer premise restored:** Brief 3A's merge-time pivot now reads corrected geometry (its prerequisite). **Doesn't fix:** the separate global off-origin lean / wind-frame bug — surfaced as Brief 20 below. See `NOTES.md` 2026-05-25 + `scratch/brief-19-persist-bake-authored-chassis-transform.md`.

- [x] **Brief 20 — Recenter all chassis to origin at the SOURCE** — **SHIPPED 2026-05-25 (Sextant).** `survey-deleaf.js` recenters every chassis to its dominant-trunk base at origin at creation time: single-tree path (`processGlb`) drops Brief 0's byte-identity carve-out, bakes the node hierarchy (`bakeSceneTransforms`) then `translate(−dominantTrunkBase)`; bundle path (`processBundleGlb`) swaps bbox-center for the same dominant-trunk recenter. **Ported** the algorithm a third time (`computeDominantTrunkBase`) rather than shared-lift — the drift hazard goes quiet from recentering alone (all three finders agree at ~origin by construction), no precedent for `src/` importing executable `arborist/` JS, and the shared-lift pairs better with the deferred delete-the-quiet-finders cleanup. Single-pass (the grid estimator has a period-2 limit cycle on boundary-straddle trunks → bit-zero unreachable; residual 185/241 <0.1m, 231 <0.5m — "≈ identity" per AC #3). Verified AC #1 (231/241 <0.5m, all minY=0), #2 (dominant-trunk not bbox), #3 (finders quiet), #6 (determinism), #7 (curation 37/38 name-keyed), #8 (bake-look exit 0), #9 (gizmo tools intact), #10 (no off-center dependency). **Surfaced:** slab propagation needs per-species re-publish FIRST (Brief 14 decouple); single-mesh FOREST chassis captured via `surveyTrunkClusters` → **Brief 22** (forest-split); mis-scale → Brief 21; `broadleaf_rt3` now no-wood (stale curation + roster entry); stale-orphan accretion in `_chassis/` fixed via clean regen (consider a `--clean` flag). Deferred cleanup (delete quiet finders, collapse Brief 19 conjugation, shared-lift) NOT done — post-verification. See `NOTES.md` 2026-05-25. Original entry below.

- [ ] **Brief 20 — Recenter all chassis to origin at the SOURCE (root fix for the trunk-centering duplication + off-origin lean/wind-frame).** Surfaced by Quartz (Brief 19 alignment-check, 2026-05-25); **reframed 2026-05-25 from "recenter at publish" to "recenter at source"** after the operator asked why two trunk-centerers exist. Root cause: the 141 single-tree chassis (Whittle, Brief 0) ship **off-origin** (byte-identity constraint kept their baked node translations, e.g. `TREE_00` at `T=[3.9,0,−3.8]`; broadleaf_rt3 base Y≈−1.3, trunk centroid X≈−3.0/Z≈−0.7), so everything downstream re-derives the center at runtime: `SpecimenViewport.jsx#computeDominantTrunk` (viewport auto-center) + `generate-salon.js#computeAutoCenterPivot` (Brief 19 bake conjugation, a port of it) — two implementations of one algorithm, plus a third `bbox-center` recenter survey-deleaf already runs on *bundle* chassis only. **The fix:** recenter ALL chassis to origin at chassis-creation time (`survey-deleaf.js`), using the **dominant-trunk method** (move/share that computation into survey-deleaf as the canonical centering; replaces its bbox-center for bundles too), dropping the byte-identity carve-out for the 141. **What it fixes:** (1) the long-standing **lean/decenter issue** for the 141 (`survey-deleaf.js:1162`); (2) **Sough's wind-frame assumption** (Brief 9a `injectFoliageSway` assumes `X≈Z≈0, base≈0`) becomes true; (3) **3A's deformer pivot** becomes trivially-origin; (4) **neutralizes the trunk-finder drift risk** — on already-centered chassis both downstream finders return ~origin, so they agree by construction (auto-center = identity, Brief 19's conjugation = identity-wrapping-identity); the KEEP-IN-SYNC hazard goes quiet without needing a shared-lift or deletion.

  **KEEP ALL GIZMO TOOLS (operator decision 2026-05-25).** Recentering only fixes *position* — it does NOT replace the authoring tools, and we keep all of them until the automation is proven in action: **tilt (X/Z)** stays — recentering doesn't de-lean (the dominant-trunk method finds the base *point*, not the axis tilt; a 3°-leaning chassis recentered still leans 3°, and de-leaning is the fuzzy auto-orient we are NOT building); **scale** stays — can't auto-size (a multipack contains arbitrary assets, no reliable canonical-height signal); **rotateY / posOffset / oubliette** stay (posOffset/oubliette become no-op-ish on centered chassis but remain the escape hatch). The trunk-finders go *quiet* (return ~origin), they are NOT deleted in this brief.

  **Deferred to a post-verification cleanup pass (NOT this brief):** deleting the now-quiet `computeDominantTrunk` / `computeAutoCenterPivot`, collapsing Brief 19's conjugation to plain `R·S·T`-about-origin, and retiring any centering tools proven genuinely redundant. Only after recentering is observed working.

  **Blast radius (why it's deliberate, operator-sign-off-required):** regenerate the chassis library (`node arborist/survey-deleaf.js`, drops byte-identity for the 141 — chassis are gitignored/regenerable, curation file is **name-keyed so it survives** [verified]) + full-roster re-bake; existing authored `posOffset` values stay valid (visual result unchanged — the recenter does at source what the viewport auto-center did at display). **Watchout:** confirm nothing downstream depends on the raw off-center chassis frame (none found — off-centeredness is a pure kit-extraction artifact). **Operator-decision-required before dispatch.** Cold dispatch after Brief 19 ships (shipped — `50f88ef`).

- [~] **Brief 21 — Normalize extreme-mis-scale chassis — FOLDED INTO BRIEF 23 (2026-05-25).** The forests Brief 23 decomposes ARE the main mis-scale offenders (acer_saccharum/burnt_tree at 900–1700m), and decomposed sub-trees are useless until rescaled — so the rescale rides Brief 23's same `survey-deleaf` regen pass (one regen, one re-publish+bake). Kept as its own entry for the spec detail below; the *escape* is to un-fold it (separate fast-follow) if Brief 23's decomposer mesh-segmentation turns out hard. Original spec: Surfaced by the Brief 20 baby's chassis sweep + the operator's "the empties are just 300' tall" diagnosis (2026-05-25). **Root cause:** a pre-existing *extraction* bug — some chassis import at **900–1700m heights** (unit mismatch, almost certainly mm/m or ×100). They render **"empty" in the Salon preview** not because they lack geometry, but because `SpecimenViewport`'s framing distance scales with `treeH` (`studioFraming`: distance ∝ treeH/2) while Brief 13 clamped the camera at `D_MAX=300 / H_MAX=120` — so a 56–140× oversized tree can't be pulled back far enough to frame; the camera is trapped looking at the lower trunk slice → reads as blank. **Distinct from two other "empty" populations:** (a) *structurally* empty (zero-mesh, caught by `_quarantine-empty.js`'s `listMeshes().length>0` check — legitimately empty); (b) *normal* scale variation (12 vs 18m — can't auto-size per operator, stays the manual scale gizmo). The extreme outliers (>~100m) ARE auto-detectable as unit-bugs — **this is the one place auto-scale is justified.** **The fix:** detect chassis with height ≫ plausible-max (e.g. >100m) at the source (`survey-deleaf.js`) → rescale to real-world height, OR flag for operator review. Un-hides the roster chunk that's been invisible-and-assumed-empty. **Distinct from Brief 20:** recenter ≠ rescale — a recentered 900m tree is still 900m and still unframeable; Brief 20 does NOT fix this. Both operate at chassis-source; compose but **don't bundle** (Brief 20 is recenter-only, scoped). **Affected (Brief 20 sweep):** `acer_saccharum_a/c`, `gray_poplar`, `honey_locust`, + others at 900–1700m. **Related-but-separate:** `acer_saccharum_a/c` are multi-tree *forests* (several trees per file), a **curation/classification** issue (reject from the single-tree chassis pool via the Brief 1.5b curation surface, or reclassify) — NOT a scale fix; handle separately. **Open question for the brief:** rescale-to-canonical-height (needs a target — species-map canonical height? normalize bbox to plausible range?) vs flag-for-operator. Operator-requested 2026-05-25. Cold dispatch; composes with Brief 20 at survey-deleaf.

- [x] **Brief 23 — Chassis-library fortifications: `--clean` guard + extreme-mis-scale rescale + forest detection/suppression + curation reconcile. SUBSUMES Brief 21; sub-phases merged-forest segmentation → Brief 23a.** — **SHIPPED 2026-05-25 (Mistral).** Forensic (AC #1) overturned the regression premise: all 42 multi-root splits present, the 17 deleted "orphans" all correctly stale → AC #2 + #6 satisfied by forensic, no restoration needed. Mesh-segmentation alignment-check found the forests are the HARD merged case (one bark prim = 17–22 trunks), so the per-tree split is sub-phased → **Brief 23a** (`scratch/brief-23a-merged-forest-segmentation.md`). Shipped 4 durable fortifications (`survey-deleaf.js` + `generate-salon.js`/`serve.js` consumer): **(1) `--clean`** idempotent-complete prune — deletes ONLY stale orphans whose `meta.source.species` is a still-present, still-walked vendor dir; REFUSES procedural/LiDAR/absent-source/no-meta (verified: deletes stale-vendor, refuses all 3 protected classes — the guard the `procedural_broadleaf` loss taught us). **(2) Rescale** >100m → ÷10^round(log10(H/15)) post-recenter; bundle-level divisor (tallest root) applied uniformly so candicands' 56/87/110m siblings stay consistent (÷10, no inversion); honey_locust/gray_poplar/poplar_fall ÷100 → 6–12.5m. **(3) Forest detection** — `surveyTrunkClusters` per-WOOD-primitive, gate **≥3 trunks-in-one-mesh** (2-trunk multistem/forked-base singles deliberately kept); writes `state/_chassis-forests.json` worklist (7) + suppresses them from the Salon catalog while keeping the 58 separable splits. **(4) Curation** — removed dead `broadleaf_rt3_a`; `--clean` surfaces curation orphans read-only (never writes curation). Determinism byte-identical (AC #4 + #8); 241 chassis. See `NOTES.md` 2026-05-25. Original re-scope entry below. **RE-SCOPED 2026-05-25 after Mistral's forensic (AC #1) overturned the "regression" premise — there was NO regression.** Coordinator-Boz miscounted (Riven decomposed **42**, not 18) and grepped the **vendor** names (`gleditsia`/`populus`) when the splits are stored under **common-name** labels (`honey_locust_*`, `poplar_fall_*`, `gray_poplar_*`, `london_plane_*`) — all 42 multi-root splits present; the 17 deleted "orphans" were all correctly stale/superseded/renamed/cross-producer (zero legit splits lost; AC #2 + #6 satisfied by forensic). **The real, net-new work:** (1) **single-mesh spatial forest decomposer** (the folded Brief 22) — the *actual* group shots `acer_saccharum` (17 trunk clusters), `acer_saccharum_lowpoly`, `burnt_tree`, which Riven's root-detector structurally can't see and no process ever split. **First alignment check: classify the forest mesh — separable primitives (split = assign primitives to clusters, easy) vs one merged mesh with intermixed verts (per-triangle nearest-trunk assignment, the crown-interleave problem, hard) — BEFORE building the segmentation; sub-phase if hard.** (2) **`--clean` guard** — `--clean` doesn't exist yet (Sextant did a manual `rm` that deleted `procedural_broadleaf`, a cross-producer artifact); build it idempotent-complete per `[[clean-regen-must-be-idempotent-complete]]` — refuse to delete any chassis the regen can't re-emit, especially cross-producer (procedural/LiDAR). (3) **Folded Brief 21 (extreme-mis-scale rescale):** the forests are 900–1700m → decomposed sub-trees are still 900m + unframeable in Salon (the framing-clamp "empty" issue: `studioFraming` distance ∝ treeH vs `D_MAX=300/H_MAX=120`). One rescale pass over ALL chassis (detect >100m → unit-fix ÷-scale) after decomposition catches forest-splits + any non-forest mis-scales. Distinct from *normal* scale variation (12 vs 18m — stays the manual gizmo) and from *structurally* empty (zero-mesh, `_quarantine-empty.js`). (4) **Curation reconcile** — the `robinia→black_locust` rename dangle + stale entries. **Fold escape:** if the decomposer's mesh-segmentation turns out hard (alignment check #1), keep Brief 21's rescale a separate fast-follow to avoid overloading one baby. **Composes:** Boz's `7eef6c5` already filters procedural + acer-LiDAR from the Salon catalog. **Recommend before 3A** (deformer wants a correct, split, centered, framable library). Baby: Mistral (warm — did the forensic). Full brief: `scratch/brief-23-restore-forest-decomposition.md` (note: brief's "restoration" framing superseded by this re-scope). Cold/warm dispatch, solo.

- [~] **Brief 22 — Spatial forest decomposition — FOLDED INTO BRIEF 23 (2026-05-25, operator-confirmed merge).** It's the single-mesh-decomposer *component* of Brief 23's restoration, not a standalone — the forests turned out to be a Brief-20 `--clean` regression to restore, not a new-feature exclusion, and the spatial decomposer is the tool for the single-mesh half. ~~Original spec retained for reference below.~~ Surfaced by the Brief 20 sweep + operator's correction (2026-05-25): multi-tree *forests* in the single-tree chassis pool (`acer_saccharum_a/c` + others) are **Brief 1.5c's domain, not curation.** **Why 1.5c (Riven) missed them:** its bundle detector is **root-based** — `isBundleDoc` → `findGeometryRoots`, `roots.length <= 1 → return false` (`survey-deleaf.js:286`). Riven explicitly found (report line 1144) that most multi-tree sources are *single-root* (one merged mesh + baked positional offset), which root-counting can't catch. There are two single-root failure modes: (1) **the lean** — one tree, baked offset → **Brief 20 (recenter) fixes it**; (2) **the forest** — *multiple trees merged into one mesh*, one root → sails into the single-tree pool, nothing addresses it. Root-counting structurally cannot detect a forest-in-one-mesh — you need **spatial clustering.** **The fix:** extend bundle detection from "count roots" to "count trunk clusters." **Reuse Brief 20's dominant-trunk binning** — it already finds multiple distinct trunk clusters in the bottom-slab on these (that's literally why it "picks one stem far from bbox-center"). Multiple clusters = multiple trees → split the mesh per-cluster (assign each tree's geometry to its nearest trunk cluster), emit per-tree chassis (`<species>_<letter>_<treeN>`, like Riven's multi-root decomposition), recenter each (Brief 20's centering). **Input:** the Brief 20 baby's sweep logs which chassis show multiple clusters (asked to capture name + cluster count) — ready-made worklist. **Composes:** builds on Brief 20 (binning + recenter); lands after it. **Interacts with Brief 21:** `acer_saccharum_a/c` are BOTH forests AND mis-scaled (900-1700m) — decompose (22) then each per-tree chassis still needs rescale (21), or rescale then decompose; either order, they compose. **Old whole-forest chassis** becomes dead weight (like Riven's `candicands` whole-bundle) — suppress via the Brief 1.5b curation surface, additive. **Open question for the brief:** clustering method (k-means with k=cluster-count vs connected-components on trunk-slab density) + how to assign canopy geometry to the right trunk when crowns interleave. Operator-requested 2026-05-25. Cold dispatch after Brief 20.
...
- [ ] **Brief 24 — Grove coverage view (roster-anchored "have vs need").** Surfaced 2026-05-25 (operator). Turns the static `arborist/ROSTER-COVERAGE.md` into a live Grove panel: a `Gallery (by model) ↔ Coverage (by roster need)` toggle. Coverage view = roster-anchored, one row per canonicalized park species (sorted by placement count), tagged 🟢literal / 🟡composite / 🔴gap with the covering composition. Read-only: a new `GET` endpoint in `serve.js` computes the roster↔chassis↔composition join (the same join `ROSTER-COVERAGE.md` does by hand); `Grove.jsx` renders it. Ships a hand-seeded operator-editable `arborist/roster-name-canon.json` (merges the messy duplicate roster names — curation, not auto-guess). Provenance derived on the fly (literal = chassis `source.species` matches target), NOT persisted/baked — that's the **Brief 25** follow-up (touches `generate-salon.js` → manifest → slab). Read-only + disjoint from the runtime/shader path; cold dispatch (3A has shipped, so no concurrency concern — just stay off `generate-salon.js`). Full brief: `scratch/brief-24-grove-coverage-view.md`. ~200–350 LOC. **Surfaces the routing diagnostic** (each park-name's current `park_species_map.json` entry + stale/dangling flags) so it doubles as the map-refresh worktable; DISPLAYS the map, never writes it.

- [ ] **Map-refresh — `src/data/park_species_map.json` (operator-curated companion to Brief 24, NOT a baby brief).** The stale (2026-04-29, pre-chassis-library) routing `bake-trees.js#pickVariant` uses to fan the 89 messy park-names onto the ~25–30 published library species. Hand-curation ("no auto-guess gets it right" per the file's own `_doc`). Informed by Brief 24's coverage join. **Seeding the roster is NOT "done" until the map routes every park-name onto a seeded species (or a deliberate filler)** — else the Grove bake substitutes the wrong tree / none. No new bake code; `unifyAtlases` already gangs the leaf+bark cards (sha1-deduped) so a ~25–30-species roster stays atlas-compact.

- [ ] **Brief 25 — Persist + bake provenance (literal vs composite).** Follow-up to Brief 24. Promotes the on-the-fly `literal`/`composite` derivation into authored state: `composition.provenance` + `targetSpecies` (auto-suggested literal when chassis `source.species` matches the species, operator-overridable), surfaced as a Salon/Grove badge and **baked into `manifest.json`** so it reaches the slab and Brief 24's coverage can read it from data instead of deriving. Touches `generate-salon.js` + manifest emission + `SalonWorkstage.jsx` → **serializes after Brief 3A + Brief 26** (shared load-bearing files). Not yet a full brief. Surfaced 2026-05-25 (operator).

- [ ] **Brief 26 (REWRITTEN 2026-05-25) — Roster-driven Salon: navigate by roster species → compose-or-unavailable.** Operator reframed the authoring model: the unit flips from LIBRARY species → **ROSTER species**. Top nav = the roster-species list (from `GET /coverage`), **replacing the SPECIES dropdown**. Click a roster species → inside view: a candidate-chassis pulldown with a **recommended ↔ show-all toggle** + full composition (chassis + bark + leaves + height) + a **not-available** option (renders no tree). The composition is library-level → reusable across Looks (existing publish model). **Keying spine SETTLED — canonical-id-per-roster-species:** each roster species has one canonical library-species-id (Sugar Maple→`acer_saccharum`, Weeping Willow→`salix_babylonica`, Green Ash→`fraxinus_pennsylvanica` even pre-chassis); composing writes the composition under that canonical id + sets `park_species_map[rosterName]=canonical`; the **chassis is free geometry** (any chassis serves any canonical id — so the old `speciesOverride`/reassignment is dissolved). Each roster species is first-class (Pin Oak ≠ Willow Oak even on shared chassis; atlas dedups geometry). No procedural-filler fallback. Sub-phase 26a (navigator + candidate compute + routing wiring, reusing Brief 24's join) before 26b (inside authoring UI). **Supersedes** the prior "Look-only toggle + reassignment" draft AND the Brief 24 clickable→Salon addendum. Touches `SalonWorkstage.jsx` + `serve.js` + `generate-salon.js` + `park_species_map.json` → **serialize after Cadastre's Brief 24 work commits**; 3A (`8010c8e`) already committed. Full brief: `scratch/brief-26-salon-picker-look-toggle-reassign.md`. ~500–700 LOC.

- [x] **Brief 27 — Grove population from Salon compositions (retire the rating gate)** — **SHIPPED 2026-05-25 (Scion, `cfb1e5d`).** Forensic resized the brief (per `[[feedback_data_flow_split_first_check]]`): the load-bearing Grove visibility gate is `serve.js#/grove` (`if (quality < 2) continue` over raw manifests), NOT `Grove.jsx#filterQuality` (optional UI filter) nor `build-index` (runtime path). And `quality>=2` IS the de-facto "published" signal — `patchManifestForSalon` stamps `qualityOverride:4` on every Salon-published variant; raw vendor chassis stay 0. So **"published ⟹ visible" was already wired** (ACs 1/2/4 functionally done by construction); real work was AC 3 = retire the *UI rating-gate + stale copy*, not a data rewire. Shipped: removed `Grove.jsx#filterQuality` Fill/Mid/Hero control, rewrote empty-state/scope copy to compose-and-republish, "All Rated"→"All Published", reframed the `serve.js#/grove` gate comment (published-not-rated). `bake-trees#pickVariant` reads resolved `v.quality` (not `qualityOverride`) — EditorCard rating ladder STAYS (only authoring surface for the still-consumed field); only its visibility-gate role retired. **Operator click-through still pending** (no harness): Re-publish a composition → confirm it lands In-Look; "All Published" scope adds/removes membership. **Deferred follow-ups (named, not built):** (a) explicit `v.published` marker decoupling the gate from the rating *value* (a hand-downrated composition would stay visible) — generate-salon schema addition; (b) the `qualityOverride` quality-lottery is provably vestigial under one-composition-per-roster-species → retire from `build-index`/`bake-trees` (separate brief). See `NOTES.md` 2026-05-25.

- [ ] **Brief 28 — Streamline asset intake ("+ Add Model / + Add Leaves"). 🧊 DEFERRED — pull forward when asset-adding is a routine cadence** (operator 2026-05-25, "not for right now"). Mix-and-match is central to the library (chassis × bark × leaves → many species reads), so smooth *part*-intake is foundational. Today: bark ≈ drop source + auto-extract at bake; **chassis** = vendor-stock-path + `survey-deleaf.js` (de-leaf/classify/recenter/rescale/meta — **whole-library**, no targeted mode); **leaves** = `compose-leaf-packs.mjs` (Color+Opacity → RGBA + meta). End-state: "+ Add Model" in the Salon chassis picker + "+ Add Leaves" in the leaves section → upload → targeted ingest → appears in the picker. **Real prerequisite** (the button is the thin layer): a single-asset ingest mode in `survey-deleaf` + a `serve.js` upload endpoint. For now: route procured assets (§2 gap-fill — Green Ash etc.) through Boz/CLI. Stub: `scratch/brief-28-streamline-asset-intake.md`.

- [ ] **Brief 27-followups (deferred, named by Scion):** (a) `v.published` schema marker (decouple Grove gate from rating value); (b) retire the vestigial `qualityOverride` quality-lottery from `build-index`/`bake-trees`. Low urgency; bundle when a bake-trees/build-index brief next opens.

- [x] **Brief 31 — Grove: click-to-select → fixed editor panel** — **SHIPPED 2026-05-25 (Cleat, `a66a2b3`, Grove.jsx only).** Fixed the pre-existing chasing-hover-card bug: the per-tile editor was a camera-anchored drei `<Html>` rendered only while `hovered`, so it chased the projection, was ambiguous which tile, and `onPointerOut → hovered=null` unmounted the card before the click landed (dead Remove button). Now **click-to-select → a fixed right-rail `GroveEditorPanel`**; `EditorCard` controls (rating/category/notes/Look-toggle/canary) reused verbatim — only render-location + driver changed (selection, not hover). Click vs orbit-drag split via R3F `e.delta>5`; deselect via ground-click + `onPointerMissed` (delta-guarded) + ×. No data/store/runtime/slab changes. First code brief under the Agent write/commit-boundaries doctrine — Cleat committed Grove.jsx only + handed Boz the FEATURES/NOTES deltas (applied). **Operator click-through pending.** **Surfaced (NOT Cleat's):** pre-existing dangling symlink `public/photos/lafayette-square/other → ../../../photos-wikimedia/other` (target missing) breaks vite build's publicDir-copy step — module transform clean (1041/1041); operator restores `photos-wikimedia/other` to unblock production builds. See `NOTES.md` 2026-05-25 + `scratch/brief-31-grove-click-select-panel.md`.

- [ ] **Build blocker (env, operator) — restore `photos-wikimedia/other`.** `public/photos/lafayette-square/other` is a dangling symlink → `../../../photos-wikimedia/other` (target missing); `npm run build` transforms all modules cleanly then fails in vite's publicDir copy. Pre-existing; surfaced by Cleat (Brief 31). Blocks any production build until the target is restored. Not a code fix.

- [x] **Brief 3A — Per-instance deformer engine (lean + twist + wander)** — **SHIPPED 2026-05-25 (Cant).** The procedural-fill diversity engine + artistic-integrity capstone: one chassis → ~100 visually-distinct reads via per-instance vertex-shader displacement, authored as per-species ranges. Three rigid ops on `transformed` BEFORE Sough's wind sway (wind oscillates around the deformed rest pose): **lean** (tilt toward a per-instance azimuth, angle grows base→top), **twist** (about local Y, grows base→top), **wander** (sinusoidal-in-height XZ centerline drift). All pivot about the trunk base = **origin** (Brief 20 — no per-chassis pivot, the premise this brief waited on). Per-instance hash seeded from the instance anchor (`instanceMatrix[3].xz`, `modelMatrix[3]` non-instanced fallback) → deterministic, one signature per tree; fresh vertex-side channels `dh5-dh8`. **Normals exact without inverse-transpose** (rigid rotations): the lean∘twist `mat3` rotates `objectNormal` — but since three.js bakes the normal in `<beginnormal_vertex>` (BEFORE `<begin_vertex>`), the matrix is built + the normal rotated THERE, then matrix + wander reused on `transformed` in `<begin_vertex>` (cross-chunk `main()`-scope locals — the inspection-point finding the brief anticipated). Range uniforms default `(0,0)` → identity, bit-exact regression-safe; sibling `applyDeformerUniforms` (not a widened `applyBarkUniforms`); single compiled program (uniform+attribute branch, no `customProgramCacheKey`). `aTreeHeightNorm` computed at runtime-merge (chassis-wide Y scan, shared LS↔preview — reintroduces Cork's retired scan, sound all along) → GLB/atlas bytes untouched. `manifest.json#deformer.range` (generate-salon) → `trees-atlas.json#deformerBySpecies` (bake-look pass-through) → per-draw uniforms. **Salon Deformer panel** (3 ops × lo/hi `DraftSlider` + "Re-roll preview sample"); fires in preview (`[[feedback_salon_preview_is_authoring_surface]]`) so the operator authors + sees the spread live (preview = one hash sample/re-roll; multi-instance deferred). Files: `treeAtlasMaterial.js` (+deformer chunk + `aTreeHeightNorm` block + `applyDeformerUniforms`), `InstancedTrees.jsx` (Y-scan + per-draw bind), `SpecimenViewport.jsx` (preview parity + bind), `SalonWorkstage.jsx` (panel), `generate-salon.js` + `bake-look.js` (manifest pass-through). vite build clean (1040 modules). **Operator-eye visual sign-off in Salon is the load-bearing AC — pending Jacob.** **3B** (designed hero slots + PlaceCard binding) + **3C** (canopy asymmetry + branch jitter — need inverse-transpose) remain open below. See `NOTES.md` 2026-05-25 + `scratch/brief-3a-per-instance-deformer-engine.md`.

- [ ] **Brief 3 (3B + 3C remainder) — Per-instance deformer rig** (architecture pivot 2026-05-23; **3A shipped 2026-05-25 by Cant — see above**). Brief 1's `composition.deformer` schema (reserved-but-empty) gets filled. **Mission**: ~100 visually-distinct instances per species from a single chassis via per-instance vertex-shader displacement. **Remaining: 3B** = designed slots (operator dials specific deformer values for landmark/hero placements; PlaceCard binds a placement to a slot ID — note `designedSlots` in the schema sketch below) + the per-Look override; **3C** = scale-along-axis, canopy asymmetry (height-attenuated radial off-center bias), branch jitter (per-vertex hash micro-displacement) — these need inverse-transpose normal work, which is precisely why 3A scoped to rigid rotations only. Replaces today's variant-lottery diversity model. Two categories per species: **designed slots** (operator dials specific deformer values in Salon for landmark / hero placements; PlaceCard binds a placement to a slot ID) and **procedural fill** (authored deformer *ranges*; per-instance hash samples within range). Schema sketch:
  ```json
  "composition": {
    "deformer": {
      "range": {
...
- [x] **Brief 14.1 — Decouple Procedural Re-publish from auto-bake** — **SHIPPED 2026-05-25 (Corbel).** Applied Lintel's Brief 14 decouple to the procedural twin he surfaced. Removed the `if (lookName) { bakeLook(lookName).catch(...) }` fire-and-forget block from `/procedural/:species/publish` in `arborist/serve.js` (was ~1452-1458) + rewrote the endpoint's leading comment to the authoring-only contract. Endpoint now: shell out to `generate-procedural.js --species <id>` (which still runs `syncLookRoster('lafayette-square', …)` in its `main()` — roster sync is **in-script, not bake-coupled**, so nothing orphaned) → `rebuildIndex()` → return. No slab touch. The `?look=` param stays accepted + echoed (vestigial), mirroring Lintel. **Verification (structural, no polluting publish run):** confirmed `generate-procedural.js` + `build-index.js` write only to `public/trees/<species>/` + `index.json` — neither touches `public/baked/<look>/trees-atlas.json` or the master PNG, so AC1 (slab unchanged) holds by construction; the removed `bakeLook` was the sole slab path. `bakeLook` stays wired at `/atlas/bake` (Grove, AC2) + the full-bake path. **Legacy chrome touched (surfaced):** `ProceduralWorkstage.jsx` is `?legacy=procedural`-only post-18A, but its Re-publish copy actively claimed "auto-bake per-Look atlas" (now false + misleading), so reworded footer tooltip + inline hint to the two-gesture model (mirrors Lintel's SalonWorkstage copy), removed the now-orphaned `activeLookId` selector, and fixed the stale `republishProceduralSpecies` store comment + the file's header jsdoc. `generate-procedural.js`'s end-of-run console hint already prints `node arborist/bake-look.js --look lafayette-square` — consistent with the decouple, left as-is.
