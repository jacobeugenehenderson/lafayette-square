...
- **The roster needs a full re-seed** from the corrected library: compose the ~25–30 intended library species in the Salon → re-publish → re-seed `design.json#/trees` → refresh `park_species_map.json` (§4 step 2) → Grove bake. **The Salon's roster navigator (Brief 26, Cadastre 2026-05-25) is now the authoring surface for this** — it lists every roster species (this coverage join) and you compose each under its canonical id (a slug, e.g. `oak_pin`) or mark it not-available. (This supersedes Brief 24's reverted clickable→Salon addendum.)
- Not a Grove code bug — the Grove faithfully renders the roster; the roster *data* + the `acer_saccharum` forest publish are stale.

## §5 — Changelog
- **2026-05-25 (Brief 26, Cadastre)** — The **Salon roster navigator** is now the authoring surface (§6 re-seed workflow). Navigate by roster species → compose under a canonical-id slug (`oak_pin`) with a recommended/show-all chassis picker + full composition, or mark not-available. Routing written to `park_species_map.json` on publish. The `/coverage` join moved into the shared `arborist/roster-coverage.js`.
