...
> ## 🔄 STATUS (2026-06-25 EOD) — THIS IS THE LIVE FORWARD-POINTER, but read this first
> **Trees currently ship ALL-MESH and the impostor arc is PARKED.** Since this baton was written, the operator reverted the aggressive impostor tiering: `bake-trees.js#PROM_THRESHOLD` is back to **0** (every visible tree → full lod1 mesh; the "Phase A LANDED `PROM_THRESHOLD` 0.06" + "Phase B opaque-shell" deltas below are **reverted/parked**, not current). The captured-impostor / opaque-shell / render-to-texture code crashed the prod build (dev-only) and is **kept out of the production renderer** — `bake-impostors.js`, `impostorGeometry.js`, `ImpostorSpecies` sit dormant on disk, `lodForRole` always returns `'lod1'`. **The GPU gauge that drove "geometry must go" is NOT a perf signal** (count-vs-fake-budget; reads red with no trees) — gate perf on device frame-ms + the operator's eye on the pan. So: **the VISION below (impostors baked from real lod0, octahedral Hero + cake-layer Browse, season-aware, riding optical parity) is the live forward plan; the "what LANDED" Phase-A/B status lines are stale.** When you pick this up, re-confirm the current `PROM_THRESHOLD` + `lodForRole` state in code before treating any "LANDED" line as true. Settled doctrine (role-at-bake, optical parity, retire GeoTierDriver) is unchanged + correct. See `arborist/NOTES.md` 2026-06-25 EOD + `arborist/ARCHITECTURE.md "Tree-render reality at LS"`.

> **Pick-up handhold from the 2026-06-24 EOD session (Boz).** Route first: `ORIENTATION.md` → `arborist/README.md §⭐ START HERE`. (The frame for this arc — role-at-bake, optical parity — is the "🧭 CONFIRMED DOCTRINE" callout, archived with `arborist/_archive/HANDOFF-visibility-cull-lods-2026-06-23.md`; the doctrine also lives in `arborist/ARCHITECTURE.md`.) Branch `curb-offset-draw`.
