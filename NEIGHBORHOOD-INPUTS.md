...
>
> Deep home: **`cartograph/INTAKE.md`** (+ `HANDOFF-neighborhood-perimeter-builder.md`). This graduates §5.2's "curated-centerline file-hack → first-class gesture" for the *extent* layer. ⚠️ **UNDER CONCERTED-LOGIC REDESIGN (2026-07-10):** the built **name-derive / 4-cardinal** model + the illegible-label aerial are being replaced — **legible perfect-fidelity fetch → visual boundary-segment selection (~20 real segments, not cardinal) → auto-radius (slider=margin)**. The old "browse framing" item folds into the redesign. Live brief: **`HANDOFF-extent-tool-concerted-logic.md`**; this §11 will be rewritten to match on land.
