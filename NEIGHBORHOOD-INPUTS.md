...
**Doctrine (revised 2026-07-02 — see §1.1):** the map has an **automated backbone** (geometry, widths, class, land-use, heights — all fetchable) *plus* a **legitimate authoring layer** for the idiosyncratic features a historical neighborhood carries that street data simply doesn't hold. The extent is a **center + radius circle** (`neighborhood_boundary.json`) — the circle IS the boundary and the slab disc; the operator carves strays out of it with **exclusion loops** (§5.2), the soft stencil fading the rim (§9).
...
### 5.2 Building membership — the excluder pen + per-building override (BUILT; excluder 2026-07-14)
Membership is authored on the Extent tool. **⭐ The circle IS the boundary** — every building inside the auto-fit disc is IN **by default** (`project_neighborhood_disc` holds: the circle is both the extent and the slab disc/fade). The operator carves the exceptions in two layers:
- **Exclusion loops (the bezier pen, `BezierPen.jsx`)** — draw editable loops around the strays to force them OUT (a commercial block, the far side of an arterial, forest/mountain past the real edge). Membership = **inside-circle − exclusion loops**; a building is out if its centroid falls in any loop. The loops are the **first-class persisted artifact** (`nb.exclusions`, lon/lat anchors + handles), reloaded editable across sessions.
...
> The named-street edges above **describe where the neighborhood sits** — but they are *not* how membership is authored. A hood is a **center+radius circle with strays carved out**: the circle is the boundary + slab disc + soft stencil (`SLAB-CONTRACT §2.1`), and the operator draws **exclusion loops** for the idiosyncratic edge (the "possibly Skinker" / "some include DeMun" fuzz), §5.2. **Onboarding step:** frame → fetch → adjust the radius → loop the strays → Bake (`INTAKE §0.5`); geography + weather (§3) light up from the committed center for free.
