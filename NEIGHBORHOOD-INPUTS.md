...
▶ `node checks/claims-reference-credits.mjs --markdown` — paste-ready credits + counts.
...
**⭐ The one invariant — the building id is the join key.** The slab carries each building's *spatial identity* (its `id`); the content layer carries **no geometry**, only `id → record`. A click resolves `raycast → building id` (slab); content resolves `id → display record`. **Every content record MUST key on the exact building id the slab assigns** — HPDM `msbf-*`, LS `bldg-*`. Collect ids that match the slab, or nothing joins. (`slab-render-vs-content-boundary`: "slab owns identity, content owns display.")
...
- **UX** — the Extent tool's top-down footprint overlay (`ExtentApp`): the pen toggle draws exclusion loops (excluded buildings read as faint ghosts); "Edit buildings" enters per-building curation (click a ghost to re-activate, an inside building to hide). Pan stays live (`Space` suspends the pen).
- **Applied in the PIPELINE** (`pipeline.js`, the membership filter → `map.json`; ⛔ the boundary **clip** that used to carry it was excised 2026-09-05, `ec7dd3f4` — membership survived, the geometry trim did not) so it's the **single source** both the 2D Designer (reads `map.json`) and the bake inherit. Commit re-projects the exclusion loops **into the re-centered frame** and writes them to `neighborhood_boundary.json.exclusions`. Both `neighborhood_boundary.json` and `building-overrides.json` are **git-tracked** (`.gitignore` excepts them) so the curation is reproducible source, not just baked into the slab.
- **⚠️ Membership is a POUR/BAKE-time filter, not live.** The Extent builder shows the loops + overrides live, but the 2D `map.json` and 3D slab only reflect them after a **re-pour + re-bake** (committed hoods take the light **rescope** path). *(A hood committed the OLD inclusion-polygon way still carries `nb.polygon`; the excluder re-bake drops it — `24323ab2` — so it clips to circle − exclusions, not the stale ring. Operator steps: `cartograph/OPERATIONS.md §7`.)*

Per §1.1 this makes idiosyncratic membership a first-class deterministic gesture — no routing, no inference (the exclusion-loop + per-building correction model — `EXTENT-DESIGN §3.2`). (Tree / path / lamp add-remove — the rest of §5.2 — remain to build; this slice is **buildings**.)
...
> The named-street edges above **are exactly how membership is authored** (2026-07-20 — this note previously said the opposite). The operator clicks those streets and the ring is rebuilt from the junctions they share; where a hood's edge is genuinely fuzzy (the "possibly Skinker" / "some include DeMun" case) exclusion loops and per-building toggles correct the margin, §5.2. The circle remains the slab disc + soft stencil (`SLAB-CONTRACT §2.1`) — it renders, it doesn't decide. **Onboarding step:** search the place → fetch → author the boundary → correct the margin → Bake (`INTAKE §0.5`); geography + weather (§3) light up from the committed center for free.
...
**Four subject classes** *(the third added 2026-07-10, the fourth 2026-07-22):*
- **The neighborhood itself** ⭐NEW — `{kind:'centroid'}`, the local frame's origin at mid-building height (the origin **is** the boundary centroid, `config.js wgs84ToLocal`). Picked in the Survey Hero dropdown; the answer for any installation that owns no set-piece and no privileged building, and the resolver's fallback when a Look has no arch. No placement knobs — the subject is the hood, there is nothing to author.
...
- **Decorative prop** — an object *not in the neighborhood*, inserted + scaled/rotated/positioned purely for framing. **The arch is this** — a stage prop authored via `Hero Distance/Scale/Rotation/Y-Offset` sliders (default `~[996,0,-332] @1.3`), **not** its real geography (`src/components/GatewayArch.jsx`). Today it's a bespoke procedural component (NPS catenary + simple custom shaders + uplights + ground disc); every *future* prop is a **brought GLB**. **A Look INSTALLS a set-piece by carrying its block** (`design.arch` → baked `scene.arch`) — presence is the gate, the same rule every look-keyed consumer already runs on; `GatewayArch` self-gates on it so no mount site names a Look, and `bake-scene` no longer stamps arch defaults into every scene.json. *(2026-07-22 — the stamped-everywhere defaults had forced a look-NAME gate, which dropped the arch from Hi-Pointe–DeMun, a St. Louis hood that owns it.)*
...
**Setup flow, all on one aerial canvas (realized as the Extent tool — `INTAKE §0.5`):**
1. **Frame + fetch** — compose the hood by eye on the global aerial, fetch only what's framed (frame-then-fetch). ⛔ Street names come FROM this fetch, so nothing about the boundary can be authored before it lands.
2. **⭐ Author the inclusion POLYGON** — a ring of **named street runs** (inherit a gazetteer ring · click the bounding streets in any order · draw). **This is what decides membership.** Corners fall out of the walk; they are never placed.
3. **Circle** — the auto-fit center + radius is the **slab disc that RENDERS** (the ground plane / the world's horizon), with the stencil fading the margin (`SLAB-CONTRACT §2.1`); the operator adjusts the radius via an in-scene handle. It **hides** what's outside, it does not decide. ⛔ *This step used to read "the circle IS the boundary and the slab clip" — retracted 2026-07-20.*
4. **Exclude + override** — draw **exclusion loops** around the strays; per-building `activate`/`hide` for the margin (§5.2). Deterministic, no routing. **Corrections on top of the polygon, not the mechanism.**

**Connective navigation.** The intake front lives in the Cartograph server, but the operator crosses the local topology — **`:5173` web (main app) · `:3333` cartograph · `:3334` arborist · `:3335` meteorologist** (`docs/agents/PIP.md`). Intake needs nav links wiring Cartograph ↔ main app ↔ the authoring helpers (cartograph + arborist most of all) so the operator moves between extent/acquisition, tree authoring, and the lit app without losing neighborhood context.
...
> ✅ **BUILT — this §11 spec is realized as the Extent tool** (`2026-07-14`; boundary model corrected `2026-07-20`). `src/cartograph/ExtentApp.jsx` + `BezierPen.jsx`: the operator authors an **inclusion polygon** from the fetched street names (the membership decision), the auto-fit **circle** is the render disc, and **exclusion loops** (frame-independent lon/lat, editable across sessions) + per-building overrides are the margin corrections; **Bake** ⛔ **does NOT re-center `geography.json` and does NOT run `reproject-raw`** (changed 2026-07-23): the frame origin stays frozen at the fetch center, the hood centre is written off-origin as `discCenter`, only the timezone re-derives. It then re-derives the skeleton, writes `neighborhood_boundary.json` (circle + `exclusions`), and pours + bakes. ⛔ **The re-center guard was removed in the same change** (`INTAKE §0.5`) — the frozen origin is the protection. The "living boundary" (§11 above) holds: a committed hood re-scopes/re-excludes and re-bakes freely.
>
> Deep home: **`cartograph/INTAKE.md §0.5`** (the as-built flow) · design of record **`EXTENT-DESIGN.md`**. ⛔ **This paragraph used to say "the earlier models are dead… the street-selection code is dormant/flagged for excision."** That was wrong on both counts and is RETRACTED (2026-07-20): **street selection is the live mechanism and is mounted**, not dead weight. What *was* genuinely excised is the order-dependent 4-cardinal corner resolver (`computeExtentCorners` + `/extent-corners` + `fetchExtentCorners`, `55df128a`), superseded by the order-independent shared-junction walk. ⚠️ After any re-center, verify parcel↔building alignment (the 2026-07-08 ~800 m address-desync gate).
