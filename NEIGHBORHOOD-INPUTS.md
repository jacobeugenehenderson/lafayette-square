...
**Setup flow, all on one aerial canvas (realized as the excluder Extent tool — `INTAKE §0.5`):**
1. **Frame + fetch** — compose the hood by eye on the global aerial, fetch only what's framed (frame-then-fetch).
2. **Circle** — the auto-fit center + radius IS the boundary and the slab clip, the stencil fading the margin (`SLAB-CONTRACT §2.1`); the operator adjusts the radius via an in-scene handle.
3. **Exclude + override** — draw **exclusion loops** around the strays; per-building `activate`/`hide` for the margin (§5.2). Deterministic, no routing.
...
> ✅ **BUILT + settled (2026-07-14) — this §11 spec is realized as the Extent tool's excluder pen.** `src/cartograph/ExtentApp.jsx` + `BezierPen.jsx`: the auto-fit **circle** is the boundary/centroid/disc; the operator draws **exclusion loops** (frame-independent lon/lat, editable across sessions) + per-building overrides; **Bake** re-centers `geography.json` to the boundary centroid, reprojects **all** frame-dependent raw (OSM + msbf + admin_boundaries + assessor parcels) via `reproject-raw.js`, re-derives the skeleton, writes `neighborhood_boundary.json` (circle + `exclusions`), then pours + bakes. A **re-center guard** protects committed hoods (`INTAKE §0.5`). The "living boundary" (§11 above) holds: a committed hood re-scopes/re-excludes and re-bakes freely.
>
> Deep home: **`cartograph/INTAKE.md §0.5`** (the flow + the "why excluder, not a traced perimeter"). **The earlier models are dead** — name-derive/4-cardinal, the concerted-logic segment-selection redesign, and the inclusion pen all preceded the excluder; their briefs are archived (`cartograph/_archive/handoffs/`) and the street-selection code is dormant/flagged for excision (`cartograph/NOTES.md`). ⚠️ After any re-center, verify parcel↔building alignment (the 2026-07-08 ~800 m address-desync gate).
