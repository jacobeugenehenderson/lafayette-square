...
- **Applied in the PIPELINE** (`pipeline.js`, the boundary clip → `map.json`) so it's the **single source** both the 2D Designer (reads `map.json`) and the bake inherit — the curation persists through every downstream step. Commit re-resolves + persists the boundary polygon **in the re-centered frame** (`neighborhood_boundary.json.polygon`) so the bake-frame buildings align.
