...
The thorn / bulge-bow / "dead-end triangle" cluster is **one root: the skeleton is NOT simplified to polygon-ready**, so OSM saw-tooth noise propagates into Survey, and the inward band offsets wander/cross/pinch. **Two layers** (verified against Chord's sub-step-1 output, 2026-06-04):

1. **The frame is over-noded** — `skeleton.js` carries far more vertices than the geometry warrants (Benton's teardrop loop = **29 pts** where ~5–13 suffice; a clean block edge should be ~2). The pre-Chord `simplify` was a weak *local* filter (perp<0.2m AND turn<2°) — useless on curves. **The 29/5m was skeleton-origin (NOT derive-added).**
2. **`tileGround.smoothChain` (`:614`) re-smooths ×4** (count-based, ~+3 pts/seg) → 29 → ~113. Not idempotent (`smoothCenterline.js:63`): re-sampling an already-dense line injects ripple → offset crossings. **This is the remaining amplifier (sub-step 2).**

> **⚠️ CORRECTED 2026-06-04 — `derive.js:1146` is NOT a third layer.** An earlier draft claimed derive re-densifies curves (→5m) and "undoes the simplify." **Verified false:** Chord's RDP **survived derive** — Benton 29→**13 in `ribbons.json`** (not re-inflated), and **64 of 66 curvy chains coarsened** (only 2 stragglers at ~5m: Park Place, motorway_link 16). derive's curve-densify is **essentially inert post-RDP** — do NOT neutralize it as a blocker; touch the 2 stragglers only if they specifically thorn. (Lesson: verify the mechanism before asserting it — `feedback_read_canon_before_forensics`.)

**Verified (read-only):** skeleton over-noding → render ×4 = Benton 29 (frame) → ~113 (render). **Chord's sub-step-1 RDP reached `ribbons.json` cleanly** (Benton 13, benton-place-0 → 2 pts; 64/66 curvy chains coarsened) — the simplified frame **survives to the render.**
...
**2. Make smoothing a single authority — the remaining amplifier (`tileGround.smoothChain:614`).** Replace the count-based ×4 (`samplesPerSeg ≈ round(t*8)`) with **density by arc-length** — robust to any input density, so it neither re-densifies an already-clean frame nor under-samples a sparse one. Preserve wide-ribbon kink-freeness (grade-sep, `tileGround:967`). → re-bake → verify even density + kink-free wide ribbons.
   - *Note: `derive.js:1146` curve-densify is **NOT** in scope — verified inert post-RDP (64/66 curvy chains survived; see the CORRECTED note above). Only the 2 stragglers (Park Place, motorway_link 16) might want a look, and only if they thorn.*
