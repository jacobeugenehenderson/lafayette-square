...
Cross-refs: [`cartograph/ARCHITECTURE.md`](cartograph/ARCHITECTURE.md) (producer), [`ls/ARCHITECTURE.md`](ls/ARCHITECTURE.md) §2 (consumer), [`ls/reference/INVENTORY-DATA.md`](ls/reference/INVENTORY-DATA.md) §A (mount status).
...
A two-channel "ground-contact" map produced by `bake-ground-ao.js`, sampled **once** at world-XZ by the grass + ground (`FadeMesh`) shaders. Meta in `ground.json#/poolmap` `{ image, size, min:[x,z], span:[w,h], scale }`; bbox = union of lamp + tree extents + margin; `size` is DERIVED per town from that span at `FX_TARGET_M_PER_TEXEL` (`bake-ground-ao.js`, `ef0ac55a`), capped — the bake logs the resulting m/texel and warns when the cap binds.
...
```jsonc
{ "version": 2, "look": "<look>", "count": <n>,
  "terrain": { "key": "<terrainIdentity>", "baseElev": <m> },
  "lamps": [ { "x": <m>, "z": <m>, "park": <bool>, "groundRaw": <m> }, … ] }
```

| Field | Meaning |
|---|---|
| `lamps[].x`, `.z` | World-metres position. |
| `lamps[].park` | Park-style vs street-style lamp (model + glow). |
| `lamps[].groundRaw` | Raw (pre-exag) height of the DRAWN ground under the lamp (`groundSampler`); the runtime lifts by `groundRaw × uExag`. Absent ⇒ the smooth terrain field. |
| `terrain` | The heightfield the anchors were sampled from (`terrainIdentity`). The consumer refuses the anchors when it differs from the slab's `terrain.bin`. ▶ `node checks/claims-anchors-know-their-terrain.mjs` |
