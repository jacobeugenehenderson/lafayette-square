...
**Bounded per-mouth disc** (centered on the fillet midpoint = the asymmetric pavement center; Lafayette 10.56/7.90) makes multi-spur tiles independent — no whole-ring reshape, so **iA is byte-identical** and the 98 m multi-spur blow-up never occurs. 39 mouths / 20 tiles built; Benton/Waverly/Saint-Vincent loops excluded (gated to `deadEndSkels`, deg-1 tips); `kennett-place`/`park-avenue-1` customs untouched (segOrd byte-stable). **OPEN:** (a) the slide emits the same small LU ramp-wedge fragments a normal corner does → **+5 `junction-band` detector flags** at the mouths (Albion/Whittemore/Nicholson/Simpson/Dolman/Hickory) — an eye-call (clean taper vs sliver), NOT a curb regression (iA untouched); (b) **8 single-fillet fallback mouths** left unwrapped (one corner only — mackay/hickory/s-18th/dolman/s-13th/henrietta/SV/carroll); (c) **strip-swappable dead-ends** (the companion below) — NOT yet rebuilt (was in a reverted attempt).
