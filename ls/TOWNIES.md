...
> ⚠️ **Two other actions auto-grant townie** (via `grantTownieStatus`, which backfills synthetic check-ins): **claiming a listing** (`postClaim` → `grantTownieStatus`, `apps-script/Code.js:636`) and **verifying a residence** (all four paths, including co-resident approval — `Code.js:1830`; see [`RESIDENTS.md`](RESIDENTS.md)). So a **guardian/keyholder is a townie immediately on claim**, and a **verified resident is a townie immediately** — both can review *other* places right away, with no real check-ins. **Residence → townie is intended** (Jacob, 2026-06-30 — "co-residents should become townies"; the whole cascade grants it). The **claim → townie** grant is likewise as-built and embraced (the auto-grant model); no longer flagged as a loophole to tighten.
