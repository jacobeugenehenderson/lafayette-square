...
The same `/checkin/<id>` route serves both **Check-in** and **Resident** QRs; `CheckinPage.jsx` branches on whether the target is residential. **Residential places have no Guardian QR** — residents are invited via the Resident QR; there's no staff-claim override (`PlaceCard.jsx:2418`, `(!isResidential || isHouse)`).
...
The Guardian QR embeds an 8-char hex `claim_secret` directly in its URL, so **the printed card is the credential** — there's no separate auth token. Generated lazily and persisted on the Listings sheet (`getClaimSecret`, `apps-script/Code.js`):
...
- **Who may fetch it:** a **full guardian** of the listing (`isFullGuardianOf`) or an **admin** (`getClaimSecretAdmin`). For a non-existent listing, **admin only** — it auto-provisions a minimal `residential/houses` row so a card can be printed (`Code.js`).
- **Validation on claim:** `postClaim` (`Code.js`) rejects a mismatched secret; first claimant becomes guardian, the rest keyholders. See [`GUARDIANS.md`](GUARDIANS.md).
- **Stripped from public reads:** `claim_secret` (with `guardian_hash`/`guardian_token`) is deleted from the `getListings` response (`Code.js`).
...
The Link QR (`/link/<token>`) syncs one person's identity (handle + avatar + vignette) across devices. The token is a **6-char alphanumeric, 5-minute TTL** held in Apps Script cache (`createLinkToken`, `Code.js`):
...
Flow: `createLinkToken` → `postClaimLinkToken` (`Code.js`) → `checkLinkToken` poll (`Code.js`). UI lives in the account popover (`App.jsx:100–128`, `LinkPage.jsx`). A linked device also unlocks the **linked-device** residence auto-verify path — see [`RESIDENTS.md`](RESIDENTS.md).
...
```
- **Backend:** the **Designs** sheet `['biz_id', 'design_json', 'updated_at']` (`Code.js`), keyed `biz_id = <listingId>-<Type>` (e.g. `lmk-010-Guardian`). `getDesign(bizId)` (`Code.js`) / `saveDesign(body)` (`Code.js`, auto-creates the sheet). API wrapper `getQrDesign(listingId, type)` (`api.js:225`).
...
| Check-in / resident flow | `src/pages/CheckinPage.jsx` | — |
| Claim flow | `src/pages/ClaimPage.jsx` | — |
