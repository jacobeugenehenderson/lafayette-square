...
1. **Townie auto-grant on claim & residence** (`Code.js:636` / `:1766` / co-resident `:1830`) — as-built, claiming a listing *or* verifying residence (any path) makes you a townie immediately. *(Documented across the cluster as of 2026-06-30.)* **Design-intent settled 2026-06-30 (Jacob): intended, embraced** — not a loophole.
...
3. **✅ RESOLVED 2026-06-30 — keyholder mislabel dissolved by business-voice replies** (not a new medallion, per Jacob). Replies render as the business (logo + name, glass `visitor` fallback) in composer + public render (`PlaceCard.jsx:1196`/`:1336`), so no per-individual role badge shows on a reply. Lobby stays resident-anonymous by design; townie/admin still badge-less (cosmetic, not pursued). See §c.
...
- **§c (medallions):** `RoleBadge.jsx:21–40` defines exactly **three** themes (`visitor` glass, `resident` `hsl(30,65%,62%)` burnt-orange, `guardian` `hsl(170,55%,60%)` teal) — no townie/keyholder/admin. *Original finding:* the reply composer hardcoded a "Guardian" medallion (`:1196`), mislabeling keyholders. **✅ Resolved 2026-06-30** — replies now render in the business voice (logo + name, glass `visitor` fallback), so no per-individual medallion is shown; keyholder gets no badge by design. The lobby composer (`:2567`, `isGuardian ? 'guardian' : 'resident'`) is **correct-by-design** (a resident-only anonymous space). Townie/admin remain badge-less (cosmetic, not pursued).
...
5. **✅ RESOLVED 2026-06-30 — business-voice replies** (see §c ✅ note). The keyholder/guardian reply mislabel is gone: replies present as the business (logo + name, glass `visitor` fallback), so no per-individual medallion. Townie/admin remain badge-less by choice (cosmetic). (= §d hole #3.)
