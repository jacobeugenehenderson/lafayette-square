...
1. **Guardian — stand up a real card first.** If the business is one of the ~18 dining listings, done. If not, **create a proper `dining` listing** (admin) — do *not* rely on `getClaimSecretAdmin` auto-provision, which stands up a `residential/houses` stub (`Code.js`) with the wrong category and an empty card.
...
2. **Owner scans** → `ClaimPage.jsx` fires `claim(listingId, secret)` → `postClaim` (`Code.js`) validates the secret.
3. **First claimant = guardian** (full perms: `menu,events,replies,photos,hours`); any subsequent claimant on the same listing = **keyholder** (`Code.js`).
4. **Identity gate** — before the success screen, the flow requires a **handle** (mandatory, `ClaimPage.jsx:71` `needsHandle` blocks everything) then offers an **avatar** (skippable). See Massage §a.
5. **Instant townie bonus** — `postClaim` calls `grantTownieStatus` (`Code.js`), backfilling synthetic check-ins so the new guardian is **immediately a townie too** (Massage §b). The success screen reads "You're a recognized local in the neighborhood" (`ClaimPage.jsx:136`).
...
2. **Fetch the claim secret** — `getClaimSecret` (`Code.js`) generates an 8-char hex lazily (`Utilities.getUuid.split('-')[0]`) and persists it to the `claim_secret` column. **Who may fetch:** a full guardian of the listing **or an admin** (`getClaimSecretAdmin`). Jacob is admin → he can pull it for any listing (`QR-CODES.md §2`).
...
1. **Neighbor scans the building's resident-invite QR** (encodes `/checkin/<buildingId>` — the same route as a check-in QR; `CheckinPage.jsx` branches on residential,, `QR-CODES.md §1`).
2. `CheckinPage` detects a residential target (`landmark.category === 'residential'` or a bare building id) → calls `claimResidence(dh, buildingId, true)` with **`auto_verify` on**.
3. **Backend `postClaimResidence`** (`Code.js`) sees `auto_verify=true` → sets `status = verified`, `verified_by = qr-invite`, `expires_at = now + 1 year` (`Code.js`) — **verified on scan** (`RESIDENTS.md §2`).
4. **Identity gate** — as with every role, posting in the Lobby needs a **handle** (§a).
5. **Instant townie bonus** — verifying a residence calls `grantTownieStatus` (`Code.js`), so the neighbor is **immediately a townie too** — reviews/bulletin/comments/DMs all unlock in the same scan (`RESIDENTS.md §1`, `TOWNIES.md §1`).
6. **Done** — the building's **Lobby** tab appears on its place card (`LobbyTab`, `PlaceCard.jsx:2492`; gated `isResidentHere`): a residents-only board (text + photos, posts anonymous), a co-resident view, and the verified-resident count. Access is re-checked **server-side** on every lobby read/write (`RESIDENTS.md §5`) — the tab is convenience, the server is the boundary.
...
| **Linked device** | `linked-device` | Another device sharing the caller's `@handle` is already verified for this building (`Code.js`) | ✅ if they've linked a device (`DEVICE-LINK.md`) |
| **Co-resident** | (verified) | A pending claim is later approved by an already-verified resident (`postVerifyResident`, `Code.js`) | ⏳ later — this is the *referral* loop (R.3) |
...
> ✅ **Confirmed in code (2026-06-30) — the decisive question.** A **plain verified resident** (not just guardian/admin) *does* see the resident-invite QR. The invite QR is rendered inside the **Lobby tab** (`QrTab` under "Resident QR", `PlaceCard.jsx:4013`), and the Lobby tab is gated `isResidentHere || isAdmin`, where `isResidentHere` = *verified resident of this building*. For a residential building `QrTab` builds `/checkin/<buildingId>` (`qrId = buildingId`). **So any verified resident can pull the building's invite QR from their own Lobby and hand/post it — the building fills itself; Jacob is not in the loop for each invite.**
>
...
- **Co-resident verify** — a verified resident **approves a neighbor's pending claim** (`postVerifyResident`, `Code.js`), surfaced via the Lobby's **co-resident view** (`RESIDENTS.md §4`). Brings on neighbors who claimed but landed `pending`.
...
1. Scan a place's **check-in QR** (`/checkin/<id>`, `App.jsx:613`) → `postCheckin` logs a row (`Code.js`).
2. Set up **identity** (handle + emoji/vignette, `IDENTITY.md`) — required to *post*, not to check in.
3. Repeat across **3 distinct calendar days within a rolling 14-day window** (`LOCAL_THRESHOLD=3`, `LOCAL_WINDOW_DAYS=14`, `Code.js`) → townie, computed server-side from check-in history (`TOWNIES.md §1`). The page shows "You're a verified local! Society Pages unlocked." at the threshold (`CheckinPage.jsx:343`).
...
- **Claiming a listing** (Guardian/keyholder, `postClaim` → `Code.js`).
- **Verifying a residence** (Resident, `Code.js`).
...
**What the handle step demands today** (`ClaimPage.jsx:152` `HandleStep`, backend `postSetHandle` `Code.js`):
- A handle matching `^[a-zA-Z0-9_]{3,20}$`, **unique** (case-insensitive, `check-handle` / `Code.js`).
- "Continue" is disabled until the typed handle is *both* valid *and* confirmed available (`ClaimPage.jsx:221`).
- **It is mandatory and has no skip** — `needsHandle` blocks the success screen. (Only the *avatar* step has "Skip for now,".)
...
**Backend reality:** `postSetHandle` accepts `{ device_hash, handle, avatar, vignette }` only — there is **no `starter_name` concept**, and `vignette` is validated `^v[0-7]$` (`Code.js`). So nothing server-side needs to change for suggestions.
...
|---|---|---|
| **Guardian / keyholder** (claimed a listing) | **Townie** ✅ | `postClaim` → `grantTownieStatus` (`Code.js`) |
| **Resident** (verified residence) | **Townie** ✅ | `postClaimResidence` → `grantTownieStatus` (`Code.js`) |
| **Guardian** of place X | **Townie for reviewing place Y?** ✅ (because claim auto-granted townie) | `postReview` is a pure townie gate (`Code.js`); it does **not** special-case guardians |
...
> ✅ **Resolved 2026-06-30 (was a spec-inaccuracy to-do).** The auto-grant is now documented across the cluster: [`GUARDIANS.md §2`](GUARDIANS.md) (the ⚠️ callout), [`RESIDENTS.md §1`](RESIDENTS.md), [`TOWNIES.md §1`](TOWNIES.md) (the ⚠️ "two other actions auto-grant townie" callout), and [`OPERATIONS.md §2`](OPERATIONS.md). **Design-intent settled 2026-06-30 (Jacob): the auto-grant model is intended** — both claim and residence reaching townie is embraced, not a loophole to tighten.
...
|---|---|---|---|
| `visitor` | glass / translucent | anonymous, no emoji set | `AvatarCircle` fallback |
| `resident` | burnt orange / brown | verified resident | review composer, lobby (`PlaceCard.jsx:2567`) |
| `guardian` | **teal / aquamarine** | claimed-business owner | on the card (`PlaceCard.jsx:1196`); post composer |
...
- **Holes in the medallion structure:** there is **no dedicated medallion for `townie`, `keyholder`, or `admin`** — and, by Jacob's 2026-06-30 decision, **keyholder deliberately gets none.** Rather than invent a keyholder badge, **business replies now present in the business voice** (logo + name, generic glass `visitor` Arch as the no-logo fallback) — see the ✅ note below. So there's no per-individual medallion on a reply to mislabel. The lobby (`PlaceCard.jsx:2567`) stays `resident`-anonymous by design (a keyholder-who-is-resident correctly reads "Resident" there). Townie/admin still have no badge beyond their emoji avatar — cosmetic, not pursued.

> ✅ **Business-voice replies (shipped 2026-06-30, `curb-offset-draw`).** Public replies on a place card used to show the *replier's personal `@handle` + emoji* (`PlaceCard.jsx:1336`), leaking a manager's identity — and the reply composer hardcoded a "Guardian" medallion even for a keyholder. **Fix:** a reply now always renders as **the business** — `listing.logo` (glass `visitor` Arch fallback) + `listing.name` — in both the composer preview and the public render; the emerald ✓ marks it an official response. Managers stay anonymous, and guardian-vs-keyholder becomes invisible to the public. Rights are unchanged (guardians always; keyholders only if granted the `replies` permission — off by default). Reachable "additional guardians" path confirmed: `postPromoteStaff` (`Code.js`) promotes a keyholder → full guardian.
...
| Check in (scan check-in QR) | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | none — logs a row (`postCheckin`, `Code.js`) |
| Set handle / avatar | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | format+unique only (`postSetHandle`, `Code.js`) |
| Post a **review** | ❌ | ✅ | ✅¹ | ✅² | ✅² | ✅² | `distinct_days≥3` (`postReview`, `Code.js`) |
| Post to **bulletin** | ❌ | ✅ | ✅¹ | ✅² | ✅² | ✅² | `isTownie` (`postBulletin`, `Code.js`) |
| **Comment** on bulletin | ❌ | ✅ | ✅¹ | ✅² | ✅² | ✅² | `isTownie` (`postComment`, `Code.js`) |
| Start a **DM thread** | ❌ | ✅ | ✅¹ | ✅² | ✅² | ✅² | `isTownie` (`postStartThread`, `Code.js`) |
| Read/post in a building **Lobby** | ❌ | ❌ | ✅ | ❌³ | ❌³ | ✅ | verified resident of *that* building (`getLobbyPosts`/`postLobbyPost`, `RESIDENTS.md §5`) |
| **Verify** a co-resident's pending claim | ❌ | ❌ | ✅ | ❌ | ❌ | ✅ | verified resident of the building (`postVerifyResident`, `Code.js`) |
| **Claim** a listing | ✅⁴ | ✅⁴ | ✅⁴ | — | — | ✅ | valid `claim_secret` (`postClaim`, `Code.js`) |
| **Claim** a residence | ✅⁶ | ✅⁶ | ✅⁶ | ✅⁶ | ✅⁶ | ✅ | auto-verify path / co-resident / admin (`postClaimResidence`, `Code.js`) |
| **Edit card** (whitelisted fields) | ❌ | ❌ | ❌ | per-field⁵ | ✅ all | ✅ | guardian or staff + `STAFF_PERM_MAP` (`postUpdateListing`, `Code.js`/`704`) |
| Upload / remove **photo** | ❌ | ❌ | ❌ | if `photos` | ✅ | ✅ | `staffHasPermission(..,'photos')` (`Code.js`/`2007`) |
| Post an **event** | ❌ | ❌ | ❌ | if `events` | ✅ | ✅ | `staffHasPermission(..,'events')` (`postEvent`, `Code.js`) |
| **Reply** to a review | ❌ | ❌ | ❌ | if `replies` | ✅ | ✅ | `staffHasPermission(..,'replies')` (`postReply`, `Code.js`) |
| Edit menu / hours | ❌ | ❌ | ❌ | if `menu`/`hours` | ✅ | ✅ | per-field `STAFF_PERM_MAP` (`Code.js`) |
| **Manage staff** (list/perms/promote/demote/revoke) | ❌ | ❌ | ❌ | ❌ | ✅ | ✅ | `isFullGuardianOf` (`Code.js`) |
| **Accept / remove** listing | ❌ | ❌ | ❌ | ❌ | ✅ | ✅ | `isFullGuardianOf` (`Code.js`/`751`) |
| **Fetch claim secret** | ❌ | ❌ | ❌ | ❌ | own listing | any listing | `isFullGuardianOf` or admin (`getClaimSecret`, `Code.js`) |
| Auto-verify a residence · auto-provision listing · setup photo folder | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | admin token (`OPERATIONS.md §1`) |

**Notes / footnotes:**
- ¹ A **Resident is auto-granted townie** at residence-verify (`grantTownieStatus`, `Code.js`), so a resident clears every townie gate.
- ² A **Guardian and Keyholder are auto-granted townie at claim** (`grantTownieStatus`, `postClaim` `Code.js`) — so they can review/bulletin/comment/DM **immediately**, including on *other* places (§b). *This is the corrected, as-built behavior; the specs once understated it.*
...
- ⁴ "Claim a listing" is open to any device holding a valid secret; the *role granted* depends on order (1st = guardian, rest = keyholder, `Code.js`). Residential listings have **no** guardian claim (`PlaceCard.jsx:2418`).
- ⁵ Keyholder edit rights are exactly the granted subset of `menu · hours · photos · replies · events` (`STAFF_PERM_MAP`); fields outside the map are guardian-only (`Code.js`).
...
1. **Townie auto-grant on claim & residence** (`Code.js` / / co-resident) — as-built, claiming a listing *or* verifying residence (any path) makes you a townie immediately. *(Documented across the cluster as of 2026-06-30.)* **Design-intent settled 2026-06-30 (Jacob): intended, embraced** — not a loophole.
...
3. **✅ RESOLVED 2026-06-30 — keyholder mislabel dissolved by business-voice replies** (not a new medallion, per Jacob). Replies render as the business (logo + name, glass `visitor` fallback) in composer + public render (`PlaceCard.jsx:1196`/), so no per-individual role badge shows on a reply. Lobby stays resident-anonymous by design; townie/admin still badge-less (cosmetic, not pursued). See §c.
...
6. **Auto-provision category mismatch** — `getClaimSecretAdmin` stands up `residential/houses` (`Code.js`), unsuitable for a restaurant claim (G.0).
...
Every procedure above was walked against the working tree on **2026-06-30** and passed; the pass
produced three fixes, all shipped (co-resident verify grants townie · `postClaim` rejects a
listing with no secret · business-voice replies). ⛔ **The blow-by-blow is retired to the Diary**
— [`_archive/ONBOARDING-phase2-verification-2026-06-30.md`](../_archive/ONBOARDING-phase2-verification-2026-06-30.md)
— because a point-in-time evidence table is exactly the thing that rots: its line numbers were
already wrong before it moved. The two gaps it left open are tracked in
