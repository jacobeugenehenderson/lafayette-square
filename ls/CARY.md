...
⛔ **And it is doing a job it cannot do. The card DISPLAYS priced modifiers the cart cannot order** — `MenuItem` renders `item.modifiers` with their price deltas (`PlaceCard.jsx:3290–3313`) off real authored data (`src/data/lafayette-square/menus.json:63` and 9 more), while `setQty` keys the cart on `sectionIdx-itemIdx` alone (`L2900`). So a customer reads "12 pc +$42.00" and has no way to choose it; the intent has to be retyped into the note, un-priced, and the total is wrong by the delta. This is a **live defect today**, not a v2 seam — ⭐ shipping v1 with note-only modifiers ships a menu that advertises options it won't sell.
