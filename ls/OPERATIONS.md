...
- **The alert text carries a tap-through link** — `Reply: <site>/?msg=<id>` — which opens the site straight into that conversation. One tap from the notification to the reply box; the admin session persists per browser, so the passphrase is asked once per device. ⛔ The link carries the **message id, never the device hash** (the hash is a bearer token and would then live in your message history and Twilio's logs). Needs `CONTACT_SITE_URL` set per installation; **unset means no link rather than a guessed domain**, and it says so in the logs.
- **The account menu shows an unread badge** when messages have arrived since you last opened the Inbox. The watermark is per-device (`localStorage`), which is fine for one operator and costs no schema. ⛔ If the count can't be fetched the badge is left **as-is, never zeroed** — "couldn't ask" must not render as "nothing waiting".
- The visitor's side is already handled: their chat **auto-opens** on their next visit if a reply is waiting, and **polls every 25s** while they're on the site, so your reply appears without a refresh. *(It used to use a Realtime subscription; migration `009` locked `sms_messages` down and Realtime enforces RLS, so it silently stopped delivering. Polling goes through an edge function on the service role and cannot break that way.)*
...
- **Water is DATA-DRIVEN, and `glint` is the only knob on it.** ⭐ `glint` is a **multiple of the physically correct wave slope**: `1` (the default) is the real ocean at the town's real wind speed, `0` is a dead-flat surface and is the *exact* pre-glint appearance — which is what makes it the falsifiable end of the dial and worth keeping. LS's pond currently ships `glint: 0` (`LafayettePark.jsx`), so the one surface an operator already knows by eye has not moved; flipping that one argument gives it the same water every other body has.
  - ⛔ **Everything else about the water is DERIVED, not authored, and should stay that way.** Wave size comes from the body's own extent, drift speed from the physics of a wave that length (`c = √(gL/2π)`), and roughness from **live wind** — `windSpeedMs` / `windDirDeg`, polled from Open-Meteo for the town's own coordinates. ⚠️ **That is the raw weather poll, not the meteorologist's directive `wind` block** (which is still owed, above) — the water does not wait on it. ⭐ A second town differs because its *data* differs; there is no per-town water tuning to do, and a knob here would be a number every new town had to re-guess.
  - ⭐ **THE STAGE NOW SHOWS THE REAL WATER.** In a Stage shot the Designer was ALSO painting the lake — a flat colour swatch stacked on top of the slab's real material — so the operator saw the swatch and never the shader. In a shot the slab owns the water now; the Designer keeps its swatch only in DESIGNER mode, where there is no slab and a flat floor is what you author against. ⛔ What you see in Stage is what ships.
  - ▶ **Before tuning any water layer, run `node checks/water-layer-budget.mjs`** — it prints what each layer contributes at a given camera pitch. ⛔ At an overhead pitch Fresnel is ~0.02, so every reflection layer is ~2% and the BODY is 98% of the picture: tuning reflections cannot change an overhead shot, and no screenshot will tell you that.
  - ▶ `node checks/claims-water-scales-with-its-body.mjs` · `node checks/claims-every-water-body-reaches-the-kit-material.mjs` · `node checks/claims-both-surfaces-draw-the-same-water.mjs`
...
- **⚠️ NOTHING IS SELLABLE UNTIL IT IS CONFIRMED** — `commerce_items` (migration `018`). A menu price is a **display figure**; the chargeable **price of record** is a separate, confirmed value in Cary. An item nobody has confirmed shows on the menu and cannot be added to a cart, and that is the intended default rather than a fault. ⭐ **The operator cost, named:** before a restaurant can sell, someone confirms its items once — a sitting at the bar, not a project. That is what buys the guarantee that no customer is ever charged a number nobody checked. Per-restaurant knobs live beside it in `commerce_places`: `ordering_paused` (stop taking orders now), `min_order_cents` (default 4000), and `tax_remitter`.
  - ⛔ **`tax_remitter` defaults to `undetermined`, and that is not a synonym for "us."** It records who remits sales tax, pending the Missouri DOR marketplace-facilitator letter ruling (`cary/legal/legal-readiness-brief.md §A`). It is a field rather than an assumption in the arithmetic precisely so the ruling is a value change, not a migration.
  - **Where the operator turns them:** the place card's **Delivery prices** panel (guardian-visible, under the menu). It lists every item with its menu price pre-filled, so confirming a correct menu is a glance and a tap rather than re-typing; per-item **86** and **Pause all ordering** live in the same panel. ⭐ The header reads *"N of M confirmed"* — that number is the one to watch before a restaurant goes live.
  - ⚠️ **Status: written, NOT YET DEPLOYED.** Four operator steps, all outside this repo, and ⛔ **all four are required — three of four leaves prices unconfirmable with no error that names the missing one**:
    1. apply `cary/supabase/migrations/018_commerce_menu.sql` to the hosted project
    2. deploy the `commerce-write` edge function
    3. set Supabase secrets `GAS_API_URL` and `COMMERCE_SHARED_SECRET`
    4. redeploy Apps Script (it gains the `guardian-check` action) and set the matching script property `COMMERCE_SHARED_SECRET`
  - ▶ `node checks/claims-price-of-record.mjs` · `node checks/claims-commerce-write-gate.mjs`

- **The sales-tax rate is AUTHORED PER INSTALLATION** — `commerce.salesTaxRate` (`src/instances/<look>.js`). It is the **jurisdiction's combined rate**, applied to **food only, never to the delivery fee** (LS `0.08725` = MO 4.225% + St. Louis 4.5%; HiPointe ships `0.09238` as an explicit placeholder to refine before delivery goes live there). ⭐ **Huron ships `null`, deliberately, and that is the better pattern**: delivery is off there, so no rate is needed, and a placeholder figure is a plausible-looking wrong number sitting where a legal value belongs. ▶ Set it from the county authority when `modules.delivery.enabled` flips, not before. ⛔ **A wrong rate is a wrong charge on a real transaction** — there is no gate that catches it, so it is set once, per town, against the local authority. ⚠️ **Until 2026-09-10 the order summary's label was the hardcoded string `Tax (8.725%)`** while the arithmetic used the instance rate — so a non-LS town displayed LS's rate and charged its own. The label now derives from the rate; the number on screen and the number charged cannot disagree again.
...
- **`CONTACT_SITE_URL`** (Supabase secret) is what puts the tap-through reply link in the alert SMS. Per installation, no default — an unset value ships an alert with no link rather than pointing the Host at someone else's town.
...
- ⛔ **Inbound SMS is NOT being forwarded to email, silently.** `SENDGRID_API_KEY` is not set in the Supabase project (`supabase secrets list`), and `sms-webhook` guards the forward with a bare `if (sendgridKey) { … }` and no `else`. So an unset key looks **exactly like a working forward** — texts land in `sms_messages` and reach nobody's inbox, with no error anywhere. ⭐ The defect is the silence, not the setting: whether forwarding *should* be on is Jacob's call, but "off" must not be indistinguishable from "working." Either set the key or make the disabled path say so.
