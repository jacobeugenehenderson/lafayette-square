...
Last updated: 2026-05-13 night (Phase C deep in flight — Slab Completeness sweep at 6/7 shipped: SC.1 / SC.2 / SC.3 / SC.5 / SC.7 all shipped + SC.4 audited empty; only SC.6 (clouds) remains pending Jacob's strip-vs-wire call. **Couplers §6 INSTANCE shipped** — `src/instance.js` carries lookId / geography / Cary contact; all LS-name / lat-lon / SMS-number hardcodes in `src/` retired. Build-break + elevation.js TDZ from b24fce5 also fixed (vite target → es2022 + assetsInclude `**/*.bin`; broke terrainShader↔elevation cycle). Remaining pre-merge: SC.6, Neon authoring controls, Cleanout plan, RUNTIME-DELTA §3 staging walk. Post-merge cleanup queued: bake-button UX refactor (Stage's vestigial ↻ → "Preview" button per project_preview_equals_ls_literally). See `cartograph/BACKLOG.md` "Slab completeness" for per-channel state.)
...
- ⏳ **Slab completeness SC.1–SC.7 — merge gate, mid-sweep.** SC.1 ✅ (sky/lighting/celestial, `c333e50`). SC.2 + SC.3 ✅ (post-FX + exposure + grade + grain + shadow + envState retirement, `015d8e0`). SC.5 ✅ (per-shot camera + Hero motion + Browse heading, `e8ba8d6`). SC.7 ⏳ underway (arch + horizon, last archState retirement, GatewayArch fork consolidation). SC.6 ⏳ pending Jacob's strip-vs-wire decision on clouds. SC.4 audited empty. Net through 4 commits: ~−1700 LOC, 4 forked consumers retired, 2 module-scope state bridges retired, 24 authored channels now ship through the slab.
