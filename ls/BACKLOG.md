...
| **Landmark labels need their own controls (distinct from street labels)** | "Landmark labels" (the park title — `LAFAYETTE PARK` / `EST. 1851 · ST. LOUIS, MO`, `ParkTitle` in `LafayettePark.jsx`) are a **different class** from street labels and need their **own operator controls**, not the shared `labels`/`parkTitle` toggle (Jacob, 2026-06-26). They sit *in the world* on the terrain (drei `<Text>`, terrain-lifted via `ElevatedGroup`), so unlike flat 2D street labels they care about **terrain occlusion, height/clearance, position, and per-landmark visibility**. Today: hardcoded position constants (`LABEL_TITLE_POS` = `-PARK_HALF*0.35` north of center) + a fixed vertical clearance. **Fix landed 2026-06-26** for the terrain-clip ("half-drowned" label): `ElevatedGroup` now lifts to the terrain MAX over the whole word's footprint (disc sample, not an 8-spoke ring) + a 4 m clearance — exag-proof, depth-correct. **Backlog:** a Landmark-Labels control group (visibility · position · height/clearance · size), separate from the street-label toggle, as the kit grows more landmarks. Non-urgent. |
