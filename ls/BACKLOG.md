...
| **Onboarding field-procedure + wiring audit — the trust-role trio** (`HANDOFF-logistician-onboarding.md`) | **DISPATCH-READY** (Boz; refocused 2026-06-30 to Jacob's trio). Turn the diamond specs (`GUARDIANS`/`RESIDENTS`/`TOWNIES`/`QR-CODES`/`IDENTITY`/`OPERATIONS`) into a field procedure for **Guardian (instant) · Resident (instant + auto-grants Townie) · Townie (earned-or-auto)** + the per-role **referral loop**. Two-phase: prose → Jacob's eye → code-verify. Heart is §Massage & fortify (frictionless identity · role-independence · medallions · the fortified rights matrix). Output = `ls/ONBOARDING.md` + rights matrix + readiness verdict. Parallel to the render-pipeline arc (no render files). `Agent: FRESH`. |
