...
| **Landmark labels need their own controls (distinct from street labels)** | "Landmark labels" (the park title — `LAFAYETTE PARK` / `EST. 1851 · ST. LOUIS, MO`, `ParkTitle` in `LafayettePark.jsx`) are a **different class** from street labels and need their **own operator controls**, not the shared `labels`/`parkTitle` toggle (Jacob, 2026-06-26). They sit *in the world* on the terrain (drei `<Text>`, terrain-lifted via `ElevatedGroup`), so unlike flat 2D street labels they care about **terrain occlusion, height/clearance, position, and per-landmark visibility**. Today: hardcoded position constants (`LABEL_TITLE_POS` = `-PARK_HALF*0.35` north of center) + a fixed vertical clearance. **Terrain-clip fix is INCOMPLETE (re-confirmed 2026-06-27 — still reads "AFAYETTE PARK").** The 2026-06-26 `ElevatedGroup` disc-sample (lift to terrain MAX over a `footprintRadius=34` m disc, 2 rings × 12 spokes, + 4 m clearance) helped but the word "LAFAYETTE PARK" at fontSize 6 is **wider than the 34 m disc**, so the leftmost **L** sits over un-sampled higher ground and depth-clips. It's the same class as before, not gone. **⭐ Root fix (Jacob, 2026-06-27): pull landmark labels OUT of the terrain-occludable stack — render them in the label/"ribbons" stack like street labels** (which never depth-fight the terrain) so position stops being a clip-roulette; we keep getting bit precisely because they live *on* the heightfield. **Stopgap queued (2026-06-27):** a manual nudge — bump `footprintRadius` 34→~55 to clear the whole word + move `LABEL_TITLE_POS`/`SUBTITLE_POS` into the natural center-left clearing — **deferred until the park-footpath agent releases `LafayettePark.jsx`** (two-writer collision). **Backlog (the real knob):** a Landmark-Labels control group (visibility · position · height/clearance · size · stack-mode: grounded vs overlay), separate from the street-label toggle, as the kit grows more landmarks — this is the per-installment knob so we never hand-nudge again. |
