...
| **Arborist roster swap** | Trees library upgrade post-stability. Hot-swappable into `public/baked/<look>/trees/` + `public/baked/<scene>/trees.json`; no LS app code changes needed. |
...
| **Onboarding field-procedure + wiring audit — the trust-role trio** (`HANDOFF-logistician-onboarding.md`) | **✅ LANDED 2026-06-30 (agent "Ferry", `curb-offset-draw`).** [`ls/ONBOARDING.md`](ONBOARDING.md) = the trust-role trio playbook (**Guardian · Resident · Townie**) — diamond procedures + per-role referral loops + readiness verdict + the fortified rights matrix; **Phase 1 prose + Phase 2 code-verify both complete.** Phase 2 found gaps and landed fixes: **co-resident verify now grants townie** (`Code.js`), **`postClaim` rejects a no-secret listing** (`Code.js`), **business-voice replies** (reply renders as the business, not the staffer — `PlaceCard.jsx`; dissolved the keyholder-medallion question — no keyholder badge). Sibling specs swept to match (`GUARDIANS`/`RESIDENTS`/`TOWNIES`/`OPERATIONS`/`FEATURES`/`STATUS`). Backend fixes ride the next GAS deploy; frontend on the next `curb-offset-draw` deploy. **Follow-ups (below):** generic business place-card templates · honorary townie. |
...
| **L1.3 — Hybrid buildings bake** | **✅ SHIPPED 2026-05-26** — `SlabBuildings` (merged mesh + render-scoped per-building index `buildings.json v2`) is the production + Preview consumer; identity resolves `raycast → id` against the slab index. Brief landed → `_archive/handoffs/`; fact home `SLAB-CONTRACT.md §6`. |
...
| **Landmark labels need their own controls (distinct from street labels)** | "Landmark labels" (the park title — `LAFAYETTE PARK` / `EST. 1851 · ST. LOUIS, MO`, `ParkTitle` in `LafayettePark.jsx`) are a **different class** from street labels and need their **own operator controls**, not the shared `labels`/`parkTitle` toggle (Jacob, 2026-06-26). They sit *in the world* on the terrain (drei `<Text>`, terrain-lifted via `ElevatedGroup`), so unlike flat 2D street labels they care about **terrain occlusion, height/clearance, position, and per-landmark visibility**. Today: hardcoded position constants (`LABEL_TITLE_POS` = `-PARK_HALF*0.35` north of center) + a fixed vertical clearance. **✅ TERRAIN-CLIP FIXED 2026-06-30 (`e66d85aa`, shipped to prod/`main`)** via the root fix below: the leading **L** no longer clips ("AFAYETTE PARK" → "LAFAYETTE PARK"). The 2026-06-26 `ElevatedGroup` disc-sample (lift to terrain MAX over a `footprintRadius=34` m disc) couldn't win — the word at fontSize 6 is wider than the 34 m disc, so the leftmost L sat over un-sampled higher ground (or a foreground tree) and depth-clipped, and any rigid lift that clears it floats the *whole* word off the grass. **⭐ Root fix applied (Jacob's 2026-06-27 direction): pull the label OUT of the terrain-occludable depth stack** — `depthTest`/`depthWrite=false` on the SDF material via drei `<Text onSync=…>` (reapplied each troika sync), `renderOrder=16` unchanged. The label now always renders regardless of terrain OR tree occlusion, at its current height (no new float), matching the street-label model that never depth-fights terrain. The `ElevatedGroup` lift is left in place (harmless, still sets a sane height). **✅ THE EYE-GATE FIRED, AND IS NOW CLOSED (2026-07-23).** The 2026-06-30 caveat — always-on-top means at oblique angles the label draws over foreground trees — came true once Jacob moved the title into the park. **Occlusion is now the CALLER's choice:** `ParkTitleMesh({ occlude })` sets `depthTest`, and the 3D scene (`ParkTitle`) passes `occlude` while the 2D Designer map (`MapLayers`) keeps it OFF (there the terrain-displaced ground would clip the quad and there are no trees to hide behind). Trees are opaque (`alphaTest`, `depthWrite` on) so they occlude correctly against `renderOrder=16` in the transparent queue. ⭐ The terrain-clip that motivated always-on-top was a **placement** problem — moving the word into the clearing removed the cause, so the depth hack could be retired rather than carried. **Placement is DONE** (Jacob wrangled the title into the park, 2026-07-23; the moved position rides `parkTitlePos` / `ParkTitleHandle`) and the per-landmark control group below. **Backlog (the real knob):** a Landmark-Labels control group (visibility · position · height/clearance · size · stack-mode: grounded vs overlay), separate from the street-label toggle, as the kit grows more landmarks — this is the per-installment knob so we never hand-nudge again. |
| **Generic place-card templates by business type** | Today the admin auto-provision path (`getClaimSecret`, `Code.js`) stands up a `residential/houses` stub, so a **restaurant that's mis-categorized as a house** starts with the wrong card. **Jacob's workflow ruling (2026-06-30):** don't pre-fix category — when the owner runs the **"Is this your place?"** claim, Jacob **switches the card** to the right kind at that moment. **The real to-do:** author **generic starter place-cards for several business kinds** (dining, retail, service, etc.) so the swap drops in a sensible template (fields/tabs/placeholders) instead of a bare shell. Turns gap #3 from a manual re-type into a one-click category swap. Home: `ls/ONBOARDING.md` gap #3 + `PLACE-CARDS.md`. |
| **Lobby posts don't recognise a linked device** | `getLobbyPosts` marks each post `is_mine` off a **single** `device_hash`, so on your linked laptop your own lobby posts do not show as yours — and since a lobby post carries no handle or avatar by design, `is_mine` is the *only* thing that identifies it to you. ⭐ Same family as the townie fix of 2026-09-10 (standing belongs to the person, not the handset) and the same one-symbol shape: `getLinkedHashes` is already imported in that file and used by its neighbours. ⚠️ Smaller blast radius — it misreads ownership rather than refusing an action — and it rides a **GAS deploy**. Found while fixing townie; home `ls/RESIDENTS.md §4`. |
| **Honorary townie — admin-bestowed instant local** | **Jacob's idea (2026-06-30):** let Jacob **grant instant townie-hood to anyone, regardless of geography** (a person of note, a remote friend, a partner) — an "honorary townie." Mechanically the grant already exists (`grantTownieStatus`, `Code.js`, backfills synthetic check-ins) but is only called internally by claim/residence-verify — there is **no admin endpoint** to bestow it directly. To-do: expose an **admin `grant-townie`** action (device-hash or `@handle` target) + a Code-Desk/admin affordance. Fills the townie referral gap (#4) with a top-down lever. Weigh against the "earned local" integrity of the badge (`TOWNIES.md`). Home: `ls/ONBOARDING.md` gap #4. |
| **Phone-slice frustum culling (trees + buildings)** | The portrait phone preview only shows a narrow **vertical slice** of the neighborhood at any moment (Jacob, 2026-06-24) — so off-slice geometry SHOULD frustum-cull. That's the key mobile-perf lever (the phone preview, not the wide desktop view, is the real measurement surface). **Trees are the only hog; buildings are cheap.** ⚠️ **Tree per-tile culling is OFF** — `InstancedTrees.jsx frustumCulled={false}` since 2026-06-21 (`fe3f88c7`: the per-tile InstancedMesh bounds culled *visible* trees on the hero pan; commit's own follow-up: "revisit correct per-tile cull bounds if mobile perf needs it"). 2026-06-24 two bound-fix attempts (three.js `computeBoundingSphere` + an explicit bound) **both over-culled** — three.js's InstancedMesh auto-cull is unreliable here (geometry baked tree-local + `matrixWorld` interaction). **Robust path (deferred, not built): manual world-space per-tile `.visible` culling** — test each (url×tile) InstancedMesh's world AABB vs the camera frustum in a `useFrame`; everything in world space, sidesteps `boundingSphere`/`matrixWorld`. Don't iterate it blind — every attempt black-screened Preview *and* Stage; needs an operator eye-verify. **Buildings:** `SlabBuildings.jsx` renders 9 **whole-map** kind-groups (foundation/wall/roof), `frustumCulled={false}`, **NOT spatially tiled** (`buildings.json` has no `tiles` key) — flipping the flag would cull nothing (the bound spans the map). Cheap at this map (34K tris / 9 draws); **were split into spatial cull-groups once** — a larger neighborhood will make them a hog, so **re-instate spatial cull-groups** (chunk each kind-group into tiles, like the trees). Full capture: `memory/tree-building-frustum-culling.md`. |
