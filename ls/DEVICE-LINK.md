...
Identity is per-device (`lsq_device_hash`), so by default your phone and your laptop are *different* neighbors. **Linking** ties a second device to the same `@handle` (and avatar/vignette) so the same identity follows you across them. Linked devices sharing a handle is also what powers the **linked-device auto-verify** paths for guardians and residents (see [`GUARDIANS.md`](GUARDIANS.md), [`RESIDENTS.md`](RESIDENTS.md)) — **and, since 2026-09-10, townie standing too** ([`TOWNIES.md §1`](TOWNIES.md)). ⭐ That was the last gate where linking did not carry your standing across, which is what the gesture is *for*.
...
The scanning device opens `/link/<token>` → `LinkPage.jsx` → `claimLinkToken(token, deviceHash)` (`postClaimLinkToken`, `apps-script/Code.js`), which registers it under the shared handle. The originating device **polls** `checkLinkToken` (`Code.js`) and updates when the link is claimed (status `pending` → `claimed`, or `expired` after 5 min).
...
|---|---|---|
| Link UI + token QR + poll | `src/App.jsx` | — |
