...
const node=[424.4,-88.7], c=[428,-78], W=70, px=1800, sc=px/W
...
s+='</svg>'
writeFileSync(new URL('./ne-curb-trace.svg',import.meta.url),s)
await sharp(Buffer.from(s)).png().toFile(new URL('./ne-curb-trace.png',import.meta.url).pathname)
console.log('ok')
