...
// Outputs:
//   - scratch/corner-input-audit.csv          (one row per corner)
//   - scratch/corner-input-audit-mississippi-park.json (deep dump)
...
for (const corner of allCorners) {
  ...
  const prevSpan = spans[(arcIdx-1+spans.length) % spans.length]
  const nextSpan = spans[(arcIdx+1) % spans.length]

  // Resolve flanking metas via the same path buildFrontageBandsV2 uses.
  ...
  if (!Bmeta && !Ameta) {
    ...
    row.notes = 'both flanking straight-spans skip'
    rows.push(row)
    continue
  }
  ...
  } else if ((Bmeta && !Ameta) || (Ameta && !Bmeta)) {
    row.failure_mode = 'wrong_flanking'
    row.notes = (Bmeta?'prev only':'next only') + ' — meta-fallback produced symmetric depths from one-sided data'
  } else {
    row.failure_mode = 'ok'
  }
...
writeFileSync(join(ROOT, 'scratch', 'corner-input-audit.csv'), csv.join('\n') + '\n')
...
writeFileSync(
  join(ROOT, 'scratch', 'corner-input-audit-mississippi-park.json'),
...
writeFileSync(
  join(ROOT, 'scratch', 'corner-input-audit-summary.json'),
  JSON.stringify(summary, null, 2),
)

console.log(``)
console.log(`Wrote: scratch/corner-input-audit.csv (${rows.length} rows)`)
console.log(`Wrote: scratch/corner-input-audit-mississippi-park.json`)
console.log(`Wrote: scratch/corner-input-audit-summary.json`)
