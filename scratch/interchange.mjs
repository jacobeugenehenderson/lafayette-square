...
const W=820,minx=129-W/2,miny=434-W/2,px=1700,sc=px/W,X=x=>((x-minx)*sc).toFixed(1),Y=y=>((y-miny)*sc).toFixed(1)
let s=`<svg xmlns="http://www.w3.org/2000/svg" width="${px}" height="${px}" style="background:#161616">`
const path=(rings,fill)=>{let dd='';for(const rr of(rings||[])){if(!rr||rr.length<3)continue;dd+=rr.map((p,i)=>(i?'L':'M')+X(p[0])+' '+Y(p[1])).join(' ')+' Z '}if(dd)s+=`<path d="${dd}" fill="${fill}" stroke="#c00" stroke-width="0.3" stroke-opacity="0.5"/>`}
...
writeFileSync(new URL('./ix-'+tag+'.svg',import.meta.url),s)
await sharp(Buffer.from(s)).png().toFile(new URL('./ix-'+tag+'.png',import.meta.url).pathname)
console.log('wrote ix-'+tag)
