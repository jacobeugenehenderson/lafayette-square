...
const cx=-355, cy=-105, W=28, px=1000
...
const path=(rings,fill,stroke)=>{let dd='';for(const rr of(rings||[])){if(!rr||rr.length<3)continue;dd+=rr.map((p,i)=>(i?'L':'M')+X(p[0])+' '+Y(p[1])).join(' ')+' Z '}if(dd)s+=`<path d="${dd}" fill="${fill}" fill-rule="evenodd" ${stroke?`stroke="${stroke}" stroke-width="1"`:''}/>`}
...
path(pr.sidewalk,'#e8e3d6','#b00')   // sidewalk with red outline to see rings
path(pr.curb,'#c89b6a'); path(pr.asphalt,'#4a4a4a')
s+='</svg>'
await sharp(Buffer.from(s)).png().toFile(new URL('./fill-albion-tight.png',import.meta.url).pathname)
console.log('wrote, sidewalk rings:',(pr.sidewalk||[]).length)
