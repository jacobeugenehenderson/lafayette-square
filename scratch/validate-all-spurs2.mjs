...
// match tiles by iA bbox (stable: iA doesn't move). hash iA.
...
const baseByBox=new Map(); g0._tiles.forEach(t=>baseByBox.set(bbox(t), iaHash(t)))
const rib2=JSON.parse(JSON.stringify(ribbons)); reshape(rib2,2.0)
const g1=buildTileGround(rib2, opts)
let identical=0,moved=0,unmatched=0
g1._tiles.forEach(t=>{ const b=baseByBox.get(bbox(t)); if(b===undefined){unmatched++;return} if(iaHash(t)===b)identical++; else moved++ })
console.log(`MATCH-BY-iA-BBOX: identical=${identical} moved=${moved} unmatched=${unmatched} (total ${g1._tiles.length})`)
// also: total iA verts sum (global byte check)
const sumV=(g)=>g._tiles.reduce((s,t)=>s+(t.iA||[]).reduce((a,r)=>a+r.length,0),0)
console.log(`total iA verts: base=${sumV(g0)} reshaped=${sumV(g1)}`)
// full-map iA hash
const allHash=(g)=>JSON.stringify(g._tiles.map(iaHash).sort())
console.log(`FULL-MAP iA byte-identical (order-independent): ${allHash(g0)===allHash(g1)}`)
