...
async function render(glb,atlasPng,out){
  const doc=await io.read(glb);const prim=getBark(doc);
  const pos=prim.getAttribute('POSITION').getArray(),uv=prim.getAttribute('TEXCOORD_0').getArray(),idx=prim.getIndices().getArray(),nrm=prim.getAttribute('NORMAL')?.getArray();
  const traw=await sharp(atlasPng).ensureAlpha().raw().toBuffer({resolveWithObject:true});
  const TW=traw.info.width,TH=traw.info.height,tp=traw.data,ch=traw.info.channels;
  // baked UVs address an atlas sub-region; they may exceed [0,1] for tiling within the region -> clamp-tile within region is complex; sample with REPEAT on full atlas
  const st=(u,v)=>{u-=Math.floor(u);v-=Math.floor(v);const x=Math.min(TW-1,Math.max(0,(u*TW)|0)),y=Math.min(TH-1,Math.max(0,((1-v)*TH)|0));const o=(y*TW+x)*ch;return[tp[o],tp[o+1],tp[o+2]];};
...
await render('scratch/ORIGBAKED-maple_sugar-lod0.glb','scratch/ORIG-bark-atlas.png','scratch/A-ORIG.png');
await render('public/baked/lafayette-square/trees/maple_sugar/skeleton-1-lod0.glb','public/baked/lafayette-square/trees-atlas-bark-color.png','scratch/B-NEW.png');
await sharp({create:{width:780,height:820,channels:3,background:{r:0,g:0,b:0}}}).composite([{input:'scratch/A-ORIG.png',left:0,top:0},{input:'scratch/B-NEW.png',left:400,top:0}]).png().toFile('scratch/LINDEN-PREDATES.png');
console.log('LEFT=ORIGINAL(HEAD, 10949t)  RIGHT=NEW-mine(14002t)');
