// Trammel E3.2 debug render — voussoir-svg + junction window/apron polys (cyan).
import { build, R, marks } from './voussoir-setup.mjs'
import fs from 'fs'
const [cx, cz, rad] = (process.argv[2] || '481.9,272.1,45').split(',').map(Number)
const out = process.argv[3] || 'scratch/trammel-view.svg'
const g = build()
...
// junction construction polys
for (const r of (g._jPolys || [])) if (r.some(inWin)) svg += `<path d="${path(r, 1)}" fill="rgba(0,220,220,0.25)" stroke="cyan" stroke-width="1"/>`
...
svg += `<text x="8" y="16" fill="#888" font-size="11">center ${cx},${cz} r=${rad} — N up, E right (axes flipped) — cyan = E3.2 junction polys</text></svg>`
