...
// also draw street centerlines to see the stubs
async function crop(name,cx,cy,W,ppx=1100){
  ...
  for(const st of r.streets){if(!st.points||st.points.length<2)continue;if(!hit(st.points))continue;s+=`<path d="${st.points.map((q,i)=>(i?'L':'M')+X(q[0])+' '+Y(q[1])).join(' ')}" fill="none" stroke="#e22" stroke-width="1.5" stroke-opacity="0.6"/>`}
  s+='</svg>'
  await sharp(Buffer.from(s)).png().toFile(new URL(`./w18-tight-${name}.png`,import.meta.url).pathname)
  console.log('wrote w18-tight-'+name)
}
await crop('Tjunc', 642, -412, 55)
await crop('stub', 548, -430, 50)
