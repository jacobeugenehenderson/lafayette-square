...
// Outputs:
//   - scratch/corner-input-audit-stage11a.csv          (one row per corner)
//   - scratch/corner-input-audit-stage11a-mississippi-park.json (deep dump)
...
for (const corner of allCorners) {
  ...
  function walkMeta(fromIdx, dir) {
    ...
    for (let step = 1; step < N; step++) {
      ...
      if (m?.type === 'straight' && !m.skip) return { meta: m, steps: step }
    }
...
writeFileSync(join(ROOT, 'scratch', 'corner-input-audit-stage11a.csv'), csv.join('\n') + '\n')
...
writeFileSync(
  join(ROOT, 'scratch', 'corner-input-audit-stage11a-mississippi-park.json'),
...
writeFileSync(
  join(ROOT, 'scratch', 'corner-input-audit-stage11a-summary.json'),
  JSON.stringify(summary, null, 2),
)

console.log(``)
console.log(`Wrote: scratch/corner-input-audit-stage11a.csv (${rows.length} rows)`)
console.log(`Wrote: scratch/corner-input-audit-stage11a-mississippi-park.json`)
console.log(`Wrote: scratch/corner-input-audit-stage11a-summary.json`)
