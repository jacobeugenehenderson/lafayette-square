...
import { smoothChain, jKey } from '../src/lib/smoothCenterline.js'
import { pickLuFromHash, hashKey, blockKeyFromRing, resolveChainSegmentation } from '../src/lib/buildBlockGeometryV2.js'
...
export function buildTileGround(ribbons, opts = {}) {
  ...
  for (const tile of tiles) {
    ...
    for (const f of fSink) {
      ...
      __vis.push(vi)
      const __k = cornerKeyAt(tile.ring[vi], tile.edges, vi)
      __keysThisTile.add(__k)
      cornerFillets[cornerKeyAt(tile.ring[vi], tile.edges, vi)] = { C: f.C, r: f.r, tA: f.tA, tB: f.tB, apex: f.apex }
    }
    if (globalThis.__CALIPER) {
      // which sharp tile-ring corners got NO fillet apex mapped to them?
      const mapped = new Set(__vis)
      const unmappedCorners = cornerIdxs.filter(i => !mapped.has(i))
      globalThis.__CALIPER.push({
        tileIdx: globalThis.__CALIPER.length,
        tileConvexCorners: cornerIdxs.length,
        blockRingCount: blockRings.length,
        blockRingAreas: blockRings.map(r => Math.round(Math.abs(signedArea(r)))),
        fSinkCount: fSink.length,
        distinctKeys: __keysThisTile.size,
        keyCollisions: fSink.length - __keysThisTile.size,
        unmappedCornerCount: unmappedCorners.length,
        unmappedCornerPts: unmappedCorners.map(i => [Math.round(tile.ring[i][0]*10)/10, Math.round(tile.ring[i][1]*10)/10]),
        tileArea: Math.round(Math.abs(signedArea(tile.ring))),
      })
    }
