...
async function crop(name, cx, cy, W, H, px=1100){
  ...
  // VOID = magenta base; intended grass median painted green over it; blocks cream; then road
  ...
  for(const st of r.streets){const col=LEGS[st.skelId||st.id];if(!col)continue;const p=st.points;s+=`<path d="${p.map((q,i)=>(i?'L':'M')+X(q[0])+' '+Y(q[1])).join(' ')}" fill="none" stroke="${col}" stroke-width="2.5"/>`;
    for(const e of [p[0],p[p.length-1]]) s+=`<circle cx="${X(e[0])}" cy="${Y(e[1])}" r="5" fill="${col}"/>`}
  s+='</svg>'
  await sharp(Buffer.from(s)).png().toFile(new URL(`./spline-${name}.png`,import.meta.url).pathname)
  console.log('wrote spline-'+name+'.png ('+px+'x'+Hpx+')  void=magenta median=green block=cream')
}
await crop('void-full', 535, -82, 380, 700)
// extra crops appended
