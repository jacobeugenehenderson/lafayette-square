...
async function crop(name, cx, cy, W, px=1200){
  ...
  for(const st of r.streets){ if(!st.points||st.points.length<2)continue; const p=st.points; let inside=false; for(const q of p) if(q[0]>minx&&q[0]<minx+W&&q[1]>miny&&q[1]<miny+W){inside=true;break;} if(!inside)continue; s+=`<path d="${p.map((q,i)=>(i?'L':'M')+X(q[0])+' '+Y(q[1])).join(' ')}" fill="none" stroke="#1414ff" stroke-width="2"/>` }
  s+='</svg>'
  await sharp(Buffer.from(s)).png().toFile(new URL(`./w18-${name}.png`,import.meta.url).pathname)
  console.log('wrote w18-'+name+'.png')
}
