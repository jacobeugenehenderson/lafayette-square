...
for(const t of cm.tiles){
  ...
  await new Promise(res=>loader.parse(ab,'',(gl)=>{
    ...
    if(m){ m.computeBoundingSphere(); m.boundsTree = new MeshBVH(m)
      const mesh=new THREE.Mesh(m,new THREE.MeshBasicMaterial()); mesh.raycast=acceleratedRaycast
...
console.log(`  meshes ${meshes.length}, ${Math.round(tris).toLocaleString()} triangles, frustumCulled=false, WITH BVH`)
