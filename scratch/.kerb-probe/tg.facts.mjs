...
import { walkSubstrate, tilesFromWalk } from '/Users/jacobhenderson/Desktop/lafayette-square.nosync/src/lib/substrateWalk.js'
...
export function buildTileGround(ribbons, opts = {}) {
  ...
  for (const tile of tiles) {
    ...
    ;(globalThis.__KERB ||= []).push({
      facts: curbFacts.map(f => f && ({ skelId: f.skelId, side: f.side, segOrd: f.segOrd, baseHW: f.baseHW, streetKey: String(f.streetKey), roadKey: String(f.roadKey), prof: f.prof })),
      runs: runs.map(r => ({ streetIdx: r.streetIdx, side: r.side, segOrd: runSegOrd(r), n: r.poly.length })),
      isMedianTile,
    })
