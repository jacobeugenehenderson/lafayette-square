...
export function buildTileGround(ribbons, opts = {}) {
  ...
  for (const tile of tiles) {
    ...
    if (opts.iaOffset !== false && !isMedianTile && ringArea > 1500) {
      ...
      _producer = 'carve'
      blockRings = legacyBlock()
      ;((ring, brs, af) => { if(!globalThis.__aOn) return
        const K=p=>`${p[0].toFixed(6)},${p[1].toFixed(6)}`
        const R=new Set(ring.map(K)); const F=new Set(); for(const r of af||[]) for(const p of r) F.add(K(p))
        let tot=0, onRing=0, onAf=0, neither=0
        for(const r of brs||[]) for(const p of r){ tot++
          const k=K(p)
          if(R.has(k)) onRing++; else if(F.has(k)) onAf++; else neither++ }
        const S=globalThis.__a; S.tiles++; S.tot+=tot; S.ring+=onRing; S.af+=onAf; S.none+=neither
        if(neither) S.badTiles++
      })(tile.ring, blockRings, aFill)
