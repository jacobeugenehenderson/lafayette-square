...
|---|---|---|---|---|---|
| **PA-1** | *"Runs on a phone"* / "mobile-viable canopy" (`SHOW-BIBLE §1`, `arborist/FEATURES`) | Tree LOD off (`lodForRole` always `'lod1'`, `InstancedTrees.jsx:728`); hero impostors don't load in the player (`impostors=0`); 27.6 MB uncompressed atlas; mobile budgets self-labeled "interim/unmeasured" | Land Column B; KTX2/Basis the atlas (27.6→~5 MB); then **measure** against the Galaxy-A54 floor | L | `ROADMAP` B1–B6 + H1 |
| **PA-2** | Octahedral / coverage-preserving impostors + distance LOD (BATON, arborist docs) | Not implemented — crossed-quad billboards only; `PROM_THRESHOLD=0` tags nothing impostor; no coverage mipmaps | Build the impostor render arc (or move it firmly to "planned" — see AP-1) | L | `ROADMAP` B3 · `BATON-tree-render-next.md` |
...
| **AP-2** ↓ | *(as first drafted:)* "*Runs on a phone* stated as **achieved**" | **OVERSTATED.** `cartograph/FEATURES.md:77` already scopes it honestly — names the two reference phones (iPhone 16 Pro Max ceiling / **Galaxy A54 floor**) and calls the measurement an *"in-flight v0.2 arc"* → `HANDOFF-preview-measurement.md`; `README.md:13` frames "works on a phone" as a *remaining-work column name*. The dev pitch does **not** claim it achieved. Mobile *budgets* being self-labeled "interim" is the true, smaller residual | Leave the FEATURES pitch as-is; only sharpen the SHOW-BIBLE slogan (AP-1). The *product* gap (measure it) is **PA-1**, not a doc fix | S | `SHOW-BIBLE §1` slogan |
