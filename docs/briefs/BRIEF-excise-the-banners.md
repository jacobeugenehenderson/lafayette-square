...
**Route first** (`CLAUDE.md` is the gate): `ORIENTATION.md` → `README §⭐ START HERE` → `docs/agents/BOZ.md §4`.

---

## 0. Why — and it is not tidiness

`CLAUDE.md §PRUNE AS YOU GO` / `docs/agents/BOZ.md §4` say **trim on subsume**: superseded content *migrates* to the dated `_archive/`;
...
2. **⛔ REPOINT EVERY REF IN THE SAME BREATH.** The dead pointer is the archivist's one unforgivable error
   (`docs/agents/BOZ.md §4`). After moving anything, grep the whole repo for its name/section and repoint to the LIVE
   home — never to the archive.
3. **Consolidate the paragraph you touch**, don't leave a scar where the banner was (`docs/agents/BOZ.md §4`).
