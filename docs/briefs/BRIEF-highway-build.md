...
evict-when: the seven checks are green on huron AND LS, and each open item below is closed or re-homed
-->

**What this is.** `ROADMAP H-3`'s live state. A motorway/trunk is the positive object H: an alignment swept by a
typical section built from its lanes and cited values, not ①'s negative space. The plan it was built from, step by
step, is in the Diary (`cartograph/_archive/BRIEF-highway-build-plan-2026-09-23.md`). **Rulings live in
...
| flare | The ramp-terminal flare RECORD, frozen at the mint (`rampTerminalFlares`); check 5 judges it by the drawn curb | `8769d512` `a7891249` | `claims-ramp-ends-classified` |
...
- **The flare DRAW** (branch `gantry/flare-on-new-2`, `flareWorkingCopy` on ②'s working copy): NOT LANDED. It needs the provincetown fixture bake, then `claims-the-curb-never-enters-the-road`, `claims-every-proto-edge-lies-on-its-owner` and `claims-ramp-ends-classified` on the combined tree. Until then check 5 is red "declared, not drawn" wherever a flare is frozen.
