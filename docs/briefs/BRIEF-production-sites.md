...
1. **Operations repo:** `/Users/jacobhenderson/Desktop/dev.nosync/theward-operations`. ⚠️ Corrected by Lintel: `src/domains.js` is the LOGIC; the records live in its D1 database (`wrangler.jsonc` binding `DB`). A town's domain = the Ward whose `mapId` is the map → its `domain`; owned = a `listed` domain record; zone state = `cloudflare.status` as of the last Operations sync. Read it; never copy its data here. **How Promote reads it is Jacob's call** — (a) a read-only `wrangler d1 execute --remote` SELECT, or (b) the Operations API with an Access service token.
